	...
	internal class SoapClient
	{
		...
		public Task<XmlDocument> InvokeAsync(string operationName, IDictionary<string, object> args)
		{
			...
			if (messageBody.Length != 0)
			{
				...
				task = Task.Factory.FromAsync((Func<AsyncCallback, object, IAsyncResult>)request.BeginGetRequestStream, (Func<IAsyncResult, Stream>)request.EndGetRequestStream, (object)null).ContinueWith(delegate(Task<Stream> requestSteamTask)
				{
					requestStream = requestSteamTask.Result;
					return Task.Factory.FromAsync(requestStream.BeginWrite, requestStream.EndWrite, messageBody, 0, messageBody.Length, null);
				}).Unwrap()
			...
			{
				task = GetWebResponse(request);
			}
			return task.ContinueWith(delegate(Task<WebResponse> task2)
			{
				using (WebResponse webResponse = task2.Result)
				{
		...
		{
			return Task.Factory.FromAsync((Func<AsyncCallback, object, IAsyncResult>)request.BeginGetResponse, (Func<IAsyncResult, WebResponse>)request.EndGetResponse, (object)null).ContinueWith(delegate(Task<WebResponse> task)
			{
