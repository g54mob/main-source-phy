	...
	public class DecalRoot : DecalBase
	{
		...
		private void RenderDecals(Camera cam)
		{
			...
			else
			{
				...
				BuiltinRenderTextureType builtinRenderTextureType = (cam.hdr ? BuiltinRenderTextureType.CameraTarget : BuiltinRenderTextureType.GBuffer3);
		...
		private CommandBuffer GetBuffer(Camera cam)
		{
			...
			else
			{
				...
				if (Application.isEditor)
				{
					...
					if (list.Exists((CommandBuffer p) => p.name == "Easy Decals"))
					{
						flag = false;
						commandBuffer = list.Find((CommandBuffer p) => p.name == "Easy Decals");
					}
				}
				if (flag)
				{
