...
namespace ch.sycoforge.Decal.Projectors
{
	internal class SkinnedBoxProjector : BoxProjector
	{
		...
		protected override void ProcessReceiver(GameObject receiver, int recursiveStepUp, int recursiveStepDown)
		{
			...
			receiver.GetComponentInChildren<MeshFilter>();
			SkinnedMeshRenderer componentInChildren = receiver.GetComponentInChildren<SkinnedMeshRenderer>();
			MeshLink componentInChildren2 = receiver.GetComponentInChildren<MeshLink>();
			...
			if (componentInChildren != null)
			{
				...
				{
					AddMesh(componentInChildren, receiver.transform, receiver.transform.localToWorldMatrix);
				}
			}
			else
			{
				if (!(componentInChildren2 != null) || !(componentInChildren2 is SkinnedMeshLink))
				{
					return;
				}
				SkinnedMeshLink skinnedMeshLink = (SkinnedMeshLink)componentInChildren2;
		...
		public static Mesh BakeNoScale(SkinnedMeshRenderer skin)
		{
			...
			for (int i = 0; i < vertices.Length; i++)
			{
				...
				if (Mathf.Abs(boneWeight.weight0) > float.Epsilon)
				{
					...
					array[i] += gameObject.transform.InverseTransformPoint(bones[boneWeight.boneIndex0].localToWorldMatrix.MultiplyPoint3x4(v)) * boneWeight.weight0;
				}
				if (Mathf.Abs(boneWeight.weight1) > float.Epsilon)
				{
					Vector3 v = bindposes[boneWeight.boneIndex1].MultiplyPoint3x4(vertices[i]);
					array[i] += gameObject.transform.InverseTransformPoint(bones[boneWeight.boneIndex1].localToWorldMatrix.MultiplyPoint3x4(v)) * boneWeight.weight1;
				}
				if (Mathf.Abs(boneWeight.weight2) > float.Epsilon)
				{
					Vector3 v = bindposes[boneWeight.boneIndex2].MultiplyPoint3x4(vertices[i]);
					array[i] += gameObject.transform.InverseTransformPoint(bones[boneWeight.boneIndex2].localToWorldMatrix.MultiplyPoint3x4(v)) * boneWeight.weight2;
				}
				if (Mathf.Abs(boneWeight.weight3) > float.Epsilon)
				{
					Vector3 v = bindposes[boneWeight.boneIndex3].MultiplyPoint3x4(vertices[i]);
					array[i] += gameObject.transform.InverseTransformPoint(bones[boneWeight.boneIndex3].localToWorldMatrix.MultiplyPoint3x4(v)) * boneWeight.weight3;
				}
			}
			mesh.vertices = array;
