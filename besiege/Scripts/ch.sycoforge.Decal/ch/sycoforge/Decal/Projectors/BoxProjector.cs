...
namespace ch.sycoforge.Decal.Projectors
{
	internal class BoxProjector : Projector
	{
		...
		internal override void Project()
		{
			...
			TerrainCollider terrainCollider = Array.Find(array, (Collider c) => c is TerrainCollider) as TerrainCollider;
		...
		protected virtual void ProcessReceiver(GameObject receiver, int recursiveStepUp, int recursiveStepDown)
		{
			...
			else if (componentInChildren2 != null)
			{
				...
				if (cullInvisibles)
				{
					renderer2 = receiver.GetComponent<Renderer>();
				}
				if (!cullInvisibles || (cullInvisibles && renderer2 != null && renderer2.enabled))
				{
			...
			for (int i = 0; i < triangles.Length; i += 3)
			{
				...
				for (int j = 0; j < 3; j++)
				{
					...
					if (InsideSphere(p, localCenter))
					{
						...
						for (int k = 0; k < 3; k++)
						{
							Vector3 item = vertices[array2[k]];
							Vector3 item2 = normals[array2[k]];
							Vector2 item3 = uv[array2[k]];
							Vector4 item4 = tangents[array2[k]];
							list.Add(count + k);
							list2.Add(item);
							list3.Add(item2);
							list4.Add(item3);
