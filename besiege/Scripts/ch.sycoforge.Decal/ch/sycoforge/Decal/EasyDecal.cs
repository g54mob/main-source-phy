...
namespace ch.sycoforge.Decal
{
	...
	public class EasyDecal : DecalBase
	{
		...
		{
			if (Source == SourceMode.Atlas && AspectCorrectionMode != AspectMode.None && Atlas != null && atlasRegionIndex < atlas.Regions.Count)
			{
		...
		private void CreateDecalGameObject()
		{
			...
			base.DecalRenderer.useLightProbes = useLightProbes;
		}
		...
		private static EasyDecal ProjectInternal(GameObject decalPrefab, GameObject parent, Vector3 position, Quaternion rotation, Vector3 scale)
		{
			...
			{
				easyDecal = ((GameObject)UnityEngine.Object.Instantiate(decalPrefab, position, rotation)).GetComponent<EasyDecal>();
			}
