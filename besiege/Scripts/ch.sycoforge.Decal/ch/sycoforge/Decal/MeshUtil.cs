		...
		internal static void AutoWeld(DynamicMesh mesh, float threshold, float bucketStep)
		{
			...
			for (int j = 0; j < vertices.Count; j++)
			{
				...
				while (true)
				{
					...
					{
						Vector3 a = list[array2[num5, num6, num7][num8]] - vertices[j];
						if (Vector3.SqrMagnitude(a) < threshold)
						{
							array[j] = array2[num5, num6, num7][num8];
							break;
						}
						num8++;
						continue;
			...
			{
				mesh.TriangleIndices[k] = array[triangleIndices[k]];
			}
			for (int l = 0; l < num; l++)
			{
				mesh.Vertices[l] = list[l];
			}
		}
