	...
	internal sealed class TextSection
	{
		private sealed class ExportTables
		{
			...
			internal uint Length
			{
				get
				{
					return (uint)((int)stubsRVA + (int)stubLength * text.moduleBuilder.unmanagedExports.Count) - text.ExportTablesRVA;
				}
			}
		...
		internal uint BaseRVA
		{
			get
			{
				return peWriter.Headers.OptionalHeader.SectionAlignment;
			}
		}
		...
		internal uint MethodBodiesRVA
		{
			get
			{
				return (ComDescriptorRVA + ComDescriptorLength + 7) & 0xFFFFFFF8u;
			}
		}
		...
		private uint ResourcesLength
		{
			get
			{
				return manifestResourcesLength;
			}
		}
		...
		internal uint StrongNameSignatureRVA
		{
			get
			{
				return (ResourcesRVA + ResourcesLength + 3) & 0xFFFFFFFCu;
			}
		}
		...
		private uint MetadataRVA
		{
			get
			{
				return (StrongNameSignatureRVA + StrongNameSignatureLength + 3) & 0xFFFFFFFCu;
			}
		}
		...
		private uint VTableFixupsRVA
		{
			get
			{
				return (MetadataRVA + MetadataLength + 7) & 0xFFFFFFF8u;
			}
		}
		...
		private uint VTableFixupsLength
		{
			get
			{
				return (uint)(moduleBuilder.vtablefixups.Count * 8);
			}
		}
		...
		internal uint DebugDirectoryRVA
		{
			get
			{
				return VTableFixupsRVA + VTableFixupsLength;
			}
		}
		...
		internal uint ExportDirectoryRVA
		{
			get
			{
				return (DebugDirectoryRVA + DebugDirectoryLength + DebugDirectoryContentsLength + 15) & 0xFFFFFFF0u;
			}
		}
		...
		private uint ExportTablesRVA
		{
			get
			{
				return ExportDirectoryRVA + ExportDirectoryLength;
			}
		}
		...
		internal uint ImportDirectoryRVA
		{
			get
			{
				return (ExportTablesRVA + ExportTablesLength + 15) & 0xFFFFFFF0u;
			}
		}
