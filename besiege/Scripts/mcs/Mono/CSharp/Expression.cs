...
namespace Mono.CSharp
{
	public abstract class Expression
	{
		...
		public virtual TypeSpec ResolveAsType(IMemberContext mc, bool allowUnboundTypeArguments = false)
		{
			...
			if (expression != null)
			{
				expression.Error_UnexpectedKind(resolveContext, ResolveFlags.Type, loc);
			}
			return null;
		}
		...
		{
			throw new NotImplementedException(string.Format("CloneTo not implemented for expression {0}", GetType()));
		}
