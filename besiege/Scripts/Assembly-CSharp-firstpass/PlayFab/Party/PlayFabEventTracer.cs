...
namespace PlayFab.Party
{
	internal sealed class PlayFabEventTracer : SingletonMonoBehaviour<PlayFabEventTracer>
	{
		...
		public PlayFabEventsInstanceAPI eventApi;
		...
		private IEnumerator WaitUntilEntityLoggedIn(float secondsBetweenWait)
		{
			...
			while (!PlayFabAuthenticationAPI.IsEntityLoggedIn())
			{
				yield return delay;
			}
			if (entityKey.Id == null)
		...
		public void DoWork()
		{
			...
			if (currentTimeInMilliseconds > lastErrorTimeInMillisecond + retryCount * 1000 && eventsRequests.Count > 0)
			{
