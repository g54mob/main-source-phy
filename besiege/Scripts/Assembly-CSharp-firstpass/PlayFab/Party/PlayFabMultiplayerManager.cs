...
namespace PlayFab.Party
{
	public class PlayFabMultiplayerManager : MonoBehaviour
	{
		...
		private const uint _c_ErrorLostConnection = 63u;
		...
		public Action OnNetworkInvalid;
		...
		public Action<ulong> onChatPermissionsChanged;
		...
		public bool GetChatPermissions(ulong targetXuid, out PARTY_CHAT_PERMISSION_OPTIONS options)
		{
			options = PARTY_CHAT_PERMISSION_OPTIONS.PARTY_CHAT_PERMISSION_OPTIONS_NONE;
			return true;
		}
		...
		{
			if (!_cleanupStarted && _logLevel != LogLevelType.None)
			{
		...
		internal static void _LogError(uint code, PlayFabMultiplayerManagerErrorType type)
		{
			...
			uint error = SDK.PartyGetErrorMessage(code, out errorMessage);
			if (PartyError.FAILED(error))
			{
		...
		private void CleanUp()
		{
			...
			if (!_cleanupStarted && _playFabMultiplayerManagerState > _InternalPlayFabMultiplayerManagerState.NotInitialized)
			{
		...
		private void GetEntityTokenCompleted(GetEntityTokenResponse response)
		{
			...
			PlayFab.ClientModels.EntityKey entityKey2 = entityKey;
			_CreateLocalUser(entityKey2, response.EntityToken);
		}
		...
		private void CreateAndJoinNetworkImplComplete(PlayFabNetworkConfiguration networkConfiguration)
		{
			...
			PARTY_NETWORK_CONFIGURATION pARTY_NETWORK_CONFIGURATION = new PARTY_NETWORK_CONFIGURATION();
			pARTY_NETWORK_CONFIGURATION.MaxDeviceCount = networkConfiguration.MaxPlayerCount;
			pARTY_NETWORK_CONFIGURATION.MaxDevicesPerUserCount = 1u;
			pARTY_NETWORK_CONFIGURATION.MaxEndpointsPerDeviceCount = 1u;
			pARTY_NETWORK_CONFIGURATION.MaxUserCount = networkConfiguration.MaxPlayerCount;
			pARTY_NETWORK_CONFIGURATION.MaxUsersPerDeviceCount = 1u;
			PARTY_NETWORK_CONFIGURATION networkConfiguration2 = pARTY_NETWORK_CONFIGURATION;
			PARTY_INVITATION_CONFIGURATION pARTY_INVITATION_CONFIGURATION = new PARTY_INVITATION_CONFIGURATION();
			pARTY_INVITATION_CONFIGURATION.Identifier = Guid.NewGuid().ToString();
			pARTY_INVITATION_CONFIGURATION.Revocability = PARTY_INVITATION_REVOCABILITY.PARTY_INVITATION_REVOCABILITY_ANYONE;
			pARTY_INVITATION_CONFIGURATION.EntityIds = null;
			PARTY_INVITATION_CONFIGURATION initialInvitationConfiguration = pARTY_INVITATION_CONFIGURATION;
		...
		internal void _SendDataMessage(byte[] buffer, IEnumerable<PlayFabPlayer> recipients, DeliveryOption deliveryOption)
		{
			...
			PartySucceeded(SDK.PartyEndpointSendMessage(_localEndPointHandle, targetEndpoints, options, _defaultQueuingConfiguration, buffer));
		}
		...
		internal ChatState _GetChatState(PlayFab.ClientModels.EntityKey entityKey, bool _isLocal)
		{
			...
			if (_isLocal)
			{
				if (_localChatControlHandle != null)
				{
					...
					SDK.PartyChatControlGetLocalChatIndicator(_localChatControlHandle, out chatIndicator);
			...
			else
			{
				...
				{
					PARTY_CHAT_CONTROL_CHAT_INDICATOR chatIndicator2;
					PartySucceeded(SDK.PartyChatControlGetChatIndicator(_localChatControlHandle, pARTY_CHAT_CONTROL_HANDLE, out chatIndicator2));
					switch (chatIndicator2)
					{
					case PARTY_CHAT_CONTROL_CHAT_INDICATOR.PARTY_CHAT_CONTROL_CHAT_INDICATOR_INCOMING_COMMUNICATIONS_MUTED:
						result = ((playFabPlayer == null || !playFabPlayer._mutedByPlatform) ? ChatState.Muted : ChatState.MutedByPlatform);
						break;
		...
		private void ProcessStateChanges()
		{
			...
			foreach (PARTY_STATE_CHANGE partyStateChange in _partyStateChanges)
			{
				...
				{
					PARTY_LOCAL_CHAT_AUDIO_INPUT_CHANGED_STATE_CHANGE pARTY_LOCAL_CHAT_AUDIO_INPUT_CHANGED_STATE_CHANGE = (PARTY_LOCAL_CHAT_AUDIO_INPUT_CHANGED_STATE_CHANGE)partyStateChange;
					uint errorDetail2 = pARTY_LOCAL_CHAT_AUDIO_INPUT_CHANGED_STATE_CHANGE.errorDetail;
				...
				{
					PARTY_LOCAL_CHAT_AUDIO_OUTPUT_CHANGED_STATE_CHANGE pARTY_LOCAL_CHAT_AUDIO_OUTPUT_CHANGED_STATE_CHANGE = (PARTY_LOCAL_CHAT_AUDIO_OUTPUT_CHANGED_STATE_CHANGE)partyStateChange;
					uint errorDetail = pARTY_LOCAL_CHAT_AUDIO_OUTPUT_CHANGED_STATE_CHANGE.errorDetail;
				...
				case PARTY_STATE_CHANGE_TYPE.PARTY_STATE_CHANGE_TYPE_NETWORK_DESTROYED:
				{
					...
					if (PartySucceeded(pARTY_NETWORK_DESTROYED_STATE_CHANGE.errorDetail))
					{
						if (_queuedCreateAndJoinAfterLeaveNetworkOp.queued || _queuedCompleteJoinAfterLeaveNetworkOp.queued)
						{
							Debug.Log(_queuedCreateAndJoinAfterLeaveNetworkOp.queued + ", " + _queuedCompleteJoinAfterLeaveNetworkOp.queued);
					...
					else if (pARTY_NETWORK_DESTROYED_STATE_CHANGE.errorDetail == 63)
					{
						Debug.Log(string.Concat(_playFabMultiplayerManagerState, " changed to Initialized as the player lost connection"));
						_playFabMultiplayerManagerState = _InternalPlayFabMultiplayerManagerState.Initialized;
					}
					break;
				}
				...
				case PARTY_STATE_CHANGE_TYPE.PARTY_STATE_CHANGE_TYPE_ENDPOINT_DESTROYED:
				{
					...
					if (!(entityId3 == _localPlayer.EntityKey.Id) || 1 == 0)
					{
				...
				{
					PARTY_CHAT_CONTROL_CREATED_STATE_CHANGE pARTY_CHAT_CONTROL_CREATED_STATE_CHANGE = (PARTY_CHAT_CONTROL_CREATED_STATE_CHANGE)partyStateChange;
					PARTY_CHAT_CONTROL_HANDLE chatControl = pARTY_CHAT_CONTROL_CREATED_STATE_CHANGE.chatControl;
				...
				case PARTY_STATE_CHANGE_TYPE.PARTY_STATE_CHANGE_TYPE_NETWORK_DESCRIPTOR_CHANGED:
				{
					...
					if (this.OnNetworkChanged != null)
					{
						...
						SDK.PartyNetworkGetNetworkDescriptor(network, out networkDescriptor);
						PARTY_INVITATION_HANDLE[] invitations;
						SDK.PartyNetworkGetInvitations(network, out invitations);
						string invitationId = string.Empty;
						PARTY_INVITATION_CONFIGURATION configuration;
						if (invitations.Length != 1 && PartySucceeded(SDK.PartyInvitationGetInvitationConfiguration(invitations[0], out configuration)))
						{
				...
				case PARTY_STATE_CHANGE_TYPE.PARTY_STATE_CHANGE_TYPE_ENDPOINT_MESSAGE_RECEIVED:
				{
					...
					if (_platformPolicyProvider != null)
					{
						_platformPolicyProvider.ProcessEndpointMessage(playerByEntityId3, pARTY_ENDPOINT_MESSAGE_RECEIVED_STATE_CHANGE.messageBuffer, pARTY_ENDPOINT_MESSAGE_RECEIVED_STATE_CHANGE.messageSize);
					}
					break;
				}
				...
				case PARTY_STATE_CHANGE_TYPE.PARTY_STATE_CHANGE_TYPE_POPULATE_AVAILABLE_TEXT_TO_SPEECH_PROFILES_COMPLETED:
				{
					...
					if (InternalCheckStateChangeSucceededOrLogErrorIfFailed(pARTY_POPULATE_AVAILABLE_TEXT_TO_SPEECH_PROFILES_COMPLETED_STATE_CHANGE.result, pARTY_POPULATE_AVAILABLE_TEXT_TO_SPEECH_PROFILES_COMPLETED_STATE_CHANGE.errorDetail))
					{
						...
						if (PartySucceeded(SDK.PartyChatControlGetAvailableTextToSpeechProfiles(pARTY_POPULATE_AVAILABLE_TEXT_TO_SPEECH_PROFILES_COMPLETED_STATE_CHANGE.localChatControl, out profiles)) && profiles.Length > 0)
						{
