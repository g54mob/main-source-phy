...
namespace PlayFab
{
	public static class PlayFabCloudScriptAPI
	{
		...
		{
			PlayFabAuthenticationContext playFabAuthenticationContext = ((request != null) ? request.AuthenticationContext : null) ?? PlayFabSettings.staticPlayer;
		...
		public static void ExecuteFunction(ExecuteFunctionRequest request, Action<ExecuteFunctionResult> resultCallback, Action<PlayFabError> errorCallback, object customData = null, Dictionary<string, string> extraHeaders = null)
		{
			...
			{
				Uri baseUri = new Uri(localApiServer);
				Uri uri = new Uri(baseUri, "/CloudScript/ExecuteFunction".TrimStart('/'));
				PlayFabHttp.MakeApiCallWithFullUri(uri.AbsoluteUri, request, AuthType.EntityToken, resultCallback, errorCallback, customData, extraHeaders, playFabAuthenticationContext, staticSettings);
			}
			else
			{
