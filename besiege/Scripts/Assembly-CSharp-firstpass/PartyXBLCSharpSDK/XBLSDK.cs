...
namespace PartyXBLCSharpSDK
{
	public class XBLSDK
	{
		...
		public static uint PartyXblInitialize(string titleId, out PARTY_XBL_HANDLE handle)
		{
			...
			uint error = PartyXblInterop.PartyXblInitialize(IntPtr.Zero, Converters.StringToNullTerminatedUTF8ByteArray(titleId), out handle2);
			return PARTY_XBL_HANDLE.WrapAndReturnError(error, handle2, out handle);
		}
		...
		public unsafe static uint PartyXblStartProcessingStateChanges(PARTY_XBL_HANDLE handle, out List<PARTY_XBL_STATE_CHANGE> stateChanges)
		{
			...
			if (PartyError.SUCCEEDED(num) && stateChangeCount != 0)
			{
				...
				{
					PARTY_XBL_STATE_CHANGE pARTY_XBL_STATE_CHANGE = PARTY_XBL_STATE_CHANGE.CreateFromPtr(*(IntPtr*)((byte*)ptr + i * sizeof(IntPtr)));
