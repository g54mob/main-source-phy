...
namespace ModIO
{
	public static class Utility
	{
		...
		{
			string text = "^(http(s)?(://))?(www.)?";
			string text2 = "[a-zA-Z0-9-_.]+";
			Regex regex = new Regex(text + text2, RegexOptions.IgnoreCase);
			return regex.IsMatch(toCheck);
		}

		public static bool IsEmail(string toCheck)
		{
			string pattern = "^([a-z0-9\\+_\\-]+)(\\.[a-z0-9\\+_\\-]+)*@([a-z0-9\\-]+\\.)+[a-z]{2,63}$";
		...
		public static int[] MapProfileIds(IList<ModProfile> profiles)
		{
			...
			for (int i = 0; i < profiles.Count; i++)
			{
				...
				array[i] = ((modProfile != null) ? modProfile.id : 0);
			}
			return array;
		}
