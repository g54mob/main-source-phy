...
namespace ModIO.UI
{
	public class ModfileContainer : MonoBehaviour
	{
		...
		protected virtual void Start()
		{
			...
			m_itemTemplate = containerTemplate.GetComponentInChildren<ModfileView>(true);
			bool flag = parent.childCount > num && parent.GetChild(num).gameObject.name == text;
			if (flag)
			{
				m_templateClone = parent.GetChild(num).gameObject;
				ModfileView[] componentsInChildren = m_templateClone.GetComponentsInChildren<ModfileView>(true);
				...
				else
				{
					...
					foreach (ModfileView modfileView in array)
					{
						UnityEngine.Object.Destroy(modfileView.gameObject);
					}
				}
			}
			if (!flag)
			{
				...
				ModfileView componentInChildren = m_templateClone.GetComponentInChildren<ModfileView>(true);
