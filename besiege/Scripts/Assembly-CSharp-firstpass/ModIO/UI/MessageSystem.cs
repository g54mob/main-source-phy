...
namespace ModIO.UI
{
	public class MessageSystem : MonoBehaviour
	{
		...
		{
			if (!(instance == null) && (messageType != MessageDisplayData.Type.Error || !messageContent.StartsWith("The supplied access token has either been revoked")) && (messageType != MessageDisplayData.Type.Error || !messageContent.StartsWith("The supplied access token has either been revoked")))
			{
		...
		private IEnumerator DisplayNextMessageRoutine()
		{
			...
			if (dialog != null)
			{
				...
				if (anim != null)
				{
					for (float animTimer2 = 0f; animTimer2 < anim.duration; animTimer2 += Time.unscaledDeltaTime)
					{
						rectTransform.anchoredPosition = Vector2.Lerp(origin, origin + anim.offset, animTimer2 / anim.duration);
						yield return null;
					}
					rectTransform.anchoredPosition = origin;
				}
