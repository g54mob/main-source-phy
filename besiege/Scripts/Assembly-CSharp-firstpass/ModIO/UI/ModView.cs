...
namespace ModIO.UI
{
	...
	public class ModView : MonoBehaviour
	{
		...
		[SerializeField]
		private ModDisplayData m_data = default(ModDisplayData);
		...
		{
			IModViewElement[] componentsInChildren = base.gameObject.GetComponentsInChildren<IModViewElement>(true);
			IModViewElement[] array = componentsInChildren;
			foreach (IModViewElement modViewElement in array)
			{
				modViewElement.SetModView(this);
			}
		}
		...
		public void AttemptUnsubscribe()
		{
			...
			Action onClose = delegate
			{
				...
				foreach (ModSubscribedDisplay modSubscribedDisplay in componentsInChildren)
				{
					modSubscribedDisplay.DisplayModSubscribed(m_profile.id, isSubscribed);
				}
			};
