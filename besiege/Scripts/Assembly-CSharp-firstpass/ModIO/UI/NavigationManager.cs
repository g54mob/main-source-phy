...
namespace ModIO.UI
{
	public class NavigationManager : MonoBehaviour
	{
		...
		public static bool allowSelectionChange;
		...
		public static NavigationManager instance
		{
			get
			{
				if (_instance == null)
				{
					_instance = UIUtilities.FindComponentInAllScenes<NavigationManager>(true);
					if (_instance == null)
					{
						GameObject gameObject = new GameObject("Navigation Manager");
						_instance = gameObject.AddComponent<NavigationManager>();
					}
				}
				return _instance;
			}
		...
		private void LateUpdate()
		{
			...
			if (currentFocus == null || EventSystem.current == null)
			{
				return;
			}
			...
			else if (allowSelectionChange && !IsValidSelection(gameObject))
			{
		...
		public void OnFocusView(IBrowserView view)
		{
			...
			if (allowSelectionChange && gameObject != EventSystem.current.currentSelectedGameObject)
			{
				EventSystem.current.SetSelectedGameObject(gameObject);
			}
		}
		...
		private static bool IsValidSelection(GameObject selectionObject)
		{
			...
			return selectionObject.activeInHierarchy && component != null && component.interactable && component.IsActive();
		}
	}
}
