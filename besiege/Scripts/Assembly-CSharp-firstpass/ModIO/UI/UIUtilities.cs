...
namespace ModIO.UI
{
	public static class UIUtilities
	{
		...
		public static T FindComponentInAllScenes<T>(bool includeInactive) where T : Behaviour
		{
			...
			{
				T result = array[i];
				if (result.hideFlags != HideFlags.NotEditable && result.hideFlags != HideFlags.HideAndDontSave && (includeInactive || result.isActiveAndEnabled))
				{
					return result;
				}
			}
		...
		public static List<T> FindComponentsInAllScenes<T>(bool includeInactive) where T : Behaviour
		{
			...
			{
				T item = array[i];
				if (item.hideFlags != HideFlags.NotEditable && item.hideFlags != HideFlags.HideAndDontSave && (includeInactive || item.isActiveAndEnabled))
				{
					list.Add(item);
				}
			}
