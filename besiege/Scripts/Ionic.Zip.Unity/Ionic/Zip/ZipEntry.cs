...
namespace Ionic.Zip
{
	...
	public class ZipEntry
	{
		...
		private void SetFdpLoh()
		{
			...
			{
				throw new BadStateException(string.Format("Exception seeking  entry({0}) offset(0x{1:X8}) len(0x{2:X8})", FileName, _RelativeOffsetOfLocalHeader, ArchiveStream.Length), innerException);
			}
		...
		internal void VerifyCrcAfterExtract(int actualCrc32)
		{
			...
			if (UncompressedSize != 0L && (Encryption == EncryptionAlgorithm.WinZipAes128 || Encryption == EncryptionAlgorithm.WinZipAes256))
			{
