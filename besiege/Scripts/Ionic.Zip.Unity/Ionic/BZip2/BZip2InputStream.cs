	...
	public class BZip2InputStream : Stream
	{
		...
		private void InitBlock()
		{
			...
			if (c != '1' || c2 != 'A' || c3 != 'Y' || c4 != '&' || c5 != 'S' || c6 != 'Y')
			{
				...
				throw new IOException(string.Format("bad block header at offset 0x{0:X}", input.Position));
			}
		...
		private void EndBlock()
		{
			...
			{
				throw new IOException(string.Format("BZip2 CRC error (expected {0:X8}, computed {1:X8})", storedBlockCRC, computedBlockCRC));
			}
		...
		private void complete()
		{
			...
			{
				throw new IOException(string.Format("BZip2 CRC error (expected {0:X8}, computed {1:X8})", storedCombinedCRC, computedCombinedCRC));
			}
		}
		...
		private void SetupBlock()
		{
			...
			for (i = 0; i <= 256; i++)
			{
				...
				{
					throw new Exception(string.Format("BZ_DATA_ERROR: cftab[{0}]={1} last={2}", i, decompressionState.cftab[i], last));
				}
			}
			for (i = 1; i <= 256; i++)
			{
