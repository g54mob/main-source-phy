...
namespace RakNet
{
	public class TeamManager : PluginInterface2
	{
		...
		internal TeamManager(IntPtr cPtr, bool cMemoryOwn)
			: base(RakNetPINVOKE.TeamManager_SWIGUpcast(cPtr), cMemoryOwn)
		{
		...
		{
			IntPtr intPtr = RakNetPINVOKE.TeamManager_GetInstance();
			return (intPtr == IntPtr.Zero) ? null : new TeamManager(intPtr, false);
		}

		public static void DestroyInstance(TeamManager i)
		{
			RakNetPINVOKE.TeamManager_DestroyInstance(getCPtr(i));
		}

		public TeamManager()
			: this(RakNetPINVOKE.new_TeamManager(), true)
		{
		}

		public TM_World AddWorld(byte worldId)
		{
			IntPtr intPtr = RakNetPINVOKE.TeamManager_AddWorld(swigCPtr, worldId);
		...
		{
			return RakNetPINVOKE.TeamManager_GetWorldCount(swigCPtr);
		}

		public TM_World GetWorldAtIndex(uint index)
		{
			IntPtr intPtr = RakNetPINVOKE.TeamManager_GetWorldAtIndex(swigCPtr, index);
			return (intPtr == IntPtr.Zero) ? null : new TM_World(intPtr, false);
		}

		public TM_World GetWorldWithId(byte worldId)
		{
			IntPtr intPtr = RakNetPINVOKE.TeamManager_GetWorldWithId(swigCPtr, worldId);
			return (intPtr == IntPtr.Zero) ? null : new TM_World(intPtr, false);
		}
