...
namespace NATTraversal
{
	public class NATHelper : MonoBehaviour
	{
		private class PortMappingDoneInfo
		{
			...
			public event Action<Mapping, bool> onPortMappingDone;

			public void fireEvent()
			{
				if (this.onPortMappingDone != null)
				{
					this.onPortMappingDone(mapping, success);
				}
			}
		}
		...
		public string facilitatorIP = "grabblesgame.com";
		...
		public IEnumerator connectToNATFacilitator()
		{
			...
			StartupResult startupResult = rakPeer.Startup(2u, new SocketDescriptor(), 1u);
			if (startupResult != StartupResult.RAKNET_STARTED)
			{
				if (LogFilter.logDebug)
				{
					Debug.Log("NATHelper: Failed to initialize network interface: " + startupResult);
				}
				yield break;
			}
			ConnectionAttemptResult connectionAttemptResult = rakPeer.Connect(facilitatorIP, facilitatorPort, "", 0);
			if (connectionAttemptResult != ConnectionAttemptResult.CONNECTION_ATTEMPT_STARTED)
			{
				if (LogFilter.logDebug)
				{
					Debug.Log("NATHelper: Failed to initialize connection to NAT Facilitator: " + connectionAttemptResult);
				}
				yield break;
			}
		...
		public void mapPort(int privatePort, int publicPort = 0, int lifetime = 0, Protocol protocol = Protocol.Both, string desc = "", Action<Mapping, bool> onPortMappingDone = null)
		{
			if (!portForwardingEnabled && LogFilter.logDebug)
			{
		...
		public IEnumerator punchThroughToServer(ulong hostGUIDlong, Action<int, int, bool> onHolePunched)
		{
			...
			if (guid != 0L)
			{
				...
				RakNetGUID destination = new RakNetGUID(hostGUIDlong);
				...
				natPunchthroughClient.OpenNAT(destination, facilitatorAddress);
		...
		public void DisconnectFromFacilitator(uint blockDuration = 0u, bool sendDisconnectNotification = false)
		{
		...
		private void OnDoneConnectingToFacilitatorInternal(ulong guid, Packet packet)
		{
			...
			if (guid == 0L)
			{
				if (LogFilter.logWarn)
				{
					...
					else
					{
						...
						if (defaultMessageIDTypes == DefaultMessageIDTypes.ID_CONNECTION_ATTEMPT_FAILED)
						{
			...
			else
			{
				...
				{
					mapPort(rakPeer.GetInternalID().GetPort());
				}
				natPunchthroughClient = new NatPunchthroughClient();
				rakPeer.AttachPlugin(natPunchthroughClient);
		...
		private void mapPortInternal(int privatePort, int publicPort, int lifetime, Protocol protocol, string desc, Action<Mapping, bool> onPortMappingDone)
		{
			if (natDevice == null)
			{
				return;
			...
			if (protocol != Protocol.Tcp)
			{
				...
				mapping = new Mapping(Open.Nat.Protocol.Udp, privatePort, publicPort, lifetime, desc);
			...
			{
				Mapping mapping2 = new Mapping(Open.Nat.Protocol.Tcp, privatePort, publicPort, lifetime, desc);
				portMappings.Add(mapping2);
				numActiveMappings++;
				PortMappingDoneInfo info2 = new PortMappingDoneInfo();
				info2.onPortMappingDone += onPortMappingDone;
				info2.mapping = mapping2;
				portMappingDoneInfo.Add(info2);
				natDevice.CreatePortMapAsync(mapping2, portForwardingTimeOut).ContinueWith(delegate(Task t)
				{
					OnPortMappingDoneInternal(t, mapping2, info2);
				});
			}
		...
		private void OnPortMappingDoneInternal(Task task, Mapping mapping, PortMappingDoneInfo onPortMappingDoneInfo)
		{
			...
			onPortMappingDoneInfo.success = task.IsCompleted;
