...
namespace ch.sycoforge.Types
{
	...
	public struct IntRect
	{
		...
		{
			if (!(point.x < (float)xMin) && !(point.x >= (float)xMax) && !(point.y < (float)yMin))
			{
				return point.y < (float)yMax;
			}
			return false;
		}
		...
		public override bool Equals(object other)
		{
			...
			if (x.Equals(intRect.x) && y.Equals(intRect.y) && width.Equals(intRect.width))
			{
				return height.Equals(intRect.height);
			}
			return false;
		}
