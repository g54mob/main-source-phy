...
namespace ch.sycoforge.Unity.TextureUtil
{
	public static class TextureUtil
	{
		...
		public static Texture2D ScaleNineSliced(Texture2D sourceTexture, Rect targetRectangle, RectOffset sizingMargins, PixelMode mode)
		{
			...
			Rect rect2 = new Rect(x3, y, width3, height);
			Rect rect3 = new Rect(x2, y, width2, height);
			Rect rect4 = new Rect(x, y2, width, height2);
			Rect rect5 = new Rect(x3, y2, width3, height2);
			Rect rect6 = new Rect(x2, y2, width2, height2);
			Rect rect7 = new Rect(x, y3, width, height3);
			Rect rect8 = new Rect(x3, y3, width3, height3);
			Rect rect9 = new Rect(x2, y3, width2, height3);
			x = targetRectangle.x;
			...
			DrawToTexture(sourceTexture, texture2D, rect, to, mode);
			DrawToTexture(sourceTexture, texture2D, rect2, to2, mode);
			DrawToTexture(sourceTexture, texture2D, rect3, to3, mode);
			DrawToTexture(sourceTexture, texture2D, rect4, to4, mode);
			DrawToTexture(sourceTexture, texture2D, rect5, to5, mode);
			DrawToTexture(sourceTexture, texture2D, rect6, to6, mode);
			DrawToTexture(sourceTexture, texture2D, rect7, to7, mode);
			DrawToTexture(sourceTexture, texture2D, rect8, to8, mode);
			DrawToTexture(sourceTexture, texture2D, rect9, to9, mode);
			stopwatch.Stop();
			return texture2D;
		}
			...
			for (int i = 0; i < (int)to.width; i++)
			{
				...
				for (int j = 0; j < (int)to.height; j++)
				{
					...
					if (flag || flag2)
					{
						...
						{
							float num7 = (float)num2 / to.width;
							float num8 = (float)num4 / to.height;
							num5 = (flag ? ((int)Mathf.Clamp(num7 * from.width, from.x, from.width - 1f)) : num5);
							num6 = (flag2 ? ((int)Mathf.Clamp(num8 * from.height, from.y, from.height - 1f)) : num6);
