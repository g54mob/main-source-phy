...
public class NonConvexMeshCollider : MonoBehaviour
{
	public class BoundingBox
	{
		...
		{
			return string.Format("X: {0:N4}-{1:N4}, Y: {2:N4}-{3:N4}, Z: {4:N4}-{5:N4}", IntervalX.Min, IntervalX.Max, IntervalY.Min, IntervalY.Max, IntervalZ.Min, IntervalZ.Max);
		}
	}
	...
	public class SpatialBinaryTree
	{
		...
		public SpatialBinaryTree(Mesh m, int maxLevels)
		{
			...
			{
				Vector3 a = m.vertices[m.triangles[num2 * 3]];
				Vector3 b = m.vertices[m.triangles[num2 * 3 + 1]];
				Vector3 c = m.vertices[m.triangles[num2 * 3 + 2]];
				Tri t = new Tri(a, b, c);
				Add(t);
			}
		}
	...
	public class Interval
	{
		...
		public Interval LowerHalf
		{
			get
			{
				return new Interval(Min, Center);
			}
		}
		...
		public Interval UpperHalf
		{
			get
			{
				return new Interval(Center, Max);
			}
		}
	...
	public class Box
	{
		...
		private IEnumerable<Vector3Int> CoveredGridPositions
		{
			get
			{
				return LastLevelBoxes.Select((Box c) => c.lastLevelGridPos);
			}
		}
		...
		private int MinGridPosX
		{
			get
			{
				return (Children != null) ? CoveredGridPositions.Min((Vector3Int p) => p.X) : lastLevelGridPos.X;
			}
		}
		...
		private int MinGridPosY
		{
			get
			{
				return (Children != null) ? CoveredGridPositions.Min((Vector3Int p) => p.Y) : lastLevelGridPos.Y;
			}
		}
		...
		private int MinGridPosZ
		{
			get
			{
				return (Children != null) ? CoveredGridPositions.Min((Vector3Int p) => p.Z) : lastLevelGridPos.Z;
			}
		}
		...
		private int MaxGridPosX
		{
			get
			{
				return (Children != null) ? CoveredGridPositions.Max((Vector3Int p) => p.X) : lastLevelGridPos.X;
			}
		}
		...
		private int MaxGridPosY
		{
			get
			{
				return (Children != null) ? CoveredGridPositions.Max((Vector3Int p) => p.Y) : lastLevelGridPos.Y;
			}
		}
		...
		private int MaxGridPosZ
		{
			get
			{
				return (Children != null) ? CoveredGridPositions.Max((Vector3Int p) => p.Z) : lastLevelGridPos.Z;
			}
		}
		...
		private Vector3Int MinGridPos
		{
			get
			{
				return minGridPos ?? (minGridPos = new Vector3Int(MinGridPosX, MinGridPosY, MinGridPosZ));
			}
		}
		...
		private Vector3Int MaxGridPos
		{
			get
			{
				return maxGridPos ?? (maxGridPos = new Vector3Int(MaxGridPosX, MaxGridPosY, MaxGridPosZ));
			}
		}
	...
	{
		Transform transform = go.transform.FindChild("Colliders");
	...
	private IEnumerable<Box> CreateMeshIntersectingBoxes(GameObject colliderGo)
	{
		...
		for (int x = 0; x < boxesPerEdge; x++)
		{
			for (int y = 0; y < boxesPerEdge; y++)
			{
				...
				{
					Vector3 center = new Vector3(bounds.center.x - bounds.size.x / 2f + s.x * (float)x + halfExtent.x, bounds.center.y - bounds.size.y / 2f + s.y * (float)y + halfExtent.y, bounds.center.z - bounds.size.z / 2f + s.z * (float)z + halfExtent.z);
					if (!avoidExceedingMesh)
					{
						if (avoidGapsInside)
						{
							bool isInsideSurface = IsInsideMesh(center, tree, pointInsideMeshCache);
							boxColliderPositions[x, y, z] = isInsideSurface;
						}
						else
						{
							bool overlapsWithMeshSurface = Physics.OverlapBoxNonAlloc(center, halfExtent, colliders, Quaternion.identity, colliderLayerMask) > 0;
							boxColliderPositions[x, y, z] = overlapsWithMeshSurface;
						}
					}
					else
					{
						bool allCornersInsideMesh = directionsFromBoxCenterToCorners.Select((Vector3 d) => new Vector3(center.x + halfExtent.x * d.x, center.y + halfExtent.y * d.y, center.z + halfExtent.z * d.z)).All((Vector3 cornerPoint) => IsInsideMesh(cornerPoint, tree, pointInsideMeshCache));
		...
		for (int x2 = 0; x2 < boxesPerEdge; x2++)
		{
			for (int y2 = 0; y2 < boxesPerEdge; y2++)
			{
				for (int z2 = 0; z2 < boxesPerEdge; z2++)
				{
					if (boxColliderPositions[x2, y2, z2])
					{
						Vector3 center2 = new Vector3(bounds.center.x - bounds.size.x / 2f + s.x * (float)x2 + s.x / 2f, bounds.center.y - bounds.size.y / 2f + s.y * (float)y2 + s.y / 2f, bounds.center.z - bounds.size.z / 2f + s.z * (float)z2 + s.z / 2f);
						yield return boxes[x2, y2, z2] = new Box(boxes, center2, s, new Vector3Int(x2, y2, z2));
					}
				}
			}
		}
