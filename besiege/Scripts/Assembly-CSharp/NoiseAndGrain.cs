...
public class NoiseAndGrain : PostEffectsBase
{
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		if (dx11Grain && supportDX11)
		{
			...
			{
				RenderTexture temporary = RenderTexture.GetTemporary(Mathf.RoundToInt((float)source.width * (1f - softness)), Mathf.RoundToInt((float)source.height * (1f - softness)));
		...
		if (softness > Mathf.Epsilon)
		{
			RenderTexture temporary2 = RenderTexture.GetTemporary(Mathf.RoundToInt((float)source.width * (1f - softness)), Mathf.RoundToInt((float)source.height * (1f - softness)));
	...
	private static void DrawNoiseQuadGrid(RenderTexture source, RenderTexture dest, Material fxMaterial, Texture2D noise, int passNr)
	{
		...
		GL.Begin(7);
		for (float num7 = 0f; (double)num7 < 1.0; num7 += num4)
		{
			for (float num8 = 0f; (double)num8 < 1.0; num8 += num5)
			{
