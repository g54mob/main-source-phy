...
public static class CLZF2
{
	...
	private static byte[] compressBuffer = new byte[0];

	public static byte[] Compress(byte[] inputBytes)
	{
		int num = inputBytes.Length * 2;
		if (compressBuffer.Length < num)
		{
			compressBuffer = new byte[num];
		}
		int num2;
		for (num2 = lzf_compress(inputBytes, ref compressBuffer); num2 == 0; num2 = lzf_compress(inputBytes, ref compressBuffer))
		{
			num *= 2;
			if (compressBuffer.Length < num)
			{
		...
		Buffer.BlockCopy(compressBuffer, 0, array, 0, num2);
		return array;
	}
