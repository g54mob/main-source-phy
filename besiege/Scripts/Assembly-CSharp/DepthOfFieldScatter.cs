...
public class DepthOfFieldScatter : PostEffectsBase
{
	...
	private RenderTexture waterTex;

	public void Awake()
	{
		sourceCam = Camera.main;
		LayerMask layerMask = sourceCam.cullingMask;
		if ((int)layerMask == ((int)layerMask | 0x40))
		{
			layerMask = (int)layerMask & -65;
			sourceCam.cullingMask = layerMask;
		}
	}
	...
	private void Cleanup()
	{
		if ((bool)waterCam)
		{
			...
			SafeDestroy(waterCam.gameObject);
			waterCam = null;
		}
		if ((bool)waterTex)
		{
			waterTex.DiscardContents();
			RenderTexture.ReleaseTemporary(waterTex);
			waterTex = null;
			Shader.SetGlobalTexture("_WaterDepthTexture", waterTex);
		}
	}
	...
	private Camera CreateWaterCam()
	{
		GameObject gameObject = new GameObject("DOF Water Plane Camera", typeof(Camera));
		Camera component = gameObject.GetComponent<Camera>();
		component.depth = -9f;
		component.enabled = false;
		component.fieldOfView = sourceCam.fieldOfView;
		component.nearClipPlane = sourceCam.nearClipPlane;
		component.farClipPlane = sourceCam.farClipPlane;
		component.cullingMask = AddPiece.CreateLayerMask(default(LayerMask), 6);
	...
	private void OnPreCull()
	{
		if (WaterController.Exist)
		{
			...
			waterCam.fieldOfView = sourceCam.fieldOfView;
			waterCam.nearClipPlane = sourceCam.nearClipPlane;
			waterCam.farClipPlane = sourceCam.farClipPlane;
			waterCam.transform.position = sourceCam.transform.position;
			waterCam.transform.rotation = sourceCam.transform.rotation;
	...
	private void GetRenderTexture(int size)
	{
		if (waterTex == null)
		{
			Vector2 res = GetRes(waterCam, size);
			if (SystemInfo.graphicsShaderLevel > 30)
			{
				waterTex = RenderTexture.GetTemporary((int)res.x, (int)res.y, 0, RenderTextureFormat.RGB111110Float);
			}
			else
			{
				waterTex = RenderTexture.GetTemporary((int)res.x / 2, (int)res.y / 2, 0, RenderTextureFormat.Default);
			}
			waterTex.wrapMode = TextureWrapMode.Clamp;
			waterTex.filterMode = FilterMode.Trilinear;
			waterTex.anisoLevel = 16;
			waterTex.antiAliasing = 8;
		}
		waterCam.targetTexture = waterTex;
	}

	private Vector2 GetRes(Camera cam, float size)
	{
		int num = (int)(size * (float)cam.pixelWidth / (float)cam.pixelHeight);
		return new Vector2(num, size);
	}
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination)
	{
		...
		focalSize = Mathf.Clamp(focalSize, 0f, 4f);
