...
namespace Battlehub.Utils
{
	public abstract class AnimationInfo<TObj, TValue> : IAnimationInfo
	{
		...
		{
			return Mathf.Sin(MathF.PI * 26f * t) * Mathf.Pow(2f, 10f * (t - 1f));
		}

		public static float ElasticEaseOut(float t)
		{
			return Mathf.Sin(MathF.PI * -26f * (t + 1f)) * Mathf.Pow(2f, -10f * t) + 1f;
		}
		...
		public static float ElasticEaseInOut(float t)
		{
			if ((double)t < 0.5)
			{
				return 0.5f * Mathf.Sin(MathF.PI * 26f * (2f * t)) * Mathf.Pow(2f, 10f * (2f * t - 1f));
			}
			return 0.5f * (Mathf.Sin(MathF.PI * -26f * (2f * t - 1f + 1f)) * Mathf.Pow(2f, -10f * (2f * t - 1f)) + 2f);
		}
