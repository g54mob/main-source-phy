...
namespace Battlehub.RTCommon
{
	...
	public class DragDropTarget : MonoBehaviour, IPointerEnterHandler, IEventSystemHandler, IPointerExitHandler, IDragDropTarget
	{
		[SerializeField]
		...
		protected virtual void Awake()
		{
			...
			{
				IDragDropTarget[] dragDropTargets = new DragDropTarget[1] { this };
				m_dragDropTargets = dragDropTargets;
			}
			else
			{
		...
		void IPointerEnterHandler.OnPointerEnter(PointerEventData eventData)
		{
			...
			if (m_editor.DragDrop.InProgress && m_editor.DragDrop.Source != this)
			{
		...
		{
			if (this.DragEnterEvent != null)
			{
				this.DragEnterEvent(eventData);
			}
		}
		...
		{
			if (this.DragLeaveEvent != null)
			{
				this.DragLeaveEvent(eventData);
			}
		}
