...
namespace Battlehub.RTCommon
{
	...
	{
		[SerializeField]
		private bool m_clampToCameraPixelRect;
		...
		public virtual Ray Ray
		{
			get
			{
				...
				if (m_clampToCameraPixelRect && !pixelRect.Contains(screenPoint))
				{
					return new Ray(Vector3.up * float.MaxValue, Vector3.up);
				}
		...
		public virtual Vector2 ScreenPoint => ScreenPointToViewPoint(m_window.Editor.Input.GetPointerXY(0));
		...
		private Vector2 ScreenPointToViewPoint(Vector2 screenPoint)
		{
			...
			RectTransformUtility.ScreenPointToLocalPointInRectangle(m_renderTextureCamera.RectTransform, screenPoint, m_renderTextureCamera.Canvas.worldCamera, out var localPoint);
			if (m_canvasScaler != null)
			{
				...
				return localPoint * m_canvasScaler.scaleFactor;
			}
			return localPoint;
		}
