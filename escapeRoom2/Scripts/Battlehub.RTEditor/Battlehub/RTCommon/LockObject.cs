namespace Battlehub.RTCommon
{
	public class LockObject
	{
		private bool m_positionX;

		private bool m_positionY;

		private bool m_positionZ;

		private bool m_rotationX;

		private bool m_rotationY;

		private bool m_rotationZ;

		private bool m_rotationFree;

		private bool m_rotationScreen;

		private bool m_scaleX;

		private bool m_scaleY;

		private bool m_scaleZ;

		private bool m_rectXY;

		private bool m_rectYZ;

		private bool m_rectXZ;

		private LockObject m_globalLock;

		public bool PositionX
		{
			get
			{
				if (!m_positionX)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_positionX;
				}
				return true;
			}
			set
			{
				m_positionX = value;
			}
		}

		public bool PositionY
		{
			get
			{
				if (!m_positionY)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_positionY;
				}
				return true;
			}
			set
			{
				m_positionY = value;
			}
		}

		public bool PositionZ
		{
			get
			{
				if (!m_positionZ)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_positionZ;
				}
				return true;
			}
			set
			{
				m_positionZ = value;
			}
		}

		public bool RotationX
		{
			get
			{
				if (!m_rotationX)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_rotationX;
				}
				return true;
			}
			set
			{
				m_rotationX = value;
			}
		}

		public bool RotationY
		{
			get
			{
				if (!m_rotationY)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_rotationY;
				}
				return true;
			}
			set
			{
				m_rotationY = value;
			}
		}

		public bool RotationZ
		{
			get
			{
				if (!m_rotationZ)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_rotationZ;
				}
				return true;
			}
			set
			{
				m_rotationZ = value;
			}
		}

		public bool RotationFree
		{
			get
			{
				if (!m_rotationFree)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_rotationFree;
				}
				return true;
			}
			set
			{
				m_rotationFree = value;
			}
		}

		public bool RotationScreen
		{
			get
			{
				if (!m_rotationScreen)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_rotationScreen;
				}
				return true;
			}
			set
			{
				m_rotationScreen = value;
			}
		}

		public bool ScaleX
		{
			get
			{
				if (!m_scaleX)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_scaleX;
				}
				return true;
			}
			set
			{
				m_scaleX = value;
			}
		}

		public bool ScaleY
		{
			get
			{
				if (!m_scaleY)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_scaleY;
				}
				return true;
			}
			set
			{
				m_scaleY = value;
			}
		}

		public bool ScaleZ
		{
			get
			{
				if (!m_scaleZ)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_scaleZ;
				}
				return true;
			}
			set
			{
				m_scaleZ = value;
			}
		}

		public bool RectXY
		{
			get
			{
				if (!m_rectXY)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_rectXY;
				}
				return true;
			}
			set
			{
				m_rectXY = value;
			}
		}

		public bool RectYZ
		{
			get
			{
				if (!m_rectYZ)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_rectYZ;
				}
				return true;
			}
			set
			{
				m_rectYZ = value;
			}
		}

		public bool RectXZ
		{
			get
			{
				if (!m_rectXZ)
				{
					if (m_globalLock == null)
					{
						return false;
					}
					return m_globalLock.m_rectXZ;
				}
				return true;
			}
			set
			{
				m_rectXZ = value;
			}
		}

		public RuntimePivotMode? PivotMode { get; set; }

		public RuntimePivotRotation? PivotRotation { get; set; }

		public bool IsPositionLocked
		{
			get
			{
				if (PositionX && PositionY)
				{
					return PositionZ;
				}
				return false;
			}
		}

		public bool IsRotationLocked
		{
			get
			{
				if (RotationX && RotationY && RotationZ && RotationFree)
				{
					return RotationScreen;
				}
				return false;
			}
		}

		public bool IsScaleLocked
		{
			get
			{
				if (ScaleX && ScaleY)
				{
					return ScaleZ;
				}
				return false;
			}
		}

		public bool IsRectLocked
		{
			get
			{
				if (RectXY && RectYZ)
				{
					return RectXZ;
				}
				return false;
			}
		}

		public void SetGlobalLock(LockObject gLock)
		{
			m_globalLock = gLock;
		}

		public LockObject()
		{
		}

		public LockObject(LockObject obj)
		{
			m_positionX = obj.m_positionX;
			m_positionY = obj.m_positionY;
			m_positionZ = obj.m_positionZ;
			m_rotationX = obj.m_rotationX;
			m_rotationY = obj.m_rotationY;
			m_rotationZ = obj.m_rotationZ;
			m_rotationFree = obj.m_rotationFree;
			m_rotationScreen = obj.m_rotationScreen;
			m_scaleX = obj.m_scaleX;
			m_scaleY = obj.m_scaleY;
			m_scaleZ = obj.m_scaleZ;
			m_rectXY = obj.m_rectXY;
			m_rectYZ = obj.m_rectYZ;
			m_rectXZ = obj.m_rectXZ;
			m_globalLock = obj.m_globalLock;
			PivotMode = obj.PivotMode;
			PivotRotation = obj.PivotRotation;
		}

		public void Reset()
		{
			m_positionX = false;
			m_positionY = false;
			m_positionZ = false;
			m_rotationX = false;
			m_rotationY = false;
			m_rotationZ = false;
			m_rotationFree = false;
			m_rotationScreen = false;
			m_scaleX = false;
			m_scaleY = false;
			m_scaleZ = false;
			m_rectXY = false;
			m_rectYZ = false;
			m_rectXZ = false;
			m_globalLock = null;
			PivotMode = null;
			PivotRotation = null;
		}
	}
}
