...
public static class OVRPlugin
{
	...
	public static readonly Version wrapperVersion = OVRP_1_104_0.version;
	...
	public static bool IsWideMotionModeHandPosesEnabled()
	{
		...
		{
			Bool enabled = Bool.False;
			if (OVRP_1_93_0.ovrp_IsSetWideMotionModeHandPosesEnabled(ref enabled) == Result.Success)
			{
				return enabled == Bool.True;
			}
			return false;
	...
	public static bool RequestSceneCapture(string requestString, out ulong requestId)
	{
		...
		if (version >= OVRP_1_72_0.version)
		{
			...
			{
				requestByteCount = ((requestString != null) ? Encoding.ASCII.GetByteCount(requestString) : 0),
	...
	{
		ulong tracker;
		Result result = CreateDynamicObjectTracker(out tracker);
		if (!result.IsSuccess())
		{
			return OVRTask.FromResult(OVRResult<ulong, Result>.FromFailure(result));
		}
		return OVRTask.FromRequest<OVRResult<ulong, Result>>(tracker, EventType.CreateDynamicObjectTrackerResult);
	}
	...
	{
		Result result = SetDynamicObjectTrackedClasses(tracker, classes);
		if (!result.IsSuccess())
		{
			return OVRTask.FromResult(OVRResult<Result>.FromFailure(result));
		}
		return OVRTask.FromRequest<OVRResult<Result>>(tracker, EventType.SetDynamicObjectTrackedClassesResult);
	}
