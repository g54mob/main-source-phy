...
public class OVRSceneManager : MonoBehaviour
{
	...
	public bool RequestSceneCapture(IEnumerable<string> requestedAnchorClassifications)
	{
		...
		return RequestSceneCapture(string.Join(','.ToString(), requestedAnchorClassifications));
	}
	...
	{
		bool num = OVRPlugin.RequestSceneCapture(requestString, out _sceneCaptureRequestId);
		if (!num)
		{
