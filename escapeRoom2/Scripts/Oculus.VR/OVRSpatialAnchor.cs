...
public class OVRSpatialAnchor : MonoBehaviour
{
	...
	public unsafe static OVRTask<OperationResult> ShareAsync(IEnumerable<OVRSpatialAnchor> anchors, IEnumerable<OVRSpaceUser> users)
	{
		...
		ulong requestId;
		OVRPlugin.Result result = OVRPlugin.ShareSpaces(oVRNativeList, (uint)oVRNativeList.Count, oVRNativeList2, (uint)oVRNativeList2.Count, out requestId);
		return result.IsSuccess() ? OVRTask.FromRequest<OperationResult>(requestId) : OVRTask.FromResult((OperationResult)result);
	}
	...
	{
		if (!OVRAnchor.EraseSpace(_anchor.Handle, eraseOptions.Storage.ToSpaceStorageLocation(), out var requestId).IsSuccess())
		{
	...
	public unsafe static OVRTask<OperationResult> SaveAsync(IEnumerable<OVRSpatialAnchor> anchors, SaveOptions saveOptions)
	{
		...
		ulong requestId;
		OVRPlugin.Result result = OVRAnchor.SaveSpaceList(oVRNativeList, (uint)oVRNativeList.Count, saveOptions.Storage.ToSpaceStorageLocation(), out requestId);
		return result.IsSuccess() ? OVRTask.FromRequest<OperationResult>(requestId) : OVRTask.FromResult((OperationResult)result);
	}
