...
public class OVRColocationSession
{
	...
	public unsafe static OVRTask<OVRResult<Guid, Result>> StartAdvertisementAsync(ReadOnlySpan<byte> colocationSessionData)
	{
		...
		fixed (byte* groupMetadata = colocationSessionData)
		{
			...
			ulong requestId;
			OVRPlugin.Result result = OVRPlugin.StartColocationSessionAdvertisement(info, out requestId);
			if (result.IsSuccess())
			{
	...
	{
		ulong requestId;
		OVRPlugin.Result result = OVRPlugin.StopColocationSessionAdvertisement(out requestId);
		if (result.IsSuccess())
		{
	...
	{
		ulong requestId;
		OVRPlugin.Result result = OVRPlugin.StartColocationSessionDiscovery(out requestId);
		if (result.IsSuccess())
		{
			return OVRTask.FromRequest<OVRResult<Result>>(requestId);
		}
	...
	{
		ulong requestId;
		OVRPlugin.Result result = OVRPlugin.StopColocationSessionDiscovery(out requestId);
		if (result.IsSuccess())
		{
			return OVRTask.FromRequest<OVRResult<Result>>(requestId);
		}
