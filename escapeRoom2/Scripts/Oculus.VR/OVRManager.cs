...
public class OVRManager : MonoBehaviour, OVRMixedRealityCaptureConfiguration
{
	...
	public TrackingOrigin trackingOriginType
	{
		...
		set
		{
			...
			else if (OVRPlugin.UnityOpenXR.Enabled)
			{
				...
				TrackingOriginModeFlags origin = TrackingOriginModeFlags.Unknown;
				switch (value)
				{
				case TrackingOrigin.EyeLevel:
					origin = TrackingOriginModeFlags.Device;
					break;
				case TrackingOrigin.FloorLevel:
				case TrackingOrigin.Stage:
					origin = TrackingOriginModeFlags.Floor;
					break;
				default:
					Debug.LogError("Unable to map TrackingOrigin {0} in Unity OpenXR");
					break;
				}
				if (!GetCurrentInputSubsystem().TrySetTrackingOriginMode(origin))
				{
			...
			else if (OVRPlugin.SetTrackingOriginType((OVRPlugin.TrackingOrigin)value))
			{
				_trackingOriginType = value;
			}
		}
	...
	public static void SetSpaceWarp(bool enabled)
	{
		...
		{
			if (camera != null && m_lastSpaceWarpCamera.TryGetTarget(out var target) && target == camera)
			{
	...
	private void InitOVRManager()
	{
		...
		if (launchSimultaneousHandsControllersOnStartup && !OVRPlugin.SetSimultaneousHandsAndControllersEnabled(enabled: true))
		{
	...
	private void Update()
	{
		...
		if (useRecommendedMSAALevel && num != display.recommendedMSAALevel)
		{
	...
	private unsafe void UpdateHMDEvents()
	{
		while (OVRPlugin.PollEvent(ref eventDataBuffer))
		{
			...
			{
				OVRDeserialize.SpaceSetComponentStatusCompleteData eventData2 = OVRDeserialize.ByteArrayToStructure<OVRDeserialize.SpaceSetComponentStatusCompleteData>(eventDataBuffer.EventData);
				OVRManager.SpaceSetComponentStatusComplete?.Invoke(eventData2.RequestId, eventData2.Result >= 0, eventData2.Space, eventData2.Uuid, eventData2.ComponentType, eventData2.Enabled != 0);
				OVRTask.SetResult(eventData2.RequestId, eventData2.Result >= 0);
				OVRAnchor.OnSpaceSetComponentStatusComplete(eventData2);
				continue;
			}
			...
			{
				OVRDeserialize.SpaceEraseCompleteData eventData = OVRDeserialize.ByteArrayToStructure<OVRDeserialize.SpaceEraseCompleteData>(eventDataBuffer.EventData);
				bool flag = eventData.Result >= 0;
				OVRAnchor.OnSpaceEraseComplete(eventData);
				OVRManager.SpaceEraseComplete?.Invoke(eventData.RequestId, flag, eventData.Uuid, eventData.Location);
				OVRTask.SetResult(eventData.RequestId, flag);
				continue;
			}
			...
			{
				OVRDeserialize.SpaceListSaveResultData eventData5 = OVRDeserialize.ByteArrayToStructure<OVRDeserialize.SpaceListSaveResultData>(eventDataBuffer.EventData);
				OVRAnchor.OnSpaceListSaveResult(eventData5);
				OVRManager.SpaceListSaveComplete?.Invoke(eventData5.RequestId, (OVRSpatialAnchor.OperationResult)eventData5.Result);
				continue;
			}
			...
			{
				OVRDeserialize.SpacesSaveResultData eventData4 = OVRDeserialize.ByteArrayToStructure<OVRDeserialize.SpacesSaveResultData>(eventDataBuffer.EventData);
				OVRAnchor.OnSaveSpacesResult(eventData4);
				OVRTask.SetResult(eventData4.RequestId, OVRResult.From(eventData4.Result));
				continue;
			}
			case OVRPlugin.EventType.SpacesEraseResult:
			{
				OVRDeserialize.SpacesEraseResultData eventData3 = OVRDeserialize.ByteArrayToStructure<OVRDeserialize.SpacesEraseResultData>(eventDataBuffer.EventData);
				OVRAnchor.OnEraseSpacesResult(eventData3);
				OVRTask.SetResult(eventData3.RequestId, OVRResult.From(eventData3.Result));
				continue;
			}
	...
	private void LateUpdate()
	{
		...
		if (camera != null)
		{
			m_lastSpaceWarpCamera.TryGetTarget(out var target);
			if (camera != target)
			{
