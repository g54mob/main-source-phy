...
namespace Battlehub.RTHandles.Demo
{
	...
	{
		[SerializeField]
		private MobileSceneInput m_mobileInput;
		...
		private void Awake()
		{
			UnityEventHelper.AddListener(m_sensitivity, (Slider slider) => slider.onValueChanged, OnSensitivityChanged);
		...
		{
			UnityEventHelper.RemoveListener(m_sensitivity, (Slider slider) => slider.onValueChanged, OnSensitivityChanged);
		}
