...
namespace Meta.XR.EnvironmentDepth
{
	internal static class EnvironmentDepthUtils
	{
		...
		{
			float fovLeftAngle = frameDesc.fovLeftAngle;
			float fovRightAngle = frameDesc.fovRightAngle;
			float fovDownAngle = frameDesc.fovDownAngle;
			float fovTopAngle = frameDesc.fovTopAngle;
			float nearZ = frameDesc.nearZ;
			float farZ = frameDesc.farZ;
			float m = 2f / (fovRightAngle + fovLeftAngle);
			float m2 = 2f / (fovTopAngle + fovDownAngle);
			float m3 = (fovRightAngle - fovLeftAngle) / (fovRightAngle + fovLeftAngle);
			float m4 = (fovTopAngle - fovDownAngle) / (fovTopAngle + fovDownAngle);
			float m5;
			float m6;
			...
			Vector4 createPoseRotation = frameDesc.createPoseRotation;
			viewMatrix = Matrix4x4.TRS(q: new Quaternion(createPoseRotation.x, createPoseRotation.y, createPoseRotation.z, createPoseRotation.w), pos: frameDesc.createPoseLocation, s: _scalingVector3).inverse;
		}
	}
}
