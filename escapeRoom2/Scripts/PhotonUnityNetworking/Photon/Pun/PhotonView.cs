...
namespace Photon.Pun
{
	...
	public class PhotonView : MonoBehaviour
	{
		...
		internal void RebuildControllerCache(bool ownerHasChanged = false)
		{
			...
			{
				ControllerActorNr = PhotonNetwork.MasterClient?.ActorNumber ?? (-1);
			}
			else
			{
		...
		public void RequestOwnership()
		{
			...
			{
				Debug.LogWarning("Attempting to RequestOwnership of GameObject '" + base.name + "' viewId: " + ViewID + ", but PhotonView.OwnershipTransfer is set to Fixed.");
			}
		}
		...
		public void TransferOwnership(Player newOwner)
		{
			...
			{
				Debug.LogWarning("Attempting to TransferOwnership of GameObject '" + base.name + "' viewId: " + ViewID + ", but provided Player newOwner is null.");
			}
		}
		...
		public void TransferOwnership(int newOwnerId)
		{
			...
			else if (PhotonNetwork.LogLevel >= PunLogLevel.Informational)
			{
				if (OwnershipTransfer == OwnershipOption.Fixed)
				{
					Debug.LogWarning("Attempting to TransferOwnership of GameObject '" + base.name + "' viewId: " + ViewID + " without the authority to do so. TransferOwnership is not allowed if PhotonView.OwnershipTransfer is set to Fixed.");
				}
				else if (OwnershipTransfer == OwnershipOption.Request)
				{
					Debug.LogWarning("Attempting to TransferOwnership of GameObject '" + base.name + "' viewId: " + ViewID + " without the authority to do so. PhotonView.OwnershipTransfer is set to Request, so only the controller of this object can TransferOwnership.");
				}
			}
		}
