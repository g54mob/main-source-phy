	...
	internal class WatchManager : IDebugManager
	{
		...
		public void ProcessType(Type type)
		{
			...
			foreach (MemberInfo memberInfo in members)
			{
				...
				if (customAttribute != null && IsWatchTypeSupported(memberInfo))
				{
					list.Add((memberInfo, customAttribute));
				}
			}
			list.AddRange(InspectedDataRegistry.GetMembersForType(type, (MemberInfo info, DebugMember _) => IsWatchTypeSupported(info)));
			WatchesDict[type] = list;
			ManagerUtils.RebuildInspectorForType(_uiPanel, _instanceCache, type, list, delegate(IMember memberController, MemberInfo member, DebugMember attribute, UnityEngine.Object instance)
			{
		...
		internal static bool IsWatchTypeSupported(MemberInfo member)
		{
			...
			return ((memberType == MemberTypes.Property || memberType == MemberTypes.Field) & !member.IsBaseTypeEqual(typeof(Enum))) | member.IsTypeEqual(typeof(Texture2D));
		}
