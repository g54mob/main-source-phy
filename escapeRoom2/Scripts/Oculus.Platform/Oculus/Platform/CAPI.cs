...
namespace Oculus.Platform
{
	public class CAPI
	{
		...
		{
			IntPtr intPtr = StringToNative(video_title);
			IntPtr intPtr2 = StringToNative(presenter_data);
			ulong result = ovr_Cowatching_SetPresenterData_Native(intPtr, intPtr2);
			Marshal.FreeCoTaskMem(intPtr);
			Marshal.FreeCoTaskMem(intPtr2);
			return result;
		}
		...
		{
			IntPtr intPtr = StringToNative(viewer_data);
			ulong result = ovr_Cowatching_SetViewerData_Native(intPtr);
			Marshal.FreeCoTaskMem(intPtr);
			return result;
		}
		...
		{
			return StringFromNative(ovr_CowatchViewer_GetData_Native(obj));
		}
		...
		{
			return StringFromNative(ovr_CowatchViewerArray_GetNextUrl_Native(obj));
		}
