		...
		public virtual void OnStatusChanged(StatusCode statusCode)
		{
			...
			case StatusCode.Disconnect:
			{
				...
				case ClientState.ConnectWithoutAuthOnceWss:
					DebugReturn(DebugLevel.INFO, string.Format("AuthOnceWss failed (WSS connection could not be established: " + DisconnectedCause.ToString() + "). Trying again with protocol: " + LoadBalancingPeer.TransportProtocol));
				...
				case ClientState.ConnectWithFallbackProtocol:
					...
					LoadBalancingPeer.TransportProtocol = ((LoadBalancingPeer.TransportProtocol == ConnectionProtocol.Udp) ? ConnectionProtocol.WebSocketSecure : ConnectionProtocol.Udp);
					NameServerPortInAppSettings = 0;
					ServerPortOverrides = default(PhotonPortDefinition);
					if (LoadBalancingPeer.Connect(NameServerAddress, ProxyServerAddress, AppId, TokenForInit))
					{
			...
			case StatusCode.EncryptionFailedToEstablish:
			{
				...
				if (State == ClientState.ConnectingToNameServer)
				{
					if (AuthMode == AuthModeOption.AuthOnceWss && ExpectedProtocol != ConnectionProtocol.WebSocketSecure)
					{
						clientState = ClientState.ConnectWithoutAuthOnceWss;
					}
					else if (EnableProtocolFallback)
					{
