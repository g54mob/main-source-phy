...
namespace NAudio.Midi
{
	public class ControlChangeEvent : MidiEvent
	{
		...
		public ControlChangeEvent(BinaryReader br)
		{
			...
			if ((b & 0x80) != 0)
			{
				throw new NAudio.Utils.InvalidDataException("Invalid controller");
			}
			controller = (MidiController)b;
			if ((controllerValue & 0x80) != 0)
			{
				throw new NAudio.Utils.InvalidDataException($"Invalid controllerValue {controllerValue} for controller {controller}, Pos 0x{br.BaseStream.Position:X}");
			}
		}
