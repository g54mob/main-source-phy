		...
		public void ReadWaveHeader(Stream stream)
		{
			...
			while (stream.Position <= num3 - 8)
			{
				...
				if (num4 == num2)
				{
					...
					{
						throw new NAudio.Utils.InvalidDataException($"Format chunk length must be between 0 and {int.MaxValue}.");
					}
				...
				if (storeAllChunks)
				{
					if (num5 > int.MaxValue)
					{
						throw new NAudio.Utils.InvalidDataException($"RiffChunk chunk length must be between 0 and {int.MaxValue}.");
					}
