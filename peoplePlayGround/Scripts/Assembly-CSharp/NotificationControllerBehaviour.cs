...
public class NotificationControllerBehaviour : MonoBehaviour
{
	...
	private IEnumerator CreateNotification(string text)
	{
		...
		notificationCount++;
		GameObject n = Object.Instantiate(NotificationPrefab, Vector3.zero, Quaternion.identity, base.transform);
		n.GetComponentInChildren<TextMeshProUGUI>().text = text;
		notificationObjects.Add(n);
		yield return new WaitForSecondsRealtime(NotificationLifetime);
		if ((bool)n)
		{
			DestroyNotification(n);
		}
	}
