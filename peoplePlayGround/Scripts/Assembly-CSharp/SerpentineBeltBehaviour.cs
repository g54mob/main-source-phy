...
public class SerpentineBeltBehaviour : Hover
{
	...
	private void Awake()
	{
		...
		Color endColor = (LineRenderer.startColor = SerpentineBeltTool.WireColor);
		lineRenderer.endColor = endColor;
		LineRenderer lineRenderer2 = LineRenderer;
		float endWidth = (LineRenderer.startWidth = SerpentineBeltTool.WireWidth);
		lineRenderer2.endWidth = endWidth;
	}
	...
	protected override bool IsMouseInsideCollider()
	{
		...
		{
			Vector3 vector = linePositionBuffer[i];
			Vector3 vector2 = linePositionBuffer[(i + 1) % linePositionBuffer.Length];
			if (Mathf.Abs(Utils.SqrdDistanceFromPointToLineSegment(mousePosition, vector, vector2)) <= num * num)
			{
				return true;
			}
		}
	...
	public override void OnMouseOverlapEvent(bool overlap)
	{
		...
		{
			LineRenderer lineRenderer = LineRenderer;
			Color startColor = (LineRenderer.endColor = Color.red);
		...
		{
			LineRenderer lineRenderer2 = LineRenderer;
			Color startColor = (LineRenderer.endColor = SerpentineBeltTool.WireColor);
