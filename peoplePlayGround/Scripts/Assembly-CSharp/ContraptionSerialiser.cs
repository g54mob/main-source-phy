...
public class ContraptionSerialiser
{
	...
	public static void SaveContraption(string name, ObjectState[] states)
	{
		...
		{
			if (bloodContainerType.IsAssignableFrom(comp.Type) && comp.storedFields.TryGetValue("SerialisableDistributions", out var value) && value is BloodContainer.SerialisableDistribution[] array2)
			{
	...
	{
		if (metadata.Version != "1.27.11")
		{
			NotificationControllerBehaviour.Show("Game version mismatch. This might cause problems.");
			Debug.LogWarning("Attempt to load save from " + metadata.Version + " into 1.27.11");
		}
