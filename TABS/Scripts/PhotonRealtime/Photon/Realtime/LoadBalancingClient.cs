...
namespace Photon.Realtime
{
	public class LoadBalancingClient : IPhotonPeerListener
	{
		...
		private bool Connect(string serverAddress, string proxyServerAddress, ServerConnection serverType)
		{
			...
			{
				DebugReturn(DebugLevel.ERROR, string.Concat("Connect() failed. Can't connect to ", serverType, " with Token == null in AuthMode: ", AuthMode));
				return false;
			}
		...
		public void Disconnect(DisconnectCause cause = DisconnectCause.DisconnectByClientLogic)
		{
			...
			{
				DebugReturn(DebugLevel.INFO, string.Concat("Disconnect() call gets skipped due to State ", State, ". DisconnectedCause: ", DisconnectedCause, " Parameter cause: ", cause));
			}
		...
		private bool CallAuthenticate()
		{
			...
			{
				DebugReturn(DebugLevel.ERROR, string.Concat("Authenticate without Token is only allowed on Name Server. Connecting to: ", Server, " on: ", CurrentServerAddress, ". State: ", State));
				return false;
			}
			if (AuthMode == AuthModeOption.Auth)
			{
