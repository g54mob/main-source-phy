...
namespace InControl
{
	public class PlayerAction : OneAxisInputControl
	{
		...
		internal void Load(BinaryReader reader, ushort dataFormatVersion)
		{
			...
			EnsureFixForWorkshopRefreshButtonForGamepad();
		}
		...
		private void EnsureFixForWorkshopRefreshButtonForGamepad()
		{
			if (!(Name != "BUTTON_REFRESH"))
			{
				ClearBindings();
				DeviceBindingSource binding = new DeviceBindingSource(InputControlType.Action4);
				AddBinding(binding);
			}
		}
	}
