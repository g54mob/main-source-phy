		...
		public UnityInputDevice(InputDeviceProfile deviceProfile, int joystickId, string joystickName)
		{
			...
			if (IsKnown)
			{
				...
				for (int i = 0; i < analogCount; i++)
				{
					...
					{
						Logger.LogError(string.Concat("Cannot map control \"", inputControlMapping.Name, "\" as InputControlType.", inputControlMapping.Target, " in profile \"", deviceProfile.DeviceName, "\" because this target is reserved as an alias. The mapping will be ignored."));
					}
					else
					{
				...
				for (int j = 0; j < buttonCount; j++)
				{
					...
					{
						Logger.LogError(string.Concat("Cannot map control \"", inputControlMapping2.Name, "\" as InputControlType.", inputControlMapping2.Target, " in profile \"", deviceProfile.DeviceName, "\" because this target is reserved as an alias. The mapping will be ignored."));
					}
					else
					{
