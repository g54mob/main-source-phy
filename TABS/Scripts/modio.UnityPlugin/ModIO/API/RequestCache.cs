...
namespace ModIO.API
{
	public static class RequestCache
	{
		...
		public static bool TryGetResponse(string url, out string response)
		{
			...
			bool result = false;
			if (LocalUser.OAuthToken == lastOAuthToken && TryGetEntry(endpointURL, out var index, out var entry))
			{
		...
		public static void StoreResponse(string url, string responseBody)
		{
			...
			if (TryGetEntry(endpointURL, out var index, out var _))
			{
		...
		public static bool TryGetMod(int gameId, int modId, out ModProfile profile)
		{
			...
			if (LocalUser.OAuthToken == lastOAuthToken && TryGetEntry(endpointURL, out var index, out var entry))
			{
				...
				else
				{
					...
					try
					{
						profile = JsonConvert.DeserializeObject<ModProfile>(responseBody, new JsonSerializerSettings
						{
		...
		private static void RemoveOldestEntries(int count)
		{
			...
			foreach (string item in new List<string>(urlResponseIndexMap.Keys))
			{
