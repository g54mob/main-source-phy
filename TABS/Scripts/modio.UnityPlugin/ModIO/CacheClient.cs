...
namespace ModIO
{
	...
	{
		public static string gameProfileFilePath => IOUtilities.CombinePath(DataStorage.CACHE_DIRECTORY, "game_profile.data");

		[Obsolete("Use DataStorage.CACHE_DIRECTORY instead")]
		public static string cacheDirectory => DataStorage.CACHE_DIRECTORY;
		...
		{
			DataStorage.WriteJSONFile(GenerateModProfileFilePath(profile.id), profile, delegate(string p, bool success)
			{
				if (onComplete != null)
				{
					onComplete(success);
		...
		{
			DataStorage.ReadJSONFile(GenerateModProfileFilePath(modId), delegate(string p, bool success, ModProfile data)
			{
				if (onComplete != null)
				{
					onComplete(data);
		...
		public static async void RequestAllModProfilesFromOffset(int offset, Action<IList<ModProfile>> onComplete)
		{
			...
			DataStorage.GetDirectories(profileDirectory, delegate(string gd_path, bool gd_success, IList<string> modDirectories)
			{
				...
				{
					Debug.LogWarning("[mod.io] Failed to read mod profile directory.\nDirectory: " + profileDirectory);
					modDirectories = new string[0];
				}
				...
				loadNextProfile = delegate
				{
					if (profilePaths.Count > 0)
					{
						...
						DataStorage.ReadJSONFile(path, async delegate(string p, bool success, ModProfile data)
						{
							if (success)
							{
								modProfiles.Add(data);
								if (ShouldPauseBetweenNestedFileLoading())
								{
									await Task.Yield();
								}
								loadNextProfile();
							...
							{
								DataStorage.DeleteFile(path, async delegate
								{
									if (ShouldPauseBetweenNestedFileLoading())
									{
										await Task.Yield();
		...
		{
			DataStorage.DeleteDirectory(GenerateModDirectoryPath(modId), delegate(string path, bool success)
			{
				if (onComplete != null)
				{
					onComplete(success);
		...
		{
			DataStorage.WriteJSONFile(GenerateModStatisticsFilePath(stats.modId), stats, delegate(string p, bool success)
			{
				if (onComplete != null)
				{
					onComplete(success);
		...
		{
			DataStorage.ReadJSONFile(GenerateModStatisticsFilePath(modId), delegate(string p, bool success, ModStatistics data)
			{
				if (onComplete != null)
				{
					onComplete(data);
		...
		public static async void RequestFilteredModStatistics(IList<int> idFilter, Action<IList<ModStatistics>> onComplete)
		{
			...
			DataStorage.GetDirectories(statisticsDirectory, delegate(string gd_path, bool gd_success, IList<string> modDirectories)
			{
				...
				{
					Debug.LogWarning("[mod.io] Failed to read mod statistics directory.\nDirectory: " + statisticsDirectory);
					modDirectories = new string[0];
				}
				...
				loadNextStatistics = delegate
				{
					if (statsPaths.Count > 0)
					{
						...
						DataStorage.ReadJSONFile(path, async delegate(string p, bool success, ModStatistics data)
						{
							if (success)
							{
		...
		{
			DataStorage.WriteJSONFile(GenerateModfileFilePath(modfile.modId, modfile.id), modfile, delegate(string p, bool success)
			{
				if (onComplete != null)
				{
					onComplete(success);
		...
		{
			DataStorage.ReadJSONFile(GenerateModfileFilePath(modId, modfileId), delegate(string p, bool success, Modfile data)
			{
				if (onComplete != null)
				{
					onComplete(data);
		...
		{
			DataStorage.WriteFile(GenerateModBinaryZipFilePath(modId, modfileId), modBinary, delegate(string p, bool success)
			{
				if (onComplete != null)
				{
					onComplete(success);
		...
		{
			DataStorage.ReadFile(GenerateModBinaryZipFilePath(modId, modfileId), delegate(string p, bool s, byte[] data)
			{
				if (onComplete != null)
				{
					onComplete(data);
		...
		{
			DataStorage.DeleteDirectory(GenerateModBinariesDirectoryPath(modId), delegate(string p, bool success)
			{
				if (onComplete != null)
				{
					onComplete(success);
		...
		{
			DataStorage.ReadJSONFile(GenerateModLogoVersionInfoFilePath(modId), delegate(string p, bool success, Dictionary<LogoSize, string> data)
			{
				if (onComplete != null)
				{
					onComplete(data);
		...
		public static void SaveModLogo(int modId, string fileName, LogoSize size, Texture2D logoTexture, Action<bool> onComplete)
		{
			...
			DataStorage.WriteFile(path, data, delegate(string p, bool success)
			{
				GetModLogoVersionFileNames(modId, delegate(Dictionary<LogoSize, string> versionInfo)
				{
					...
					DataStorage.WriteJSONFile(GenerateModLogoVersionInfoFilePath(modId), versionInfo, null);
				});
				if (onComplete != null)
				{
					onComplete(success);
		...
		{
			DataStorage.ReadFile(GenerateModLogoFilePath(modId, size), delegate(string p, bool success, byte[] data)
			{
				Texture2D obj = null;
		...
		{
			DataStorage.ReadFile(GenerateModGalleryImageFilePath(modId, imageFileName, size), delegate(string p, bool success, byte[] data)
			{
				Texture2D obj = null;
				if (success && data != null)
				{
		...
		{
			DataStorage.ReadFile(GenerateModYouTubeThumbnailFilePath(modId, youTubeId), delegate(string p, bool success, byte[] data)
			{
				Texture2D obj = null;
				if (success && data != null)
				{
		...
		{
			DataStorage.WriteJSONFile(GenerateModTeamFilePath(modId), modTeam, delegate(string p, bool success)
			{
				if (onComplete != null)
				{
					onComplete(success);
		...
		{
			DataStorage.ReadJSONFile(GenerateModTeamFilePath(modId), delegate(string p, bool success, List<ModTeamMember> data)
			{
				if (onComplete != null)
				{
					onComplete(data);
		...
		{
			DataStorage.DeleteFile(GenerateModTeamFilePath(modId), delegate(string p, bool success)
			{
				if (onComplete != null)
				{
					onComplete(success);
		...
		{
			DataStorage.ReadFile(GenerateUserAvatarFilePath(userId, size), delegate(string p, bool success, byte[] data)
			{
				Texture2D obj = null;
				if (success && data != null)
				{
		...
		{
			DataStorage.DeleteDirectory(GenerateUserAvatarDirectoryPath(userId), delegate(string p, bool success)
			{
				if (onComplete != null)
				{
					onComplete(success);
		...
		{
			bool result = false;
			DataStorage.WriteJSONFile(GenerateUserProfileFilePath(userProfile.id), userProfile, delegate(string p, bool s)
			{
				result = s;
			});
			return result;
		...
		{
			bool result = false;
			DataStorage.DeleteFile(GenerateUserProfileFilePath(userId), delegate(string p, bool s)
			{
				result = s;
			});
			return result;
		...
		private static bool ShouldPauseBetweenNestedFileLoading()
		{
			return false;
		}
	}
