...
namespace ModIO
{
	public static class DataStorage
	{
		...
		public static string INSTALLATION_DIRECTORY => PLATFORM_IO.InstallationDirectory;

		public static string CACHE_DIRECTORY => PLATFORM_IO.CacheDirectory;

		static DataStorage()
		{
			if (InjectedIO.platformIO != null)
			{
				Debug.LogFormat("DM: Using injected platformIO");
				PLATFORM_IO = InjectedIO.platformIO;
			}
			else
			{
				Debug.LogFormat("DM: Not using injected platformIO");
		...
		public static void WriteJSONFile<T>(string path, T jsonObject, WriteFileCallback onComplete)
		{
			...
			onComplete?.Invoke(path, success: false);
		}
