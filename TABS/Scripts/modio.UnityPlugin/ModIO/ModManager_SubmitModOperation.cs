	...
	internal class ModManager_SubmitModOperation
	{
		...
		public void SubmitNewMod(EditableModProfile newModProfile)
		{
			...
			if (text == null)
			{
				...
				if (newModProfile.youTubeURLs.isDirty || newModProfile.sketchfabURLs.isDirty || newModProfile.galleryImageLocators.isDirty)
				{
		...
		private void SubmitModChanges_Internal(ModProfile profile)
		{
			...
			if (eModProfile.galleryImageLocators.isDirty)
			{
				...
				{
					ImageLocatorData imageLocatorData = value[i];
					removedImageFileNames.Remove(imageLocatorData.fileName);
				}
				addedImageFilePaths = new List<string>();
				value = eModProfile.galleryImageLocators.value;
				for (int i = 0; i < value.Length; i++)
				{
					ImageLocatorData imageLocatorData2 = value[i];
					addedImageFilePaths.Add(imageLocatorData2.url);
				}
				galleryImageLocators = profile.media.galleryImageLocators;
				foreach (GalleryImageLocator galleryImageLocator2 in galleryImageLocators)
				{
			...
			if (eModProfile.sketchfabURLs.isDirty)
			{
				...
				string[] value2 = eModProfile.sketchfabURLs.value;
				...
				value2 = profile.media.sketchfabURLs;
			...
			if (eModProfile.youTubeURLs.isDirty)
			{
				...
				string[] value2 = eModProfile.youTubeURLs.value;
				...
				value2 = profile.media.youTubeURLs;
			...
			if (eModProfile.tags.isDirty)
			{
				...
				string[] value2 = eModProfile.tags.value;
			...
			if (eModProfile.metadataKVPs.isDirty)
			{
				...
				MetadataKVP[] value3 = eModProfile.metadataKVPs.value;
				foreach (MetadataKVP metadataKVP in value3)
				{
					if (removedKVPs.TryGetValue(metadataKVP.key, out var value4) && value4 == metadataKVP.value)
					{
						removedKVPs.Remove(metadataKVP.key);
					}
				}
				addedKVPs = MetadataKVP.ArrayToDictionary(eModProfile.metadataKVPs.value);
				value3 = profile.metadataKVPs;
				foreach (MetadataKVP metadataKVP2 in value3)
				{
					if (addedKVPs.TryGetValue(metadataKVP2.key, out var value5) && value5 == metadataKVP2.value)
					{
				...
				DataStorage.CreateDirectory(Path.GetDirectoryName(imageArchivePath), delegate(string path, bool success)
				{
					if (success)
					{
						try
						{
							...
							{
								foreach (string addedImageFilePath in addedImageFilePaths)
								{
									zipFile.AddFile(addedImageFilePath);
								}
								zipFile.Save(imageArchivePath);
							}
