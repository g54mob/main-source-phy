...
namespace ModIO
{
	public static class DataUpdater
	{
		...
		private static void Update_2_0_to_2_1_UserData()
		{
			...
			if (IOUtilities.TryReadJsonObjectFile<GenericJSONObject>(text, out var jsonObject))
			{
				int[] fieldData = null;
		...
		{
			fieldData = default(T);
			if (jsonObject.data.ContainsKey(fieldName) && jsonObject.data[fieldName] is JArray jArray)
			{
