...
namespace ModIO
{
	public static class DebugUtilities
	{
			...
			if (uploadHandler != null)
			{
				...
				if (binaryFields != null)
				{
					...
					foreach (BinaryDataParameter item2 in binaryFields)
					{
						...
						stringBuilder.Append(" (");
						stringBuilder.Append((item2.contents == null) ? "NULL_DATA" : ValueFormatting.ByteCount(item2.contents.Length, null));
						stringBuilder.Append(")");
						stringBuilder.AppendLine();
						num2++;
					}
		...
		public static void ParseURLEncodedFormData(byte[] data, out List<StringValueParameter> stringFields)
		{
			...
			string[] array = Encoding.UTF8.GetString(data).Split(new char[1] { '&' }, StringSplitOptions.RemoveEmptyEntries);
			foreach (string text in array)
			{
				...
				if (array2.Length != 0)
				{
					if (array2.Length != 2)
					{
						stringFields.Add(StringValueParameter.Create(text, "BADLY_FORMATTED_FORMDATA"));
					}
					else
					{
						stringFields.Add(StringValueParameter.Create(array2[0], array2[1]));
					}
				}
			}
		}
		...
		public static void ParseMultipartFormData(byte[] data, out List<StringValueParameter> stringFields, out List<BinaryDataParameter> binaryFields)
		{
			...
			foreach (string text4 in array2)
			{
				...
				if (num2 < 0)
				{
					binaryDataParameter.key = "KEY_NOT_FOUND";
				}
				else
				{
					num3 = num2 + text5.Length;
					num4 = text4.IndexOf("\"", num3);
					binaryDataParameter.key = text4.Substring(num3, num4 - num3);
				}
				text5 = "filename=\"";
				num2 = text4.IndexOf(text5);
				if (num2 < 0)
				{
					binaryDataParameter.fileName = "FILENAME_NOT_FOUND";
				}
				else
				{
					num3 = num2 + text5.Length;
					num4 = text4.IndexOf("\"", num3);
					binaryDataParameter.fileName = text4.Substring(num3, num4 - num3);
				}
				text5 = text2 + text2;
				num2 = text4.IndexOf(text5);
				if (num2 < 0)
				{
					binaryDataParameter.contents = null;
				}
				else
				{
					num3 = num2 + text5.Length;
					int length = text4.Length - num3 - text2.Length;
					binaryDataParameter.contents = Encoding.UTF8.GetBytes(text4.Substring(num3, length));
				}
				binaryFields.Add(binaryDataParameter);
			}
		}
