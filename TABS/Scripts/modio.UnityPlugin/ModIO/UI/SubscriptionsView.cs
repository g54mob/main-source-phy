...
namespace ModIO.UI
{
	public class SubscriptionsView : MonoBehaviour, IModSubscriptionsUpdateReceiver, IBrowserView
	{
		...
		public string nameFieldFilter => m_nameFieldFilter;

		public Comparison<ModProfile> sortDelegate => m_sortDelegate;
		...
		public IEnumerable<ModView> modViews => null;
		...
		{
			ISubscriptionsViewElement[] componentsInChildren = base.gameObject.GetComponentsInChildren<ISubscriptionsViewElement>(includeInactive: true);
			for (int i = 0; i < componentsInChildren.Length; i++)
			{
				componentsInChildren[i].SetSubscriptionsView(this);
			}
		...
		public void Refresh()
		{
			...
			{
				noSubscriptionsDisplay.gameObject.SetActive(value: true);
			}
			if (noResultsDisplay != null)
			{
				noResultsDisplay.gameObject.SetActive(value: false);
			}
			IList<int> subscribedModIds = LocalUser.SubscribedModIds;
			ModProfileRequestManager.instance.RequestModProfiles(subscribedModIds, delegate(ModProfile[] profiles)
			{
