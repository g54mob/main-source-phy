...
namespace ModIO.UI
{
	public class YouTubeThumbnailContainer : MonoBehaviour, IModViewElement
	{
		...
		protected virtual void Start()
		{
			...
			m_itemTemplate = containerTemplate.GetComponentInChildren<YouTubeThumbnailDisplay>(includeInactive: true);
			bool flag = parent.childCount > num && parent.GetChild(num).gameObject.name == text;
			if (flag)
			{
				m_templateClone = parent.GetChild(num).gameObject;
				YouTubeThumbnailDisplay[] componentsInChildren = m_templateClone.GetComponentsInChildren<YouTubeThumbnailDisplay>(includeInactive: true);
			...
			if (!flag)
			{
				...
				YouTubeThumbnailDisplay componentInChildren = m_templateClone.GetComponentInChildren<YouTubeThumbnailDisplay>(includeInactive: true);
		...
		public virtual void DisplayThumbnails(int modId, IList<string> youTubeIds)
		{
			...
			if (m_itemTemplate != null)
			{
				...
				m_templateClone.SetActive(m_youTubeIds.Length != 0 || !hideIfEmpty);
			}
		}
