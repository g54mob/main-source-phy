...
namespace ModIO.UI
{
	...
	public class ExplorerFilterTagsDropdown : MonoBehaviour, IExplorerViewElement, ICancelHandler, IEventSystemHandler
	{
		...
		public GameTagCategoryDisplay categoryDisplay => base.gameObject.GetComponent<GameTagCategoryDisplay>();
		...
		public void UpdateSelectedTagsDisplay(IEnumerable<string> selectedTags)
		{
			...
			foreach (TagContainerItem tagItem in categoryDisplay.tagItems)
			{
				...
				tagItem.GetComponentInChildren<StateToggleDisplay>(includeInactive: true).isOn = flag;
			}
			m_isUpdating = false;
		}
		...
		public void ToggleTagInExplorerFilter(TagContainerItem tagItem)
		{
			if (m_view != null && !m_isUpdating)
			{
				...
				if (tagItem.GetComponentInChildren<StateToggleDisplay>(includeInactive: true).isOn)
				{
		...
		public static bool HasValidTemplate(ExplorerFilterTagsDropdown selector, out string helpMessage)
		{
			...
			bool result = true;
			if (selector.categoryDisplay.template.tagTemplate.gameObject.GetComponentInChildren<StateToggleDisplay>(includeInactive: true) == null)
			{
