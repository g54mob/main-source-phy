...
namespace ModIO.UI
{
	...
	public class ExplorerTagFilterView : MonoBehaviour, IGameProfileUpdateReceiver
	{
		...
		private void Start()
		{
			tagCategoryPrefab.GetComponent<ModTagContainer>();
		...
		private void UpdateSelectionDisplay()
		{
			foreach (ModTagCategoryDisplay categoryDisplay in m_categoryDisplays)
			{
				...
				{
					tagDisplay.GetComponent<Toggle>().isOn = m_selectedTags.Contains(tagDisplay.data.tagName);
				}
				modTagContainer.tagClicked += TagClickHandler;
			}
		}
		...
		private GameObject CreateCategoryDisplayInstance(ModTagCategory category, GameObject prefab, RectTransform container)
		{
			...
			foreach (ModTagDisplay tagDisplay in gameObject.GetComponent<ModTagContainer>().tagDisplays)
			{
				Toggle component2 = tagDisplay.GetComponent<Toggle>();
				component2.isOn = m_selectedTags.Contains(tagDisplay.data.tagName);
