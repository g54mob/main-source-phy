	...
	public class ModfileView : MonoBehaviour
	{
		...
		public Modfile modfile
		{
			...
			set
			{
				...
				{
					m_modfile = value;
					if (string.IsNullOrEmpty(m_modfile.changelog))
					{
		...
		{
			IModfileViewElement[] componentsInChildren = base.gameObject.GetComponentsInChildren<IModfileViewElement>(includeInactive: true);
			for (int i = 0; i < componentsInChildren.Length; i++)
			{
				componentsInChildren[i].SetModfileView(this);
			}
		}
	}
}
