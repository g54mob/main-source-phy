...
namespace ModIO.UI
{
	public class GalleryImageContainer : MonoBehaviour, IModViewElement
	{
		...
		protected virtual void Start()
		{
			...
			m_itemTemplate = containerTemplate.GetComponentInChildren<GalleryImageDisplay>(includeInactive: true);
			bool flag = parent.childCount > num && parent.GetChild(num).gameObject.name == text;
			if (flag)
			{
				m_templateClone = parent.GetChild(num).gameObject;
				GalleryImageDisplay[] componentsInChildren = m_templateClone.GetComponentsInChildren<GalleryImageDisplay>(includeInactive: true);
			...
			if (!flag)
			{
				...
				GalleryImageDisplay componentInChildren = m_templateClone.GetComponentInChildren<GalleryImageDisplay>(includeInactive: true);
		...
		public virtual void DisplayImages(int modId, IList<GalleryImageLocator> locators)
		{
			...
			if (m_itemTemplate != null)
			{
				...
				m_templateClone.SetActive(m_locators.Length != 0 || !hideIfEmpty);
			}
		}
