...
namespace ModIO
{
	public static class ModManager
	{
		...
		public const int BaseModVersion = 1;

		public const string ModVersionFileResource = "ModVersion";

		public static int ModVersion { get; private set; }

		[Obsolete("Use DataStorage.INSTALLATION_DIRECTORY instead")]
		public static string installationDirectory => DataStorage.INSTALLATION_DIRECTORY;
		...
		static ModManager()
		{
			...
			ReadAndSetModVersion();
		}

		private static void ReadAndSetModVersion()
		{
			TextAsset textAsset = Resources.Load<TextAsset>("ModVersion");
			if (textAsset != null)
			{
				if (int.TryParse(textAsset.text, out var result))
				{
					if (result < 1)
					{
						Debug.LogError("Parsed ModVersion is invalid.");
					}
					else
					{
						ModVersion = result;
				...
				{
					Debug.LogError("Failed to parse ModVersion file.");
				}
			}
			else
			{
				Debug.LogError("Failed to load ModVersion file from Resources.");
			}
		}
		...
		public static void TryInstallMod(int modId, int modfileId, Action<bool> onComplete)
		{
			...
			getFileExistsCallback = delegate(string path, bool success)
			{
				...
				try
				{
					Debug.Log("DM: Reading Zip: " + archivePath);
					DataStorage.ReadFile(archivePath, delegate(string readPath, bool readSuccess, byte[] readData)
					{
						int remainingOpsCount = 1;
						using (MemoryStream zipStream = new MemoryStream(readData))
						{
							using (ZipFile zipFile = ZipFile.Read(zipStream))
							{
								Debug.Log("DM: Extracting Zip to: " + tempLocation);
								foreach (ZipEntry item in zipFile)
								{
									int num = remainingOpsCount + 1;
									remainingOpsCount = num;
									...
									{
										Debug.LogError("Bad Read on " + item.FileName + ". Probably a corrupt thumbnail file.");
									}
									DataStorage.WriteFile(Path.Combine(tempLocation, item.FileName), memoryStream.GetBuffer(), delegate
									{
										Proceed();
									});
								}
						...
						void Proceed()
						{
							int num2 = remainingOpsCount - 1;
							remainingOpsCount = num2;
		...
		public static void QueryInstalledMods(IList<int> modIdFilter, Action<IList<KeyValuePair<ModfileIdPair, string>>> onComplete)
		{
			DataStorage.GetDirectories(DataStorage.INSTALLATION_DIRECTORY, delegate(string path, bool exists, IList<string> modDirectories)
			{
				...
				if (exists && modDirectories != null)
				{
					...
					{
						string[] array = IOUtilities.GetPathItemName(modDirectory).Split('_');
						if (array.Length == 0 || !int.TryParse(array[0], out var result))
						{
							result = 0;
						}
						if (modIdFilter == null || modIdFilter.Contains(result))
						{
							if (result == 0 || array.Length <= 1 || !int.TryParse(array[1], out var result2))
							{
								result2 = 0;
							}
		...
		public static IEnumerator DownloadAndUpdateMods_Coroutine(IList<int> modIds, Action onCompleted = null)
		{
			...
			AddTagsToDisableCrossPlatformMods(modFilter);
			while (!isRequestResolved && attemptCount < attemptLimit)
			{
				...
				if (error != null)
				{
					...
					int num = attemptCount + 1;
					attemptCount = num;
					int num2 = calcReattemptDelay(error);
					yield return new WaitForSecondsRealtime(num2);
					continue;
				}
				foreach (ModProfile item in profiles)
				{
					lastestBuilds.Add(item.currentBuild);
				}
				isRequestResolved = true;
			}
		...
		public static IEnumerator AssertDownloadedAndInstalled_Coroutine(IEnumerable<Modfile> modfiles, Action onCompleted = null)
		{
			...
			QueryInstalledModVersions(excludeDisabledMods: false, delegate(List<ModfileIdPair> r)
			{
			...
			int i = 0;
			while (i < unmatchedModfiles.Count)
			{
				...
				else
				{
					...
					{
						if (item.modId == m.modId && item.modfileId == m.id)
						{
							isInstalled = true;
							break;
						}
					...
					if (!isInstalled)
					{
						string path = CacheClient.GenerateModBinaryZipFilePath(m.modId, m.id);
						bool fileExists = false;
						long fileSize = -1L;
						string fileHash = null;
						bool isIOOpDone = false;
						DataStorage.GetFileSizeAndHash(path, delegate(string p, bool success, long byteCount, string md5Hash)
						{
							fileExists = success;
							fileSize = byteCount;
							fileHash = md5Hash;
			...
			for (int num2 = 0; num2 < unmatchedModfiles.Count; num2++)
			{
				int modIndex = num2;
				Modfile modfile = unmatchedModfiles[num2];
				if (modfile.downloadLocator != null && modfile.downloadLocator.dateExpires > ServerTimeStamp.Now)
				{
					continue;
				}
				int num = awaitingModfileUpdates + 1;
				awaitingModfileUpdates = num;
				APIClient.GetModfile(modfile.modId, modfile.id, delegate(Modfile updatedModfile)
				{
					int num3 = awaitingModfileUpdates - 1;
					awaitingModfileUpdates = num3;
					if (modfile.downloadLocator == null || modfile.downloadLocator.dateExpires <= ServerTimeStamp.Now)
					{
			...
			foreach (Modfile item2 in badModfiles)
			{
				unmatchedModfiles.Remove(item2);
			}
			if (unmatchedModfiles.Count > 0)
			{
				bool startNextDownload = false;
				Modfile downloadingModfile = null;
				Action<ModfileIdPair, FileDownloadInfo> onDownloadSucceeded = delegate(ModfileIdPair idPair, FileDownloadInfo info)
				{
				...
				Action<ModfileIdPair, WebRequestError> onDownloadFailed = delegate(ModfileIdPair idPair, WebRequestError e)
				{
					if (idPair.modfileId == downloadingModfile.id)
					{
				...
				DownloadClient.modfileDownloadSucceeded += onDownloadSucceeded;
				DownloadClient.modfileDownloadFailed += onDownloadFailed;
				foreach (Modfile item3 in unmatchedModfiles)
				{
					if (DownloadClient.GetActiveModBinaryDownload(item3.modId, item3.id) == null)
					{
						downloadingModfile = item3;
						startNextDownload = false;
						string targetFilePath = CacheClient.GenerateModBinaryZipFilePath(downloadingModfile.modId, downloadingModfile.id);
						DownloadClient.StartModBinaryDownload(item3, targetFilePath);
		...
		public static void GetGameProfile(Action<GameProfile> onSuccess, Action<WebRequestError> onError)
		{
			CacheClient.LoadGameProfile(delegate(GameProfile cachedProfile)
			{
				...
				{
					APIClient.GetGame(delegate(GameProfile profile)
					{
		...
		public static void GetModProfile(int modId, Action<ModProfile> onSuccess, Action<WebRequestError> onError)
		{
			...
			APIClient.GetMod(modId, delegate(ModProfile p)
			{
				if (p == null)
				{
					if (onError != null)
					{
						onError(WebRequestError.GenerateLocal("Invalid ModProfile (GetModProfile)"));
					}
				}
				else
				{
		...
		public static void GetModProfiles(IList<int> orderedIdList, Action<ModProfile[]> onSuccess, Action<WebRequestError> onError)
		{
			...
			for (int i = 0; i < orderedIdList.Count; i++)
			{
				...
				if (RequestCache.TryGetMod(PluginSettings.GAME_ID, num, out var profile))
				{
		...
		public static void FetchModEvents(IEnumerable<int> modIdFilter, int fromTimeStamp, int untilTimeStamp, Action<List<ModEvent>> onSuccess, Action<WebRequestError> onError)
		{
			...
			modEventFilter.sortFieldName = "date_added";
			modEventFilter.AddFieldFilter("date_added", new MinimumFilter<int>(0)
			{
				minimum = fromTimeStamp,
				isInclusive = false
			});
			modEventFilter.AddFieldFilter("date_added", new MaximumFilter<int>(0)
			{
			...
			if (modIdFilter != null)
			{
				modEventFilter.AddFieldFilter("mod_id", new InArrayFilter<int>
				{
					filterArray = modIdFilter.ToArray()
				});
			}
			FetchAllResultsForQuery(delegate(APIPaginationParameters p, Action<RequestPage<ModEvent>> s, Action<WebRequestError> e)
			{
				APIClient.GetAllModEvents(modEventFilter, p, s, e);
			}, onSuccess, onError);
		}
		...
		{
			RequestFilter modEventFilter = new RequestFilter();
			modEventFilter.sortFieldName = "id";
			modEventFilter.AddFieldFilter("id", new MinimumFilter<int>(0)
			{
				minimum = eventId,
				isInclusive = false
			});
		...
		public static void FetchAllUserEvents(int fromTimeStamp, int untilTimeStamp, Action<List<UserEvent>> onSuccess, Action<WebRequestError> onError)
		{
			...
			userEventFilter.sortFieldName = "date_added";
			userEventFilter.AddFieldFilter("date_added", new MinimumFilter<int>(0)
			{
				minimum = fromTimeStamp,
				isInclusive = false
			});
			userEventFilter.AddFieldFilter("date_added", new MaximumFilter<int>(0)
			{
				maximum = untilTimeStamp,
				isInclusive = true
			});
			userEventFilter.AddFieldFilter("game_id", new EqualToFilter<int>(0)
			{
				filterValue = PluginSettings.GAME_ID
			});
			FetchAllResultsForQuery(delegate(APIPaginationParameters p, Action<RequestPage<UserEvent>> s, Action<WebRequestError> e)
			{
				APIClient.GetUserEvents(userEventFilter, p, s, e);
			}, onSuccess, onError);
		}
		...
		{
			RequestFilter userEventFilter = new RequestFilter();
			userEventFilter.sortFieldName = "id";
			userEventFilter.AddFieldFilter("id", new MinimumFilter<int>(0)
			{
				minimum = eventId,
				isInclusive = false
			});
		...
		public static void UploadModBinaryDirectory(int modId, EditableModfile modfileValues, string binaryDirectory, bool setActiveBuild, Action<Modfile> onSuccess, Action<WebRequestError> onError)
		{
			DataStorage.GetDirectoryExists(binaryDirectory, delegate(string dir_path, bool dir_exists)
			{
				if (dir_exists)
				{
					...
					DataStorage.GetFiles(binaryDirectory, null, recurseSubdirectories: true, delegate(string path, bool success, IList<string> fileList)
					{
		...
		public static void UploadModBinaryFileList(int modId, EditableModfile modfileValues, string rootDirectory, IList<string> fileList, bool setActiveBuild, Action<Modfile> onSuccess, Action<WebRequestError> onError)
		{
			...
			{
				int num = rootDirectoryLength + 1;
				rootDirectoryLength = num;
			}
			...
			DataStorage.CreateDirectory(text, delegate(string path, bool success)
			{
				if (success)
				{
					try
					{
						using (ZipFile zipFile = new ZipFile())
						{
							...
							{
								string directoryName = Path.GetDirectoryName(file.Substring(rootDirectoryLength));
				...
				string text2 = "Unable to create directory for mod binary archive prior to upload.\nDirectory: " + path;
				Debug.LogError("[mod.io] " + text2);
				if (onError != null)
				{
					WebRequestError obj5 = WebRequestError.GenerateLocal(text2);
					onError(obj5);
				}
			});
		}
		...
		public static void UploadModBinary_Unzipped(int modId, EditableModfile modfileValues, string unzippedBinaryLocation, bool setActiveBuild, Action<Modfile> onSuccess, Action<WebRequestError> onError)
		{
			...
			DataStorage.CreateDirectory(Path.GetDirectoryName(binaryZipLocation), delegate
			{
				try
				{
					using (ZipFile zipFile = new ZipFile())
					{
						zipFile.AddFile(unzippedBinaryLocation, "");
						zipFile.Save(binaryZipLocation);
					}
					zipSucceeded = true;
				}
				catch (Exception e)
		...
		public static void GetAuthenticatedUserProfile(Action<UserProfile> onSuccess, Action<WebRequestError> onError)
		{
			...
			{
				onSuccess?.Invoke(LocalUser.Profile);
			}
		}
		...
		public static void FetchAuthenticatedUserMods(Action<List<ModProfile>> onSuccess, Action<WebRequestError> onError)
		{
			...
			FetchAllResultsForQuery(onSuccess: delegate(List<ModProfile> modProfiles)
			{
			...
			}, query: delegate(APIPaginationParameters p, Action<RequestPage<ModProfile>> s, Action<WebRequestError> e)
			{
				APIClient.GetUserMods(userModsFilter, p, s, e);
			}, onError: onError);
		}
		...
		{
			List<ModfileIdPair> installedModVersions = GetInstalledModVersions(excludeDisabledMods: false);
		...
		public static void GetUserProfile(int userId, Action<UserProfile> onSuccess, Action<WebRequestError> onError)
		{
			if (UserAuthenticationData.instance.userId == userId)
			{
				...
				APIClient.GetAuthenticatedUser(delegate(UserProfile profile)
				{
			...
			{
				onError?.Invoke(WebRequestError.GenerateLocal("Non-authenticated user profiles can no-longer be fetched."));
			}
		}
		...
		public static void AddTagsToDisableCrossPlatformMods(RequestFilter modFilter)
		{
			string[] tagsToDisableCrossPlatformMods = GetTagsToDisableCrossPlatformMods();
			if (tagsToDisableCrossPlatformMods == null)
			{
				return;
			}
			List<string> list = new List<string>(tagsToDisableCrossPlatformMods);
			if (modFilter.fieldFilterMap != null && modFilter.fieldFilterMap.TryGetValue("tags", out var value))
			{
				int i = 0;
				for (int count = value.Count; i < count; i++)
				{
					string item = (string)value[i].filterValue;
					list.Remove(item);
				}
				if (list.Count <= 0)
				{
			...
			modFilter.AddFieldFilter("tags", new InArrayFilter<string>
			{
				filterArray = list.ToArray()
			});
		}

		public static string[] GetTagsToDisableCrossPlatformMods()
		{
			return null;
		}
	}
