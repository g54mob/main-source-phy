...
public class DodgeMove : Move, IRemotelyControllable
{
	[HideInInspector]
	...
	{
		float t = move.forceCurve[move.forceCurve.length - 1].time;
		float c = 0f;
		...
		while (c < t && !data.Dead)
		{
			if (CheckConditions())
			{
				for (int j = 0; j < rigs.Count; j++)
				{
					...
					if (!usedAsMovement || !data || !data.anim || data.anim.currentState != 0)
					{
						float num3 = massM * forceMultiplier * data.ragdollControl * move.forceCurve.Evaluate(c);
						if (move.force != 0f && (bool)rigs[j])
						{
							rigs[j].AddForce(num3 * move.force * move.randomMultiplier * forceDirection, ForceMode.Acceleration);
						}
						if (move.torque != 0f && (bool)rigs[j])
						{
							rigs[j].AddTorque(num3 * move.torque * forceDirection, ForceMode.Acceleration);
						}
					}
				}
			}
			...
			{
				num4 *= animationSpeedWhenPositiveCurve;
			}
			c += Time.fixedDeltaTime * num4;
