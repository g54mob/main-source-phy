...
public class DMLoginDialog : MonoBehaviour, IBrowserView
{
	[Serializable]
	...
	[Tooltip("Black background which is shown when we open the Workshop from the Custom Content menu.")]
	...
	{
		string toCheck = inputField.text.Trim().Replace(" ", "");
	...
	public void TrySubmitAuthentication()
	{
		...
		if (Utility.IsEmail(text))
		{
			...
			{
				ProcessWebRequestError(e, isSecurityCode: false);
			});
		}
		else if (Utility.IsSecurityCode(text))
		{
			...
			{
				ProcessWebRequestError(e, isSecurityCode: true);
			});
		}
		else
		{
