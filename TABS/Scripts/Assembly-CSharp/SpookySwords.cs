...
public class SpookySwords : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (num2 && (bool)data.targetMainRig && counter > attackRate)
		{
			...
			if ((bool)data.targetMainRig && data.distanceToTarget < 10f)
			{
		...
		for (int k = 0; k < swords.Count; k++)
		{
			if (!(swords[k].gameObject == null))
			{
				...
				swords[k].move.velocity = (swordPoints[k].transform.position - swords[k].gameObject.transform.position) * followSpeed;
				swords[k].gameObject.transform.rotation = quaternion.LookRotation(Vector3.up, swordPoints[k].transform.position - base.transform.position);
			}
		}
	}
	...
	private IEnumerator DoAttack(SpookySword attackSword, Rigidbody targ)
	{
		...
		while (counter < t && (!stick || !stick.stuck))
		{
			...
			attackSword.move.velocity = Vector3.Lerp(attackSword.move.velocity, throwCurve.Evaluate(counter) * throwSpeed * (targ.position - attackSword.gameObject.transform.position).normalized, num * 8f);
		...
		counter = 0f;
		while (counter < 0.5f && (!stick || !stick.stuck))
		{
