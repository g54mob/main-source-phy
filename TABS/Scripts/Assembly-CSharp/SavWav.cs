...
public static class SavWav
{
	...
	public static bool Save(string filename, AudioClip clip, bool saveInProject = false)
	{
		if (!filename.ToLower().EndsWith(".wav"))
		{
			filename += ".wav";
		}
		string text = ((!saveInProject) ? Path.Combine(Application.persistentDataPath, filename) : Path.Combine(Application.dataPath.Replace("/Assets", ""), filename));
		Debug.Log(text);
