...
public class LineEffects : MonoBehaviour
{
	...
	public bool disableAfterSec = true;
	...
	private void Start()
	{
		...
		tile = UnityEngine.Random.Range(0.7f, 1.3f);
	}
	...
	public void DrawLine(Vector3 start, Vector3 end, Vector3 bezier)
	{
		...
		for (int i = 0; i < line.positionCount; i++)
		{
			...
			for (int j = 0; j < effects.Length; j++)
			{
				...
				float time = num3;
				float mainCurveTiling = effects[j].mainCurveTiling;
				mainCurveTiling *= tile;
				if (effects[j].tilingPerMeter)
				{
					if (lineType == LineType.Line)
					{
						mainCurveTiling *= (start - end).magnitude;
					}
					if (lineType == LineType.Ring)
					{
						mainCurveTiling *= radius;
					}
				}
				num3 *= mainCurveTiling;
				if (effects[j].mainCurveScrollSpeed > 0f)
				{
					num3 += effects[j].mainCurveScrollSpeed * Time.time;
				}
				if (effects[j].mainCurveScrollSpeed < 0f)
				{
					num3 += (0f - effects[j].mainCurveScrollSpeed) * (100000f - Time.time);
				}
				num3 %= 1f;
			...
			if (lineType == LineType.Ring)
			{
				...
				vector3 = ((!(vector != Vector3.zero)) ? Vector3.up : Vector3.Cross(MainCam.instance.transform.forward, vector2 - vector).normalized);
			}
			if (lineType == LineType.Line)
			{
				vector2 = Vector3.Lerp(start, end, t);
				vector3 = Vector3.Cross(start - end, MainCam.instance.transform.forward).normalized;
			...
			vector = vector2;
			line.SetPosition(i, vector2 + num * offsetMultiplier * vector3);
		}
		if (lineType == LineType.Ring)
		{
			SmoothSeam();
		...
		counter += Time.deltaTime * globalTimeSpeed;
		if (counter > 1f)
		{
			if (debug || loop)
			{
	...
	private IEnumerator DoPlay(Transform fromTransform, Transform toTransform, float bezierOffset = 0f)
	{
		...
		while (base.gameObject.activeSelf && (bool)fromTransform && (bool)toTransform)
		{
			if (bezierOffset != 0f)
			{
				...
				Vector3 vector = bezierOffset * num * Vector3.Cross(Vector3.forward, toTransform.position - fromTransform.position).normalized;
				currentBez = ((!(currentBez == Vector3.zero)) ? Vector3.Lerp(currentBez, vector, Time.deltaTime * 5f) : vector);
	...
	private IEnumerator DoPlay(Transform fromTransform, Vector3 toPosition, float bezierOffset = 0f)
	{
		...
		while (base.gameObject.activeSelf && (bool)fromTransform)
		{
			if (bezierOffset != 0f)
			{
				...
				Vector3 vector = bezierOffset * num * Vector3.Cross(Vector3.forward, toPosition - fromTransform.position).normalized;
				currentBezOffset = ((!(currentBezOffset == Vector3.zero)) ? Vector3.Lerp(currentBezOffset, vector, Time.deltaTime * 2f) : vector);
