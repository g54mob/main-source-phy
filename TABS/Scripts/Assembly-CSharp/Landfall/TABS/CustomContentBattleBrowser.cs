...
{
	public class CustomContentBattleBrowser : CustomContentGridBrowser
	{
		...
		private void SpawnBattle(TABSCampaignLevelAsset battle)
		{
			spawnedObjects.Add(Object.Instantiate(battlePrefab, base.CurrentLayoutGroup.transform).GetComponent<CustomContentBattleButton>().Setup(battle)
				.gameObject);
			}
			...
			public override void Populate(int page = 0, int newLayoutGroup = 0)
			{
				...
				Clear();
				TABSCampaignLevelAsset[] array = ContentDatabase.Instance().GetUserCampaignLevelsByFilter(Filter.CreateMatchNamePartAndTypeFilter(filterInputField.text, UnitCreatorFactionBrowser.showDownloaded ? WorkshopTypeFilter.Workshop : WorkshopTypeFilter.Local)).ToArray();
				UpdateNewContentGraphic(default(DMNewContentManager.NewContentID), WorkshopContentType.Battle);
