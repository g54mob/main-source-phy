...
{
	public class BattleCreatorCampaignCreatorUI : MonoBehaviour, IBattleCreatorMenu
	{
		...
		[SerializeField]
		private Transform m_SelectedMapsGrid;

		[SerializeField]
		private GameObject m_ContentCell;

		[SerializeField]
		private Transform m_moveBattleArrowButton;
		...
		private Selectable lastBattleSelected;
		...
		private BattleCreatorState m_BattleCreatorState;
				...
				UnityAction saveLocalAction = delegate
				{
					...
					m_CampaignScreenshotMaker.PopulateImages(TABSCampaignAsset.DeserializeCampaign(sequence, contentDatabase.GetCampaignLevel), delegate
					{
						CampaignHandler.SaveCampaign(sequence, existingAsset, delegate(bool success)
						{
							...
							if (success)
							{
								ServiceLocator.GetService<ModalPanel>().PopUp("Save Successful!");
							}
						});
					});
				};
				TABSCampaignAsset existingCampaign = contentDatabase.GetUserCampaignsByFilter(Filter.CreateMatchLocalAndExactNameFilter(m_campaignName)).FirstOrDefault();
				if (existingCampaign != null)
				{
					ServiceLocator.GetService<ModalPanel>().Choice("Save and replace?", "Campaign: " + sequence.CampaignName + " Does Already Exist, Do You Want To Overwrite?", delegate
					{
		...
		private bool ValidateCampaign()
		{
			...
			if (service != null)
			{
				...
				{
					service.PopUp("No Maps Have Been Selected, Cannot Save Campaign!", SelectCampaignNameInput);
					return false;
				}
				if (!ValidateCampaignNameText())
				{
					service.PopUp("Campaign Name Cannot Be Empty!", SelectCampaignNameInput);
					return false;
				}
				if (!ValidateCampaignDescriptionText())
				{
					service.PopUp("Campaign Description Cannot Be Empty!", SelectCampaignNameInput);
					return false;
				}
			}
			return true;
		...
		private void Populate()
		{
			...
			{
				foreach (TABSCampaignAsset userCampaign in ContentDatabase.Instance().GetUserCampaigns())
				{
					if (m_BattleCreatorState == BattleCreatorState.Upload)
					{
						if (!CheckForSelectedContent(userCampaign))
						{
							SpawnCampaignCell(userCampaign);
						}
					}
					else
					{
			...
			if (m_ContentFilter != ContentTypeFilter.Units)
			{
				return;
			}
			foreach (UnitBlueprint userUnitBlueprint in ContentDatabase.Instance().GetUserUnitBlueprints())
			{
				if (!userUnitBlueprint.IsCustomUnit)
				{
					continue;
				}
				if (m_BattleCreatorState == BattleCreatorState.Upload)
				{
					if (!CheckForSelectedContent(userUnitBlueprint))
					{
						SpawnUnitCell(userUnitBlueprint);
					}
				}
				else
				{
				...
				m_FileIO.FileExists(item.FilePath, FileHandlingFileType.CustomContentOrLocalStorageFile, delegate(bool exists)
				{
					...
					else
					{
						if (new FileInfo(item.FilePath).Extension == CustomContentFilePaths.FileEndingBattle)
						{
							...
							else if (m_BattleCreatorState == BattleCreatorState.Upload)
							{
								if (!CheckForSelectedContent(item))
								{
									SpawnLevelCell(item);
								}
							}
							else if (m_WorkshopFilter == WorkshopTypeFilter.Local && !item.IsModIOLevel)
							{
								SpawnLevelCell(item);
							}
						}
		...
		private bool CheckForSelectedContent(IDatabaseEntity item)
		{
			int childCount = m_SelectedMapsGrid.childCount;
			for (int i = 0; i < childCount; i++)
			{
				BattleCreatorAssetUICell component = m_SelectedMapsGrid.GetChild(i).GetComponent<BattleCreatorAssetUICell>();
				if ((bool)component && component.ContentType == m_ContentFilter && component.ContentName == item.Entity.Name)
				{
					return true;
				}
			}
		...
		private void SpawnUnitCell(IDatabaseEntity unit)
		{
			GameObject obj = UnityEngine.Object.Instantiate(m_ContentCell);
			BattleCreatorAssetUICell battleCreatorAssetUICell = obj.FetchComponent<BattleCreatorAssetUICell>();
			string levelName = unit.Entity.Name;
			battleCreatorAssetUICell.Init(new BattleCreatorAssetUICellBase.UnitData(levelName, (UnitBlueprint)unit, null, OnLevelDeleteClicked, OnLevelCogClicked, m_ContentFilter, m_BattleCreatorState));
			obj.transform.SetParent(m_LibraryGrid, worldPositionStays: true);
		...
		private void SpawnCampaignCell(TABSCampaignAsset campaign)
		{
			GameObject obj = UnityEngine.Object.Instantiate(m_ContentCell);
			BattleCreatorAssetUICell battleCreatorAssetUICell = obj.FetchComponent<BattleCreatorAssetUICell>();
			string levelName = campaign.Entity.Name;
			battleCreatorAssetUICell.Init(new BattleCreatorAssetUICellBase.CampaignData(levelName, campaign, null, OnLevelDeleteClicked, OnLevelCogClicked, null, null, m_ContentFilter, m_BattleCreatorState));
			obj.transform.SetParent(m_LibraryGrid, worldPositionStays: true);
			obj.transform.localScale = Vector3.one;
			obj.SetActive(value: true);
		}
		...
		private void SpawnLevelCellAlreadySelected(TABSCampaignLevelAsset level, int index)
		{
			if (!(level == null))
			{
				GameObject obj = UnityEngine.Object.Instantiate(m_ContentCell, m_LibraryGrid, worldPositionStays: true);
				...
				component.Init(m_LibraryGrid, m_SelectedMapsGrid, m_campaignList);
				component.Init(new BattleCreatorAssetUICellBase.CampaignLevelData(level.Entity.Name, level, null, null, null, null, null, m_ContentFilter, m_BattleCreatorState));
		...
		{
			m_OptionMenu.Open(cellUI, m_BattleCreatorState);
		}
		...
		private void OnUploadClicked(BattleCreatorAssetUICellBase cellUI)
		{
		}

		private void ClearAll(bool force = false)
		{
			ClearLibrary();
			if (m_BattleCreatorState != BattleCreatorState.Upload || force)
			{
				ClearSelected();
			}
		...
		private void ClearLibrary()
		{
			...
			{
				UnityEngine.Object.DestroyImmediate(m_LibraryGrid.GetChild(num).gameObject);
			}
		}

		private void ClearSelected()
		{
			for (int num = m_SelectedMapsGrid.childCount - 1; num >= 0; num--)
			{
				UnityEngine.Object.DestroyImmediate(m_SelectedMapsGrid.GetChild(num).gameObject);
			}
		}

		private void OnSwitchedContentFilter(ContentTypeFilter filter)
		{
			if (m_ContentFilter != filter)
			{
				m_ContentFilter = filter;
				ClearAll();
				Populate();
			}
		}
		...
		{
			BattleCreatorState battleCreatorState = m_BattleCreatorState;
			if (battleCreatorState == BattleCreatorState.CampaignCreator)
			{
				if (m_AlreadySelectedCampaign == null)
				{
					m_HeaderText.text = "CREATE CAMPAIGN";
				}
				else
				{
					m_HeaderText.text = "EDIT CAMPAIGN: " + m_AlreadySelectedCampaign.Entity.Name;
				}
			...
			{
				Debug.LogError("Invalid creator state for screen, " + base.gameObject.name + " : " + m_BattleCreatorState);
			}
		}
		...
		public void Open(BattleCreatorState state, object data = null)
		{
			...
			SandboxUIManager.UIMode = SandboxUIManager.PlacementUIMode.SelectionMode;
			m_BattleCreatorState = state;
		...
		public void Close()
		{
			...
			SandboxUIManager.UIMode = SandboxUIManager.PlacementUIMode.PlacementMode;
			InstancedHandler<SandboxUIManager>.Instance.ClearActions();
		...
		public void OpenDesignCampaign()
		{
			...
			if (m_libraryGridButtons.Count > 0)
			{
		...
		public void OpenSaveCampaign(bool revertCampaignSelection)
		{
			...
			if (revertCampaignSelection && m_libraryGridButtons != null)
			{
				...
				foreach (Selectable libraryGridButton in m_libraryGridButtons)
				{
					CampaignCreatorBattleCell component = libraryGridButton.GetComponent<CampaignCreatorBattleCell>();
					component.campaignList = m_campaignList;
		...
		public bool NavigateUIWithController(PlayerActions playerActions)
		{
			...
			case CampaignCreatorUIMode.Designing:
				...
				{
					EventSystem.current.currentSelectedGameObject?.GetComponent<CampaignCreatorBattleCell>()?.ToggleFromList();
				}
				break;
		...
		private void OnInputSourceChanged(InputType type)
		{
			...
			m_moveBattleArrowButton.GetComponent<Button>().enabled = type == InputType.Controller;
		}
		...
		private void ResetTexts()
		{
			if (!(m_AlreadySelectedCampaign != null))
			{
				OnCampaignNameChanged(string.Empty);
				OnCampaignDescriptionChanged(string.Empty);
				OnThankyouTitleChanged(string.Empty);
				OnThankyouDescriptionChanged(string.Empty);
			}
		}
	}
}
