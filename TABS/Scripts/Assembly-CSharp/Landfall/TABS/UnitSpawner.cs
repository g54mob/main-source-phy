...
{
	public class UnitSpawner : MonoBehaviour, GameObjectPooling.IPoolable
	{
		...
		private int originalAmountToSpawn;
		...
		private void Start()
		{
			...
			originalAmountToSpawn = amountToSpawn;
			if (!IsManagedByPool)
			{
				InitializeOnSpawn();
			}
		...
		public void Release()
		{
			amountToSpawn = originalAmountToSpawn;
		}
		...
		public Unit Spawn()
		{
			...
			if (spawnWithRootScaleAndMass && isHumanoid)
			{
				...
				unitBlueprint.Spawn(spawnPos, base.transform.rotation, team, out unitToSpawn);
