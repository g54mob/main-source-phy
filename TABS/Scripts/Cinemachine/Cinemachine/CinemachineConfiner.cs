...
namespace Cinemachine
{
	...
	public class CinemachineConfiner : CinemachineExtension
	{
		...
		protected override void ConnectToVcam(bool connect)
		{
			...
			CinemachineVirtualCamera cinemachineVirtualCamera = base.VirtualCamera as CinemachineVirtualCamera;
			if (cinemachineVirtualCamera == null)
			{
				return;
			}
			CinemachineComponentBase[] componentPipeline = cinemachineVirtualCamera.GetComponentPipeline();
			for (int i = 0; i < componentPipeline.Length; i++)
			{
				if (componentPipeline[i].BodyAppliesAfterAim)
				{
					GetExtraState<VcamExtraState>(cinemachineVirtualCamera).applyAfterAim = true;
					break;
				}
			}
		}
		...
		protected override void PostPipelineStageCallback(CinemachineVirtualCameraBase vcam, CinemachineCore.Stage stage, ref CameraState state, float deltaTime)
		{
			...
			if ((extraState.applyAfterAim && stage == CinemachineCore.Stage.Finalize) || (!extraState.applyAfterAim && stage == CinemachineCore.Stage.Body))
			{
