...
namespace Backtrace.Unity
{
	...
	public class BacktraceDatabase : MonoBehaviour, IBacktraceDatabase
	{
		...
		private void Update()
		{
			...
			LastFrameTime = Time.time;
			if (DatabaseSettings.AutoSendMode && Time.time - _lastConnection > (float)DatabaseSettings.RetryInterval)
			{
				_lastConnection = Time.time;
		...
		public BacktraceDatabaseRecord Add(BacktraceData data, bool @lock = true)
		{
			...
			BacktraceDatabaseRecord backtraceDatabaseRecord = BacktraceDatabaseContext.Add(data);
			if (!@lock)
			{
		...
		public BacktraceDatabaseRecord Add(BacktraceReport backtraceReport, Dictionary<string, string> attributes, MiniDumpType miniDumpType = MiniDumpType.None)
		{
			...
			BacktraceData backtraceData = backtraceReport.ToBacktraceData(attributes, Configuration.GameObjectDepth);
			return BacktraceDatabaseContext.Add(backtraceData);
		}
		...
		private void FlushRecord(BacktraceDatabaseRecord record)
		{
			...
			Stopwatch stopwatch = (Configuration.PerformanceStatistics ? new Stopwatch() : Stopwatch.StartNew());
			...
			if (text != null)
			{
				dictionary["_mod_duplicate"] = record.Count.ToString();
		...
		{
			Stopwatch stopwatch = (Configuration.PerformanceStatistics ? Stopwatch.StartNew() : new Stopwatch());
		...
		protected virtual bool InitializeDatabasePaths()
		{
			...
			{
				UnityEngine.Debug.LogWarning("Backtrace database path is empty.");
				return false;
			}
		...
		protected virtual void LoadReports()
		{
			...
			foreach (FileInfo record in BacktraceDatabaseFileContext.GetRecords())
			{
				BacktraceDatabaseRecord backtraceDatabaseRecord = BacktraceDatabaseRecord.ReadFromFile(record);
				if (backtraceDatabaseRecord == null)
				{
					continue;
				}
				backtraceDatabaseRecord.DatabasePath(DatabaseSettings.DatabasePath);
				if (!backtraceDatabaseRecord.Valid())
				{
					try
					{
						UnityEngine.Debug.Log("Removing record from Backtrace Database path - invalid record.");
						backtraceDatabaseRecord.Delete();
					}
					catch (Exception)
					{
						UnityEngine.Debug.LogWarning($"Cannot remove file from database. File name: {record.FullName}");
					}
				}
				else
				{
		...
		private bool ValidateDatabaseSize()
		{
			...
			if (DatabaseSettings.MaxDatabaseSize != 0L && BacktraceDatabaseContext.GetSize() > DatabaseSettings.MaxDatabaseSize)
			{
				int num = 5;
