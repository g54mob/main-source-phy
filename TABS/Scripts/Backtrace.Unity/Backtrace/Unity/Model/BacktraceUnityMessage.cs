...
namespace Backtrace.Unity.Model
{
	internal class BacktraceUnityMessage
	{
		...
		private string GetFormattedMessage(bool backtraceFrame)
		{
			...
			stringBuilder.AppendFormat("[{0}] {1}<{2}>: {3}", DateTime.Now.ToUniversalTime().ToString(), backtraceFrame ? "(Backtrace)" : string.Empty, Enum.GetName(typeof(LogType), Type), Message);
