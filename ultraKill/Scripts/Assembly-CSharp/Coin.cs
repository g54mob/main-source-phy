...
public class Coin : MonoBehaviour, ITarget
{
	...
	private void Start()
	{
		...
		explosiveQuery = new VisionQuery("CoinExplosive", (TargetDataRef t) => t.target.Type == TargetType.EXPLOSIVE && t.target.GameObject.activeSelf && ExplosiveReflectCheck(t) && t.DistanceTo(cachedPlayerPos) < 100f && !t.IsObstructed(base.transform.position, env_lm));
		...
		untrackCoinTokenSource = CancellationTokenSource.CreateLinkedTokenSource(base.destroyCancellationToken);
	}
	...
	public void GetDeleted()
	{
		...
		UnityEngine.Object.Destroy(base.transform.GetChild(1).GetComponent<AudioSource>());
