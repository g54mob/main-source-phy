...
public class FinalCyberRank : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (highScoresDisplayed && (bool)friendScrollRect && (bool)friendScrollRect.content)
		{
			float num6 = ((Mouse.current != null) ? Mouse.current.scroll.y.ReadValue() : 0f);
			if (num6 != 0f)
			{
				friendScrollRect.verticalNormalizedPosition += num6 / friendScrollRect.content.sizeDelta.y;
			}
			if (MonoSingleton<InputManager>.Instance.LastButtonDevice is Gamepad && Gamepad.current != null)
			{
				...
				if (vector.y != 0f)
				{
					friendScrollRect.verticalNormalizedPosition += 0.01f * Time.unscaledDeltaTime * controllerScrollSpeed * Mathf.Sign(vector.y);
				}
			}
		}
	...
	private async void FetchTheScores()
	{
		...
		for (int i = 0; i < array2.Length; i++)
		{
			...
			tRank.text = leaderboardEntry2.GlobalRank.ToString();
			GameObject[] array3 = templateHighlight;
			if (array3 != null && array3.Length > 0)
			{
				array3 = templateHighlight;
				foreach (GameObject gameObject2 in array3)
				{
					if (!(gameObject2 == null))
					{
						...
						gameObject2.SetActive(user.IsMe);
					}
				}
			}
	...
	public void Appear()
	{
		if (i < toAppear.Length)
		{
			...
			else if (toAppear[i] == killsText.gameObject)
			{
				...
				{
					checkedKills = savedKills;
					killsText.GetComponent<AudioSource>().SetPlayOnAwake(playOnAwake: false);
					StartCoroutine(InvokeRealtimeCoroutine(Appear, timeBetween * 2f));
					killsText.text = checkedKills.ToString("0");
				}
				else
			...
			else if (toAppear[i] == waveText.gameObject)
			{
				...
				{
					checkedWaves = savedWaves;
					waveText.GetComponent<AudioSource>().SetPlayOnAwake(playOnAwake: false);
					StartCoroutine(InvokeRealtimeCoroutine(Appear, timeBetween * 2f));
			...
			else if (toAppear[i] == styleText.gameObject)
			{
				if (skipping)
				{
					checkedStyle = savedStyle;
					styleText.text = checkedStyle.ToString("0");
					styleText.GetComponent<AudioSource>().SetPlayOnAwake(playOnAwake: false);
					StartCoroutine(InvokeRealtimeCoroutine(Appear, timeBetween * 2f));
					totalPoints += savedStyle;
					PointsShow();
				}
