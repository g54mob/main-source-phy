...
public class Ferryman : EnemyScript, IHitTargetCallback
{
	...
	private void SlowUpdate()
	{
		...
		else
		{
			...
			else if (path.status != NavMeshPathStatus.PathComplete && path.corners != null && path.corners.Length != 0 && (!eid.target.isOnGround || Vector3.Distance(path.corners[path.corners.Length - 1], PredictPlayerPos(vertical: true)) > 5f))
			{
				flag = true;
			}
	...
	private void Update()
	{
		...
		bool flag = (bool)nma && nma.isOnNavMesh && nma.velocity.magnitude > 2f && gce.onGround;
	...
	private void PhaseChangeUpdate()
	{
		...
		else if (nma.pathStatus == NavMeshPathStatus.PathComplete || jumping)
		{
			nma.enabled = true;
			anim.SetBool("Running", value: true);
		}
