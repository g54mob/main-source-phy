...
public class GamepadSelectionOutline : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (currentSelectedGameObject == null || !currentSelectedGameObject.activeInHierarchy || !currentSelectedGameObject.TryGetComponent<Selectable>(out var component) || MonoSingleton<InputManager>.Instance == null || !(MonoSingleton<InputManager>.Instance.LastButtonDevice is Gamepad) || canvas.renderMode != RenderMode.ScreenSpaceOverlay)
		{
			image.enabled = false;
			return;
		}
		if (component.TryGetComponent<ControllerDisallowedSelection>(out var component2))
		{
		...
		{
			EnsureVisibility(componentInParent, component3, componentInParent != lastScrollRect);
		}
