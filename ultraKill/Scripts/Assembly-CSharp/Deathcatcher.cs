...
public class Deathcatcher : MonoBehaviour
{
	...
	public void EnemyDeath(EnemyIdentifier eid)
	{
		...
		else if (canRespawnIdols || eid.enemyType != EnemyType.Idol)
		{
			...
			{
				CaughtEnemy item = new CaughtEnemy(eid, value);
				deadCaughtEnemies.Add(item);
			}
		}
	}
	...
	private void OnGUI()
	{
		if (DeathCatcherDebug.Active)
		{
			...
			GUILayout.Label($"Deathcatcher Caught Enemies: {deadCaughtEnemies.Count}", GUILayout.Width(200f));
			GUI.EndGroup();
		}
	}
