...
public class Turret : EnemyScript
{
	...
	private void NavigationUpdate()
	{
		...
		{
			if (!nma.pathPending && (nma.remainingDistance < 1f || nma.path.status == NavMeshPathStatus.PathInvalid))
			{
				wandering = false;
			}
		}
	...
	private bool GetWanderPosition(out Vector3 pos)
	{
		...
		if (NavMesh.SamplePosition(pos, out var hit, 5f, nma.areaMask))
		{
			pos = hit.position;
			return true;
		}
	...
	private void Update()
	{
		...
		else if (!hasVision)
		{
			...
			{
				UnityEngine.Object.Destroy(currentSound.gameObject);
			}
	...
	private void Aiming()
	{
		...
		if (outOfSightTimer >= 1f)
		{
			...
			currentSound = UnityEngine.Object.Instantiate(cancelSound, torso);
			CancelAim();
			return;
		}
	...
	private void StartWindup()
	{
		...
		currentSound = UnityEngine.Object.Instantiate(aimWarningSound, torso);
	}
	...
	private void Kick()
	{
		...
		currentSound = UnityEngine.Object.Instantiate(kickWarningSound, torso);
		UnparryableFlash();
	}
	...
	public void Interrupt()
	{
		if (!mach.limp)
		{
			...
			currentSound = UnityEngine.Object.Instantiate(interruptSound, torso);
		}
	}
	...
	private void FootStep(float targetPitch)
	{
		...
		UnityEngine.Object.Instantiate(footStep, base.transform.position, Quaternion.identity).SetPitch(UnityEngine.Random.Range(targetPitch - 0.1f, targetPitch + 0.1f));
	}

	private void Thunk()
	{
		UnityEngine.Object.Instantiate(thunkSound, base.transform.position, Quaternion.identity);
	}

	private void ExtendBarrel()
	{
		UnityEngine.Object.Instantiate(extendSound, base.transform.position, Quaternion.identity);
	}
