...
public class V2 : EnemyScript, IEnrage, IAlter, IAlterOptions<bool>
{
	...
	private void Update()
	{
		...
		if (sliding)
		{
			...
			else if ((bool)nma && !playerInSight)
			{
				StopSlide();
			}
			else
		...
		if (shootCooldown <= 0f && aiming && (!nma || playerInSight))
		{
			ShootCheck();
		}
		if (slideOnly)
	...
	private void UpdateAnimations()
	{
		...
		anim.SetBool("RunningLeft", anim.transform.rotation.eulerAngles.y > base.transform.rotation.eulerAngles.y);
	...
	private void FixedUpdate()
	{
		...
		{
			playerInSight = !nma || !Physics.Raycast(base.transform.position + Vector3.up, target.position - (base.transform.position + Vector3.up), Vector3.Distance(target.position, base.transform.position + Vector3.up), LayerMaskDefaults.Get(LMD.Environment));
			if (running)
			{
				Move();
			}
