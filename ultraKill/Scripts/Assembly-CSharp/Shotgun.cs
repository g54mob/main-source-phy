...
public class Shotgun : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if ((float)sinceLastCore > 0.5f && flag2 && variation != 1 && gc.activated && !GameStateManager.Instance.PlayerInputLocked && ((variation == 0 && !resettingCores) || (variation == 2 && MonoSingleton<WeaponCharges>.Instance.shoSawCharge >= 1f && gunReady)))
		{
			...
			{
				grenadeVector = Vector3.Normalize(targeter.CurrentTargetAimPosition - cam.transform.position);
			}
		...
		if (variation == 2)
		{
			...
			if (charging)
			{
				...
				else if (sawZone.chainsawNonlethalHits > 40 && !chainsawBroken)
				{
					...
					GameObject obj = Object.Instantiate(chainsawBreakEffect, chainsawBladeRenderer.transform.position, Quaternion.identity);
	...
	private void Shoot()
	{
		...
		if ((bool)targeter.CurrentTarget && targeter.IsAutoAimed)
		{
			direction = targeter.GetAimDirectionFrom(MonoSingleton<CameraController>.Instance.GetDefaultPos());
		}
		...
		else
		{
			...
			if ((bool)targeter.CurrentTarget && targeter.IsAutoAimed)
			{
				...
				gameObject2.transform.LookAt(vector2);
			}
		...
		for (int i = 0; i < array2.Length; i++)
		{
			...
			PortalPhysicsV2.Raycast(position, vector3.normalized, vector3.magnitude, default(LayerMask), out var _, out var portalTraversals3, out endPoint);
	...
	private void ShootSinks()
	{
		...
		if (portalTraversals.Length != 0)
		{
			...
			else if (blocked)
			{
				...
				endPoint = Vector3.Reflect(forward, nativePortalTransform.back);
	...
	private void ShootSaw(bool noSaw)
	{
		...
		if (!noSaw)
		{
			...
			if ((bool)targeter.CurrentTarget && targeter.IsAutoAimed)
			{
				vector = targeter.GetAimDirectionFrom(MonoSingleton<CameraController>.Instance.GetDefaultPos());
			}
			bool blocked = false;
			Transform[] array = shootPoints;
			for (int i = 0; i < array.Length; i++)
