...
public class BloodsplatterManager : MonoSingleton<BloodsplatterManager>
{
	...
	internal delegate int CreateBloodstain_000002BE_0024PostfixBurstDelegate(ref int propIndex, ref int currentBloodCount, ref NativeArray<InstanceProperties> props, in float3 pos, in float3 norm, bool clipToSurface, int parent = 0);

	internal static class CreateBloodstain_000002BE_0024BurstDirectCall
	{
		...
		private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
		{
			if (Pointer == (IntPtr)0)
			{
				Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(CreateBloodstain_000002BE_0024PostfixBurstDelegate).TypeHandle);
			}
			P_0 = Pointer;
		}
		...
		static CreateBloodstain_000002BE_0024BurstDirectCall()
		{
			Constructor();
		}
	...
	{
		return CreateBloodstain_000002BE_0024BurstDirectCall.Invoke(ref propIndex, ref currentBloodCount, ref props, in pos, in norm, clipToSurface, parent);
	}
	...
	private void Start()
	{
		...
		stainCreateQueue = new NativeQueue<BloodstainCreateCommand>(Allocator.Persistent);
	...
	private void PlayBloodSound(Vector3 position)
	{
		...
		{
			splatterClip.PlayClipAtPoint(goreAudioGroup, position, 256, 1f, 1f, 0.5f, AudioRolloffMode.Logarithmic);
		}
	}
	...
	private void Update()
	{
		...
		float num2 = num / 2f;
		BloodstainCreateCommand item;
		while (stainCreateQueue.TryDequeue(out item))
		{
			if (!continuousGibCollisions && (float)UnityEngine.Random.Range(0, 100) >= (item.halfChance ? num2 : num))
			{
				continue;
			}
			ref RaycastHit hit = ref item.hit;
			...
			Bloodsplatter bloodsplatter = splatters[item.splatterId];
			if (!bloodsplatter.gz)
			{
	...
	private void RebuildMesh()
	{
		...
		generateBloodMeshJobHandle = IJobParallelForExtensions.Schedule(generateBloodMeshJob, currentBloodCount, 256);
	}
