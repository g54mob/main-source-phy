...
public class PowerIntro : MonoBehaviour
{
	...
	{
		AudioClip audioClip = introOverride;
		if (introOverride == null)
		{
			audioClip = MonoSingleton<PowerVoiceController>.Instance.Intro();
		}
		else if (persistentData != null && persistentData.PerformedIntro && persistentData.RepeatedIntroOverrideClip)
		{
			audioClip = ((persistentData.RepeatedIntroClips.Length != 0) ? persistentData.RepeatedIntroClips[Random.Range(0, persistentData.RepeatedIntroClips.Length)] : null);
		}
		...
		if (audioClip != null)
		{
			...
			{
				audioClip.PlayClipAtPoint(MonoSingleton<AudioMixerController>.Instance.allGroup, base.transform.position, 20, 0.8f, 1f, Random.Range(0.9f, 1.1f), AudioRolloffMode.Linear, 1f, 150f);
			}
		}
