...
public class RocketLauncher : MonoBehaviour
{
				...
				if (lastKnownTimerAmount != MonoSingleton<WeaponCharges>.Instance.rocketCannonballCharge && (!wid || wid.delay == 0f))
				{
					for (float num2 = 4f; num2 > 0f; num2 -= 1f)
					{
						if (MonoSingleton<WeaponCharges>.Instance.rocketCannonballCharge >= num2 / 4f && lastKnownTimerAmount < num2 / 4f)
						{
							...
							audioSource.SetPitch(1.6f + num2 * 0.1f);
		...
		else if (variation == 0)
		{
			...
			if (lastKnownTimerAmount != MonoSingleton<WeaponCharges>.Instance.rocketFreezeTime && (!wid || wid.delay == 0f))
			{
				for (float num3 = 4f; num3 > 0f; num3 -= 1f)
				{
					...
					{
						Object.Instantiate(timerWindupSound).SetPitch(0.6f + num3 * 0.1f);
						break;
					}
				}
	...
	public void Shoot()
	{
		...
		{
			gameObject.transform.LookAt(MonoSingleton<CameraFrustumTargeter>.Instance.CurrentTargetAimPosition);
		}
	...
	public void ShootCannonball()
	{
		...
		if ((bool)MonoSingleton<CameraFrustumTargeter>.Instance.CurrentTarget && MonoSingleton<CameraFrustumTargeter>.Instance.IsAutoAimed)
		{
			...
			{
				vector3 = matrix4x.Value.MultiplyPoint3x4(vector3);
			}
			rigidbody.transform.LookAt(vector3);
		}
		rigidbody.velocity = (1f - cbCharge) * MonoSingleton<NewMovement>.Instance.rb.velocity + rigidbody.transform.forward * Mathf.Max(15f, cbCharge * 150f);
	...
	public void ShootNapalm()
	{
		...
		if ((bool)MonoSingleton<CameraFrustumTargeter>.Instance.CurrentTarget && MonoSingleton<CameraFrustumTargeter>.Instance.IsAutoAimed)
		{
			...
			if (matrix4x.HasValue)
			{
				vector2 = matrix4x.Value.MultiplyPoint3x4(vector2);
			}
			rigidbody.transform.LookAt(vector2);
		}
