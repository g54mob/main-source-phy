...
public class Enemy : MonoBehaviour, IAlter, IAlterOptions<float>, ITarget, IPortalTraveller
{
	...
	protected void Awake()
	{
		...
		if (!IsStatue() || !limp)
		{
			...
			deathTokenSource = CancellationTokenSource.CreateLinkedTokenSource(base.destroyCancellationToken);
	...
	protected virtual void Update()
	{
		...
		if (isMassDieing)
		{
			base.transform.position = new Vector3(origPos.x + UnityEngine.Random.Range(-0.5f, 0.5f), origPos.y + UnityEngine.Random.Range(-0.5f, 0.5f), origPos.z + UnityEngine.Random.Range(-0.5f, 0.5f));
			if (transforms.Count != 0 && UnityEngine.Random.Range(0f, 1f) < Time.deltaTime * 5f)
			{
	...
	{
		if (!limp && (bool)eid && !eid.dead && !(nma == null) && !isSpider && !isDrone && !isOnOffNavmeshLink)
		{
	...
	private void UpdateGroundedState(bool hasGc, bool hasNma, bool hasAnim)
	{
		...
		if (!knockedBack)
		{
			if (!grounded && gc.onGround && nma != null && IsZombie())
			{
		...
		if (hasAnim && grounded && nma.enabled && variableSpeed && nma.isOnNavMesh)
		{
	...
	private void UpdateFalling()
	{
		...
		if (IsMachine())
		{
			flag = falling && rb != null && !overrideFalling && (!nma || !nma.isOnOffMeshLink);
		}
		if (IsZombie())
		{
			flag = falling && !limp;
		...
		if (flag || overrideFalling)
		{
			...
			if ((bool)gc)
			{
				...
				if (IsSpider())
				{
					flag2 = (bool)gc && gc.onGround && nma != null;
				}
				if (flag2)
				{
					HandleLanding();
	...
	private void UpdateFallingState(bool hasGc, bool hasNma, bool hasRb, bool hasAnim)
	{
		...
		if (!falling && !gc.onGround && (!nma.enabled || !nma.isOnOffMeshLink))
		{
			StartFalling();
		}
	...
	{
		fallSpeed = 0f;
		if (aud != null && aud.clip == scream && aud.isPlaying)
		{
			aud.Stop();
		}
	...
	private void HandleFallingSound(float speedDiff)
	{
		...
		else if (fallTime > 0.05f && speedDiff < 0f)
		{
			...
			if ((flag3 || !(fallSpeed > GetSplatVelocity() || flag2)) && !aud.isPlaying && !limp)
			{
				...
				aud.volume = 1f;
				aud.priority = 78;
				aud.SetPitch(UnityEngine.Random.Range(0.8f, 1.2f));
	...
	private void HandleLanding()
	{
		...
		if (IsZombie())
		{
			flag = NavMesh.SamplePosition(base.transform.position, out var hit, 4f, nma.areaMask);
			if (flag)
			{
	...
	public void BloodExplosion()
	{
		...
		{
			if (transform != null && UnityEngine.Random.Range(0f, 1f) < 0.33f)
			{
				GameObject gore = bsm.GetGore(GoreType.Head, eid);
				if ((bool)gore)
				{
			...
			for (int i = 0; i < 40; i++)
			{
				...
				if (i < 30)
				{
					...
					if ((bool)gib)
					{
						...
						{
							ReadyGib(gib, transforms[UnityEngine.Random.Range(0, transforms.Count)].gameObject);
						}
						gib.transform.localScale *= UnityEngine.Random.Range(4f, 7f);
					}
					else
					{
						i = 30;
				...
				if (i < 35)
				{
					...
					if ((bool)gib)
					{
						...
						gib.transform.localScale *= UnityEngine.Random.Range(3f, 6f);
					}
					else
					{
						i = 35;
				...
				gib.transform.localScale *= UnityEngine.Random.Range(3f, 4f);
			}
		}
		base.enabled = false;
		isMassDieing = false;
	...
	public void SetDestination(Vector3 position)
	{
		...
		{
			if (NavMesh.SamplePosition(position, out var hit, 1f, nma.areaMask))
			{
				position = hit.position;
			}
	...
	public void StopKnockBack()
	{
		...
		else
		{
			...
			else if (NavMesh.SamplePosition(hitInfo.point, out hit, IsZombie() ? 8 : 4, nma.areaMask))
			{
				knockedBack = false;
				nma.updatePosition = true;
				nma.updateRotation = true;
	...
	{
		if (aud == null || hurtSounds.Length == 0)
		{
			return;
		}
		bool flag = false;
	...
	{
		tempGib.transform.SetPositionAndRotation(target.transform.position, UnityEngine.Random.rotation);
			...
			foreach (Rigidbody rigidbody in array)
			{
				if ((bool)rigidbody)
				{
					...
					if (IsStatue())
					{
						...
						rigidbody.AddForce(UnityEngine.Random.onUnitSphere * 2.5f, ForceMode.VelocityChange);
					}
					else if (man != null)
					{
						rigidbody.AddForce((rigidbody.position - eid.overrideCenter.transform.position).normalized * UnityEngine.Random.Range(20f, 30f), ForceMode.VelocityChange);
						rigidbody.AddTorque(UnityEngine.Random.onUnitSphere * 360f, ForceMode.VelocityChange);
	...
	public bool? OnTravel(PortalTravelDetails details)
	{
		...
		else
		{
			...
			else if (details.isLinkTraversal)
			{
				...
				if (NavMesh.SamplePosition(vector2, out var hit, 0.5f, nma.areaMask))
				{
			...
			else if (details.isIntersectTraversal)
			{
				...
				if ((bool)nma)
				{
					if (NavMesh.SamplePosition(vector2, out var hit2, 1f, nma.areaMask))
					{
		...
		if (isTraversingPortalLink)
		{
			...
			if (nma.isOnNavMesh && nma.CalculatePath(portalLinkData.startNavDestination, path))
			{
				nma.path = path;
			}
		...
		bool SampleEndPos(Vector3 vector10, out Vector3 reference)
		{
			...
			if (NavMesh.SamplePosition(reference, out var hit3, 2f, nma.areaMask))
			{
				reference = hit3.position;
				return true;
			}
			return false;
		}
	...
	private void HandlePortalLinkTraversal(bool hasNma)
	{
		...
		if (!isTraversingPortalLink)
		{
			if (nma.currentOffMeshLinkData.valid)
			{
			...
			else if (!nma.currentOffMeshLinkData.valid && nma.isOnOffMeshLink)
			{
				nma.ResetPath();
				nma.Warp(base.transform.position);
			}
		...
		{
			Vector3 startPos = portalLinkData.data.startPos;
			...
			portalLinkData.linkEndPos = portalLinkData.data.endPos;
			...
			portalLinkData.startPortalTargetEnterLocalPos = nativePortalTransform.WorldToLocal((float3)portalLinkData.portalTargetPos);
