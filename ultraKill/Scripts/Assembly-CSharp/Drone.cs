...
public class Drone : EnemyScript
{
	...
	private void Start()
	{
		...
		if (!chargeParticle)
		{
			...
			{
				if (!componentsInChildren[i].TryGetComponent<SpawnEffect>(out var _))
				{
	...
	public override void OnTeleport(PortalTravelDetails details)
	{
		...
		{
			Vector3 vector = nativePortal.transform.back;
	...
	private void Update()
	{
		...
		if (ShouldProcessAttack())
		{
			...
			else if (projectile != null && (!vc || (eid.enemyType == EnemyType.Virtue && vc.virtueCooldown == 0f) || (eid.enemyType == EnemyType.Providence && vc.providenceCooldown == 0f)))
			{
				ProcessAttack();
			}
		}
	...
	private void ProcessAttack()
	{
		if ((bool)vc)
		{
			...
			{
				vc.providenceCooldown = 1f / eid.totalSpeedModifier;
			}
		}
	...
	public void Dodge(Vector3 direction)
	{
		...
		if ((bool)dodgeSound && !hasDimensionalTarget)
		{
			dodgeSound.PlayClipAtPoint(MonoSingleton<AudioMixerController>.Instance.allGroup, base.transform.position, 128, 0f, 0.15f, UnityEngine.Random.Range(0.75f, 1.25f));
		}
	...
	public void Explode()
	{
		...
		if (spawnOnDeath != null && spawnOnDeath.Length != 0 && !eid.puppet)
		{
			...
			foreach (AssetReference assetReference in array)
			{
				...
				{
					UnityEngine.Object.Instantiate(assetReference.ToAsset(), base.transform.position, base.transform.rotation, gz.transform);
				}
			}
		}
