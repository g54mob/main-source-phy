...
public class GamepadObjectSelector : MonoBehaviour
{
	...
	{
		if (s_Selectors.Count > 0 && s_Selectors.Peek() == this && EventSystem.current != null && MonoSingleton<InputManager>.Instance?.LastButtonDevice is Gamepad)
		{
	...
	private GameObject SelectFirstChild(GameObject obj)
	{
		...
		{
			if (!(selectable.gameObject == obj) && selectable.isActiveAndEnabled && selectable.navigation.mode != Navigation.Mode.None && (allowNonInteractable || selectable.interactable))
			{
