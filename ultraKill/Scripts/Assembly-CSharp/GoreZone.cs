...
public class GoreZone : MonoBehaviour
{
	...
	public Transform deletedLimbZone;
	...
	private void Awake()
	{
		...
		if (deletedLimbZone == null)
		{
			GameObject gameObject3 = new GameObject("Deleted Limb Zone");
			deletedLimbZone = gameObject3.transform;
			deletedLimbZone.SetParent(base.transform, worldPositionStays: true);
		}
	...
	private void SlowUpdate()
	{
		...
		for (int num2 = Mathf.FloorToInt(gibZone.childCount - maxGibs) - 1; num2 >= 0; num2--)
		{
			...
			{
				if (gameObject2.layer == 10 || gameObject2.layer == 11)
				{
					DisableLimb(gameObject2);
				}
				else
				{
	...
	private void DisableLimb(GameObject limb)
	{
		limb.SetActive(value: false);
		limb.transform.localScale = Vector3.zero;
		limb.transform.SetParent(deletedLimbZone);
	}
	...
	public void ResetGibs()
	{
		for (int num = gibZone.childCount - 1; num > 0; num--)
		{
			...
			else if (child.gameObject.layer == 10 || child.gameObject.layer == 11)
			{
				DisableLimb(child.gameObject);
			}
			else
			{
				UnityEngine.Object.Destroy(child.gameObject);
