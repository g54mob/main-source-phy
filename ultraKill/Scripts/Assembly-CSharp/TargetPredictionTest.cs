...
public class TargetPredictionTest : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (vision.TrySee(query, out var data))
		{
			...
			if (showGravity)
			{
				previewGravityPosition.position = data.PredictTargetPosition(time, aimAtHead: false, includeGravity: true);
			}
			if (showGroundMovement)
			{
				previewAssumeGroundMovementPosition.position = data.PredictTargetPosition(time, aimAtHead: false, includeGravity: false, assumeGroundMovement: true);
			}
			if (showGravityAndGroundMovement)
			{
				previewGravityAndGroundMovementPosition.position = data.PredictTargetPosition(time, aimAtHead: false, includeGravity: true, assumeGroundMovement: true);
			}
		}
		else
		{
