...
public class HeatResistance : MonoSingleton<HeatResistance>
{
	...
	private void Update()
	{
		...
		if (recharging || greenFlash.color.a != 0f)
		{
			greenFlash.color = new Color(greenFlash.color.r, greenFlash.color.g, greenFlash.color.b, recharging ? 1f : Mathf.MoveTowards(greenFlash.color.a, 0f, Time.deltaTime));
		}
		meterLabel.color = ((heatResistance > 0f) ? Color.white : Color.Lerp(Color.red, Color.white, (float)hurtTimer % 0.5f));
		...
		if (heatResistance == 0f)
		{
			screenShatter.color = new Color(1f, 1f, 1f, 0.5f);
		}
