...
public class GroundWave : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (!isTraversingLink && (bool)nma && nma.enabled && nma.isOnOffMeshLink)
		{
		...
		if ((bool)faceDirection)
		{
			...
			else if ((bool)nma && nma.enabled && nma.desiredVelocity.sqrMagnitude > 0.01f)
			{
	...
	public void TrackTick()
	{
		...
		if (!isTraversingLink && (bool)nma && nma.enabled)
		{
			if ((float)sinceParried < 1f)
			{
