...
public class Zapper : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		if (distance > maxDistance || raycastBlocked)
		{
			...
			foreach (AudioSource audioSource in array2)
			{
				...
				audioSource.SetPitch((!raycastBlocked) ? 1 : 2);
			}
	...
	private void RecalculatePortalIntersections(bool allowMigration = true)
	{
		...
		for (int i = 0; i < portalTraversals.Count; i++)
		{
			...
			Vector3 vector4 = nativePortalTransform.center;
			Vector3 vector5 = nativePortalTransform.forward;
	...
	public void Break(bool successful = false)
	{
		if (!broken)
		{
			...
			{
				UnityEngine.Object.Instantiate(cableSnap, base.transform.position, Quaternion.identity);
			}
			if ((bool)attachedEnemy)
			{
				attachedEnemy.zapperer = this;
	...
	public void ChargeBoost(float amount)
	{
		...
		UnityEngine.Object.Instantiate(boostSound, base.transform.position, Quaternion.identity);
