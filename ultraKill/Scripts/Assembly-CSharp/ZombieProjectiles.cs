...
public class ZombieProjectiles : EnemyScript
{
	...
	private GameObject playerTargetGO;
	...
	private void SetValues()
	{
		if (!valuesSet)
		{
			...
			playerTargetGO = MonoSingleton<PlayerTracker>.Instance.GetTarget().gameObject;
			if ((bool)aimer)
			{
	...
	private void SlowUpdate()
	{
		...
		{
			if ((bool)nma && (hasVision || hasDimensionalTarget) && afraid)
			{
				Flee();
			}
	...
	private void Flee()
	{
		if (inFleeRange && nma.enabled && nma.isOnNavMesh && !nma.pathPending)
		{
			...
			if (NavMesh.SamplePosition(vector2, out var hit, 1f, nma.areaMask))
			{
				vector2 = hit.position;
			}
			else if (NavMesh.FindClosestEdge(vector2, out hit, nma.areaMask))
			{
				vector2 = hit.position;
			}
	...
	{
		if (stationary || zmb.isTraversingPortalLink || !nma || !nma.enabled || !nma.isOnNavMesh)
		{
			return;
		}
		if (shouldSortie)
		...
		else
		{
			...
			if (hasVision || hasDimensionalTarget)
			{
				if (wanderer)
				{
					...
					if ((bool)nma && nma.velocity.magnitude < 1f)
					{
						Wander();
						return;
					}
			...
			if (NavMesh.FindClosestEdge(eid.target.position, out var hit, nma.areaMask))
			{
	...
	private void Update()
	{
		...
		if (nma.enabled && (nma.velocity.magnitude > 2.5f || zmb.isTraversingPortalLink))
		{
	...
	public void Swing()
	{
		...
		{
			anim.SetFloat("AttackType", (UnityEngine.Random.Range(0f, 1f) > 0.66f) ? 1 : 0);
		}
	...
	public void ThrowProjectile()
	{
		...
		{
			TargetData data = vision.CalculateData(targetHandle);
			flag = data.target.isPlayer;
			if (flag)
			{
				if (difficulty >= 4)
				{
					float time = Vector3.Distance(vector, data.position) / (float)((difficulty == 5) ? 90 : UnityEngine.Random.Range(110, 180));
					vector2 = data.PredictTargetPosition(time, aimAtHead: true);
				}
				else
				{
					vector2 = data.headPosition;
				}
			}
			else
			{
				vector2 = data.position;
			}
		}
		else if (eid.target != null)
		{
			...
			if (flag)
			{
				if (difficulty >= 4)
				{
					float time2 = Vector3.Distance(vector, playerTargetGO.transform.position) / (float)((difficulty == 5) ? 90 : UnityEngine.Random.Range(110, 180));
					vector2 = MonoSingleton<PlayerTracker>.Instance.PredictPlayerPosition(time2, aimAtHead: true);
				}
				else
				{
					vector2 = eid.target.headPosition;
				}
			}
			else
			{
				EnemyIdentifierIdentifier componentInChildren = eid.target.targetTransform.GetComponentInChildren<EnemyIdentifierIdentifier>();
				vector2 = (componentInChildren ? componentInChildren.transform.position : eid.target.position);
			}
		}
		else
		{
		...
		ProjectileSpread componentInChildren2 = currentProjectile.GetComponentInChildren<ProjectileSpread>();
		if ((bool)componentInChildren2)
		{
			...
			if (difficulty <= 2)
			{
				if (difficulty == 2)
				{
					componentInChildren2.spreadAmount = 5f;
				}
				...
				{
					componentInChildren2.spreadAmount = 2f;
				}
				componentInChildren2.projectileAmount = 3;
			}
		}
		Projectile componentInChildren3 = currentProjectile.GetComponentInChildren<Projectile>();
		if ((bool)componentInChildren3)
		{
			componentInChildren3.targetHandle = targetHandle;
			componentInChildren3.isTargetPlayer = flag;
			componentInChildren3.safeEnemyType = EnemyType.Stray;
			if (difficulty > 2)
			{
				componentInChildren3.speed *= 1.35f;
			}
			else if (difficulty == 1)
			{
				componentInChildren3.speed *= 0.75f;
			}
			else if (difficulty == 0)
			{
				componentInChildren3.speed *= 0.5f;
			}
			componentInChildren3.damage *= eid.totalDamageModifier;
			if (flag2)
			{
				componentInChildren3.Explode();
			}
		}
	}
	...
	private void Wander()
	{
		if (!hasVision || !nma || !nma.enabled || !nma.isOnNavMesh)
		{
			return;
		}
		...
		if ((Physics.Raycast(wanderTarget, Vector3.down, out var hitInfo2, 15f, lm) && NavMesh.SamplePosition(hitInfo2.point, out var hit, 0.5f, 1)) || NavMesh.SamplePosition(wanderTarget, out hit, 15f, 1))
		{
			isWandering = true;
