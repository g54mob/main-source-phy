...
public class Wet : MonoBehaviour
{
	...
	public void Dry(Vector3 position)
	{
		...
		if (!wetParticle)
		{
			...
			wetParticle.transform.SetParent(base.transform, worldPositionStays: true);
			wetParticle.transform.localPosition = position;
		}
		wetParticle.Play();
	}
	...
	private void ReturnWetParticle()
	{
		if ((bool)wetParticle)
		{
			MonoSingleton<PooledWaterStore>.Instance.ReturnToQueue(wetParticle.gameObject, Water.WaterGOType.wetparticle);
			wetParticle = null;
		}
	}
}
