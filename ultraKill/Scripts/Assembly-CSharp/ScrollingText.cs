			...
			while ((float)textTimer >= secondsBetweenLetters && currentLetter < message.Length)
			{
				...
				if (currentLetter < message.Length)
				{
					if (fillMissingText)
					{
						if (writingCursor)
						{
							text.text = text.text + "<mark=#" + ColorUtility.ToHtmlStringRGB(text.color) + ">" + message[currentLetter] + "</mark><alpha=#00>" + message.Substring(currentLetter + 1);
						}
						else
						{
				...
				if ((bool)clickAudio && message[currentLetter - 1] != '\n' && message[currentLetter - 1] != ' ')
				{
