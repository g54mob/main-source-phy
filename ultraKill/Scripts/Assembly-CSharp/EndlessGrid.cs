...
public class EndlessGrid : MonoSingleton<EndlessGrid>
{
	...
	{
		float num = (float)anw.deadEnemies / (float)(enemyAmount - totalDeathcatchers);
		...
		for (int num4 = endlessEvents.Count - 1; num4 >= 0; num4--)
		{
			...
			else if (num >= endlessEvents[num4].waveProgressToActivate)
			{
				...
				if (specialAntiBuffer <= 0 && num2 > 0)
				{
					...
					if (num7 > 0 && meleePositions.Count > 0)
					{
						...
						for (int k = 0; k < num7; k++)
						{
							...
							{
								if (currentWave >= endlessEnemy.spawnWave && (float)points - (float)maxPoints * 0.1f >= (float)endlessEnemy.spawnCost + array[num8])
								{
							...
							if ((float)points < (float)maxPoints * 0.1f)
							{
								points = Mathf.RoundToInt((float)maxPoints * 0.1f);
							}
						}
					}
				}
