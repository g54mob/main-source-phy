	...
	public void CheckItem(bool prelim = false)
	{
		...
		if (componentInChildren != null)
		{
			if (componentInChildren.itemType == acceptedItemType)
			{
				...
				{
					if ((bool)elementChangeEffect)
					{
						elementChangeEffect.Play();
					}
					if ((bool)soundOnActivated)
					{
						Object.Instantiate(soundOnActivated, base.transform.position, Quaternion.identity);
					}
				}
		...
		if (!prelim)
		{
			...
			if (!hideEffects)
			{
				...
				if ((bool)soundOnDeactivated)
				{
					Object.Instantiate(soundOnDeactivated, base.transform.position, Quaternion.identity);
				}
			}
			ArenaStatus[] array4 = arenaStatuses;
			for (int i = 0; i < array4.Length; i++)
