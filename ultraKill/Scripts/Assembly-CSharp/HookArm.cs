...
public class HookArm : MonoSingleton<HookArm>
{
	...
	private void OnPortalTraversed(PortalTravelDetails details)
	{
		...
		for (int i = 0; i < details.portalSequence.Count; i++)
		{
			...
			Vector3 exit = travelMatrix.MultiplyPoint3x4(center);
			float3 forward = nativePortalTransform.forward;
			Vector3 exitDir = travelMatrix.MultiplyVector(forward);
			PortalTraversalV2 item = new PortalTraversalV2(center, forward, exit, exitDir, handle, portalObject);
	...
	private void RecalculateThrowingIntersections(bool allowMigration = true)
	{
		...
		for (int num = portalTraversals.Count - 1; num >= 0; num--)
		{
			...
			Vector3 vector3 = nativePortalTransform.center;
			Vector3 vector4 = nativePortalTransform.forward;
	...
	private void Update()
	{
		...
		if (MonoSingleton<InputManager>.Instance.InputSource.Hook.WasPerformedThisFrame)
		{
			...
			else if (cooldown <= 0f)
			{
				...
				{
					throwDirection = (targeter.CurrentTargetAimPosition - base.transform.position).normalized;
				}
				else
				{
		...
		else if (state == HookState.Caught)
		{
			...
			if (portalTraversals.Count > 0)
			{
				...
				for (int num9 = portalTraversals.Count - 1; num9 >= 0; num9--)
				{
					...
					Vector3 a = portalObject.GetTransform(portalTraversalV3.portalHandle.side).center;
					Vector3 b = portalObject.GetTransform(portalTraversalV3.portalHandle.side.Reverse()).center;
		...
		if (state == HookState.Pulling)
		{
			...
			if (portalTraversals.Count > 0)
			{
				...
				for (int num12 = portalTraversals.Count - 1; num12 >= 0; num12--)
				{
					...
					Vector3 a2 = portalObject2.GetTransform(portalTraversalV4.portalHandle.side).center;
					Vector3 b2 = portalObject2.GetTransform(portalTraversalV4.portalHandle.side.Reverse()).center;
			...
			if (!MonoSingleton<NewMovement>.Instance.boost || MonoSingleton<NewMovement>.Instance.sliding)
			{
				...
				if (list5 != null && list5.Count > 0)
				{
					...
					Vector3 vector12 = nativePortalTransform.right;
					Vector3 vector13 = nativePortalTransform.up;
