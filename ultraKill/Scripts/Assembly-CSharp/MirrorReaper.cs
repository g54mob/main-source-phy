...
public class MirrorReaper : EnemyScript
{
	...
	public void TrackTick()
	{
		...
		if (teleportedToEscape || stationary)
		{
			...
			else
			{
				...
				NavMesh.CalculatePath(base.transform.position, target.GetNavPoint(), nma.areaMask, navMeshPath);
				if (navMeshPath.corners.Length > 1)
				{
					nma.enabled = false;
					base.transform.LookAt(new Vector3(navMeshPath.corners[1].x, base.transform.position.y, navMeshPath.corners[1].z));
					return;
				}
				hiddenVisibilityTimer = Mathf.MoveTowards(hiddenVisibilityTimer, 1f, updateRate);
			}
	...
	private void TeleportToEscapePoint()
	{
		...
		if (!NavMesh.SamplePosition(vector, out var hit, 1f, nma.areaMask))
		{
		...
		{
			if (!NavMesh.SamplePosition(escapePoints[i].position, out var hit2, 1f, nma.areaMask))
			{
				Debug.Log("Position " + i + " not close to a NavMesh");
			}
			else if (!NavMesh.CalculatePath(hit2.position, vector, nma.areaMask, navMeshPath))
			{
				Debug.Log("Position " + i + " invalid");
			}
			else if (navMeshPath.status == NavMeshPathStatus.PathPartial && Vector3.Distance(vector, navMeshPath.corners[navMeshPath.corners.Length - 1]) > 10f)
			{
			...
			else
			{
				...
				case 0:
				{
					float distance = navMeshPath.GetDistance();
					if (distance > num && navMeshPath.corners.Length < 2)
					{
				...
				case 2:
				{
					float num4 = navMeshPath.GetDistance() + (float)(navMeshPath.corners.Length * 10);
					if (num4 > num)
					{
						transform = escapePoints[i];
						num = num4;
	...
	{
		anim.SetFloat("ProjectileBarrageSpeed", (dontSpamProjectiles || !((float)sinceOverGround > 5f)) ? 1 : 2);
	...
	{
		if (NavMesh.SamplePosition(EnemyTarget.GetNavPoint(target.position), out var hit, 5f, nma.areaMask))
		{
	...
	private bool IsTargetOverGround(bool getNewValue = false)
	{
		...
		if (getNewValue)
		{
			isTargetOverGround = PortalPhysicsV2.Raycast(target.position + Vector3.up, base.transform.up * -1f, out var hitInfo, 30f, LayerMaskDefaults.Get(LMD.Environment)) && NavMesh.SamplePosition(hitInfo.point, out var _, 1f, nma.areaMask);
		}
	...
	{
		if (!nma || !nma.enabled || !nma.isOnNavMesh)
		{
			return false;
		}
		NavMeshPath navMeshPath = new NavMeshPath();
		nma.CalculatePath(point, navMeshPath);
	...
	private void PlaySound(AudioClip clip, float pitchRange = 0.1f)
	{
		if (!(clip == null))
		{
			clip.PlayClipAtPoint(MonoSingleton<AudioMixerController>.Instance.allGroup, base.transform.position, 25, 1f, 1f, Random.Range(1f - pitchRange, 1f + pitchRange), AudioRolloffMode.Linear, 1f, 50f).transform.SetParent(base.transform, worldPositionStays: true);
		}
	}
