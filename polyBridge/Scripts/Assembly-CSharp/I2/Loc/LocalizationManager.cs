...
namespace I2.Loc
{
	public static class LocalizationManager
	{
		...
		private static void SelectStartupLanguage()
		{
			...
			if (!string.IsNullOrEmpty(empty) && HasLanguage(empty, AllowDiscartingRegion: true, Initialize: false))
			{
		...
		public static LanguageData GetLanguageDataFromCode(string code)
		{
			int i = 0;
			for (int count = Sources.Count; i < count; i++)
			{
				...
				{
					if (Sources[i].mLanguages[j].Code == code)
					{
						return Sources[i].mLanguages[j];
					}
				}
			}
			return null;
		...
		public static List<string> GetAllLanguagesCode(bool allowRegions = true, bool SkipDisabled = true)
		{
			...
			for (int count = Sources.Count; i < count; i++)
			{
				for (int j = 0; j < Sources[i].mLanguages.Count; j++)
				{
					string code = Sources[i].mLanguages[j].Code;
					if (!string.IsNullOrEmpty(code) && Sources[i].mLanguages[j].IsLoaded())
					{
						list.Add(code);
					}
				}
