	...
	public class SplineRenderer : MeshGenerator
	{
		...
		public void GenerateVertices(Vector3 vertexDirection, bool orthoGraphic)
		{
			...
			for (int i = 0; i < base.clippedSamples.Length; i++)
			{
				...
				{
					position += base.offset.x * -Vector3.Cross(base.clippedSamples[i].direction, base.clippedSamples[i].normal) + base.offset.y * base.clippedSamples[i].normal + base.offset.z * base.clippedSamples[i].direction;
				}
				Vector3 vector = ((!orthoGraphic) ? (vertexDirection - position).normalized : vertexDirection);
				Vector3 normalized = Vector3.Cross(base.clippedSamples[i].direction, vector).normalized;
				...
				for (int j = 0; j < _slices + 1; j++)
				{
					...
					tsMesh.vertices[num] = position - normalized * base.clippedSamples[i].size * 0.5f * base.size + normalized * base.clippedSamples[i].size * num2 * base.size;
