...
namespace Dreamteck.Splines
{
	...
	public class Spline
	{
		...
		[Range(0f, 0.9999f)]
		public double precision = 0.8999999761581421;
		...
		public void Evaluate(SplineResult result, double percent)
		{
			...
			else
			{
				...
				if (Mathf.Max(Mathf.Max(result.direction.x, result.direction.y), result.direction.z) <= 0.009f)
				{
					double percent2 = DMath.Clamp01(percent - num6);
					result.direction = result.position - EvaluatePosition(percent2);
				}
			}
			if (type == Type.Bezier && percent <= num6 && points[0].tangent2 != points[0].position)
			{
				result.direction = points[0].tangent2 - points[0].position;
			}
			else if (type == Type.Bezier && percent >= 1.0 - num6 && points[points.Length - 1].tangent2 != points[points.Length - 1].position)
			{
				result.direction = points[points.Length - 1].position - points[points.Length - 1].tangent;
			}
		...
		public double Travel(double start, float distance, Direction direction)
		{
			...
			int num3 = ((direction == Direction.Forward) ? DMath.CeilInt(start * (double)num2) : DMath.FloorInt(start * (double)num2));
			float num4 = 0f;
			double num5 = start;
			while (true)
			{
		...
		public void ConvertToBezier()
		{
			...
			case Type.Hermite:
			{
				for (int i = 0; i < points.Length; i++)
				{
					...
					if (i == 0)
					{
						Vector3 vector = hermitePoints[1] - hermitePoints[2];
						if (closed)
						{
					...
					{
						Vector3 vector2 = hermitePoints[2] - hermitePoints[3];
						points[i].SetTangentPosition(points[i].position + vector2 / 3f);
					}
					else
					{
						Vector3 vector3 = hermitePoints[0] - hermitePoints[2];
