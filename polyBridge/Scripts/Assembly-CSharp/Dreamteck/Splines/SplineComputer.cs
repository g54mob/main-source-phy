	...
	public class SplineComputer : MonoBehaviour
	{
		...
		public class Morph
		{
			...
			private void Update()
			{
				...
				{
					Vector3 position = morphStates[0].points[i].position;
					Vector3 tangent = morphStates[0].points[i].tangent;
					Vector3 tangent2 = morphStates[0].points[i].tangent2;
					...
					{
						position += (morphStates[j].points[i].position - morphStates[0].points[i].position) * morphStates[j].percent;
						tangent += (morphStates[j].points[i].tangent - morphStates[0].points[i].tangent) * morphStates[j].percent;
						tangent2 += (morphStates[j].points[i].tangent2 - morphStates[0].points[i].tangent2) * morphStates[j].percent;
						normal += (morphStates[j].points[i].normal - morphStates[0].points[i].normal) * morphStates[j].percent;
		...
		{
			return DMath.Clamp01((double)pointIndex / (double)(pointCount - 1));
		}
		...
		public double Travel(double start, float distance, Spline.Direction direction)
		{
			...
			int num3 = ((direction == Spline.Direction.Forward) ? DMath.CeilInt(start * (double)num2) : DMath.FloorInt(start * (double)num2));
			float num4 = 0f;
			_ = Vector3.zero;
			double num5 = start;
			while (true)
			{
				...
				Vector3 vector = EvaluatePosition(num5);
