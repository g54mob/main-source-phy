...
namespace Dreamteck.Splines
{
	...
	public class SplineFollower : SplineTracer
	{
		...
		{
			if (!followStarted && autoStartPosition)
			{
				SetPercent(Project(GetTransform().position).percent);
			}
			followStarted = true;
		...
		public void Move(float distance)
		{
			...
			while (num < distance)
			{
				...
				if (_direction == Spline.Direction.Forward)
				{
					...
					{
						if (this.onEndReached != null && lastClippedPercent < 1.0)
						{
							flag = true;
						}
						...
						{
							CheckTriggersClipped(from.percent, base.clippedSamples[base.clippedSamples.Length - 1].percent);
							from.CopyFrom(base.clippedSamples[0]);
							num2 = 1;
						}
						else if (wrapMode == Wrap.PingPong)
						{
							_direction = Spline.Direction.Backward;
							CheckTriggersClipped(from.percent, base.clippedSamples[base.clippedSamples.Length - 1].percent);
							from.CopyFrom(base.clippedSamples[base.clippedSamples.Length - 1]);
				...
				else
				{
					...
					{
						if (this.onBeginningReached != null && lastClippedPercent > 0.0)
						{
							flag2 = true;
						}
						if (wrapMode == Wrap.Default)
						...
						if (wrapMode == Wrap.Loop)
						{
							CheckTriggersClipped(from.percent, base.clippedSamples[0].percent);
							from.CopyFrom(base.clippedSamples[base.clippedSamples.Length - 1]);
							num2 = base.clippedSamples.Length - 2;
						}
						else if (wrapMode == Wrap.PingPong)
				...
				lastClippedPercent = ClipPercent(_result.percent);
				to.CopyFrom(base.clippedSamples[num2]);
				float num6 = Vector3.Distance(to.position, from.position);
				num += num6;
				if (num >= distance)
				{
					float num7 = num - distance;
					double t = 1.0 - (double)(num7 / num6);
					SplineResult.Lerp(from, to, t, _result);
					if (base.samplesAreLooped)
					{
						if (_direction == Spline.Direction.Forward && from.percent > to.percent)
						{
							_result.percent = DMath.Lerp(from.percent, 1.0, t);
						}
						else if (_direction == Spline.Direction.Backward && from.percent < to.percent)
						{
							_result.percent = DMath.Lerp(1.0, to.percent, t);
						}
					}
					CheckTriggersClipped(from.percent, _result.percent);
					break;
