	...
	public class SplineTracer : SplineUser
	{
		...
		public SplineResult offsettedResult
		{
			get
			{
				...
				Vector3 upwards = splineResult.direction;
				splineResult.position += splineResult.right * (motion.offset.x + vector.x) + splineResult.normal * (motion.offset.y + vector.y);
				Quaternion quaternion = customRotations.Evaluate(Quaternion.Euler(motion.rotationOffset) * Quaternion.LookRotation(splineResult.direction, splineResult.normal), splineResult.percent);
				splineResult.direction = quaternion * Vector3.forward;
				quaternion = customRotations.Evaluate(Quaternion.Euler(motion.rotationOffset) * Quaternion.LookRotation(splineResult.normal, upwards), splineResult.percent);
				splineResult.normal = quaternion * Vector3.forward;
				return splineResult;
			}
		}
		...
		public Node GetNextNode()
		{
			...
			_address.GetEvaluationValues(_result.percent, out var splineComputer, out percent, out direction);
			...
			int[] availableNodeLinksAtPosition = splineComputer.GetAvailableNodeLinksAtPosition(percent, direction);
			...
			{
				int num = splineComputer.pointCount - 1;
				int num2 = 0;
				for (int i = 0; i < availableNodeLinksAtPosition.Length; i++)
				{
					if (splineComputer.nodeLinks[availableNodeLinksAtPosition[i]].pointIndex < num)
					{
						num = splineComputer.nodeLinks[availableNodeLinksAtPosition[i]].pointIndex;
						num2 = i;
					}
				}
				return splineComputer.nodeLinks[availableNodeLinksAtPosition[num2]].node;
			}
			int num3 = 0;
			int num4 = 0;
			for (int j = 0; j < availableNodeLinksAtPosition.Length; j++)
			{
				if (splineComputer.nodeLinks[availableNodeLinksAtPosition[j]].pointIndex > num3)
				{
					num3 = splineComputer.nodeLinks[availableNodeLinksAtPosition[j]].pointIndex;
					num4 = j;
				}
			}
			return splineComputer.nodeLinks[availableNodeLinksAtPosition[num4]].node;
		}
		...
		public double TravelClipped(double start, float distance, Spline.Direction direction)
		{
			...
			double a = start;
			int num2 = ((direction == Spline.Direction.Forward) ? DMath.CeilInt(start * (double)(base.clippedSamples.Length - 1)) : DMath.FloorInt(start * (double)(base.clippedSamples.Length - 1)));
			float num3 = 0f;
			while (true)
			{
