...
namespace Dreamteck.Splines
{
	...
	public class PolygonColliderGenerator : SplineUser
	{
		...
		private void GeneratePath()
		{
			...
			{
				Vector2 vector = new Vector2(0f - base.clippedSamples[i].direction.y, base.clippedSamples[i].direction.x).normalized * base.clippedSamples[i].size;
				vertices[i] = new Vector2(base.clippedSamples[i].position.x, base.clippedSamples[i].position.y) + vector * size * 0.5f + vector * offset;
				vertices[base.clippedSamples.Length + (base.clippedSamples.Length - 1) - i] = new Vector2(base.clippedSamples[i].position.x, base.clippedSamples[i].position.y) - vector * size * 0.5f + vector * offset;
			}
		}
