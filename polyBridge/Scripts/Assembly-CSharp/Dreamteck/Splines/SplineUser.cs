	...
	public class SplineUser : MonoBehaviour
	{
		...
		public bool samplesAreLooped
		{
			get
			{
				...
				if (rootUser.computer.isClosed && _loopSamples)
				{
		...
		private void SampleComputer()
		{
			...
			if (uniformSample)
			{
				...
				{
					_address.Evaluate(_samples[j], _address.Travel(_samples[j - 1].percent, distance, Spline.Direction.Forward, num2));
				}
				...
				{
					_address.Evaluate(_samples[_samples.Length - 1], 1.0);
				}
			...
			else
			{
				...
				{
					double percent = (double)k / (double)(num2 - 1);
					if (computer.isClosed && k == num2 - 1)
					{
						percent = 0.0;
					}
					_address.Evaluate(_samples[k], percent);
					_samples[k].percent = percent;
				}
			}
			if (_samples.Length == 0)
			{
			...
			if (_samples.Length > 1 && _averageResultVectors)
			{
				...
				for (int l = 0; l < _samples.Length - 1; l++)
				{
					Vector3 normalized = (_samples[l + 1].position - _samples[l].position).normalized;
					_samples[l].direction = (vector + normalized).normalized;
					_samples[l].normal = (_samples[l].normal + _samples[l + 1].normal).normalized;
					vector = normalized;
				}
				if (computer.isClosed)
				{
		...
		public virtual double Travel(double start, float distance, Spline.Direction direction)
		{
			...
			double a = start;
			int num2 = ((direction == Spline.Direction.Forward) ? DMath.CeilInt(start * (double)(samples.Length - 1)) : DMath.FloorInt(start * (double)(samples.Length - 1)));
			float num3 = 0f;
			while (true)
			{
