...
namespace Dreamteck.Splines
{
	...
	public class ObjectBender : SplineUser
	{
		...
		public void BendObject(BendProperty p)
		{
			...
			if (p.applyRotation)
			{
				p.transform.rotation = bendResult.rotation * quaternion * p.originalRotation;
			}
			else
			{
			...
			if (p.editMesh != null)
			{
				for (int i = 0; i < p.vertexPercents.Length; i++)
				{
					...
					{
					case Axis.X:
						p.editMesh.normals[i] = Quaternion.LookRotation(bendResult.direction, bendResult.normal) * quaternion * Quaternion.FromToRotation(Vector3.up, bendResult.normal) * p.normals[i];
						break;
					case Axis.Y:
						p.editMesh.normals[i] = Quaternion.LookRotation(bendResult.direction, bendResult.normal) * quaternion * Quaternion.FromToRotation(Vector3.up, bendResult.normal) * p.normals[i];
						break;
			...
			if (p._editColliderMesh != null)
			{
				for (int j = 0; j < p.colliderVertexPercents.Length; j++)
				{
					...
					{
					case Axis.X:
						p.editColliderMesh.normals[j] = Quaternion.LookRotation(bendResult.direction, bendResult.normal) * quaternion * Quaternion.FromToRotation(Vector3.up, bendResult.normal) * p.colliderNormals[j];
						break;
					case Axis.Y:
						p.editColliderMesh.normals[j] = Quaternion.LookRotation(bendResult.direction, bendResult.normal) * quaternion * Quaternion.FromToRotation(Vector3.up, bendResult.normal) * p.colliderNormals[j];
						break;
			...
			for (int k = 0; k < p.splinePointPercents.Length; k++)
			{
				...
				{
				case Axis.X:
					splinePoint.normal = Quaternion.LookRotation(bendResult.direction, bendResult.normal) * quaternion * Quaternion.FromToRotation(Vector3.up, bendResult.normal) * splinePoint.normal;
					break;
				case Axis.Y:
					splinePoint.normal = Quaternion.LookRotation(bendResult.direction, bendResult.normal) * quaternion * Quaternion.FromToRotation(Vector3.up, bendResult.normal) * splinePoint.normal;
					break;
