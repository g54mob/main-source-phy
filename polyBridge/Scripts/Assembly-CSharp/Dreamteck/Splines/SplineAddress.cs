...
namespace Dreamteck.Splines
{
	...
	public class SplineAddress
	{
		...
		public void Evaluate(SplineResult result, double percent)
		{
			...
			{
				double percent2 = 0.0;
				GetEvaluationValues(percent, out var computer, out percent2, out var _);
				if (!(computer == null))
				{
					computer.Evaluate(result, percent2);
		...
		public double Project(Vector3 point, int subdivide = 4, double from = 0.0, double to = 1.0)
		{
			...
			for (int i = 0; i < _elements.Length; i++)
			{
				double num2 = PathToLocalPercent(from, i);
				double to2 = PathToLocalPercent(to, i);
				double num3 = _elements[i].computer.Project(point, subdivide, num2, to2);
				float sqrMagnitude = (_elements[i].computer.EvaluatePosition(num3) - point).sqrMagnitude;
		...
		public float CalculateLength(double from = 0.0, double to = 1.0)
		{
			...
			for (int i = 0; i < _elements.Length; i++)
			{
				double num2 = PathToLocalPercent(from, i);
				double to2 = PathToLocalPercent(to, i);
				num += _elements[i].computer.CalculateLength(num2, to2);
			}
			return num;
		}
