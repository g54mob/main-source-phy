	...
	public class CustomRotationModule
	{
		...
		public Quaternion Evaluate(Quaternion baseRotation, double time)
		{
			...
			for (int i = 0; i < keys.Count; i++)
			{
				...
				Quaternion quaternion = Quaternion.Euler(keys[i].rotation.x, keys[i].rotation.y, keys[i].rotation.z);
				baseRotation = Quaternion.Slerp(baseRotation, baseRotation * quaternion, num);
			}
			return baseRotation;
		}
	}
