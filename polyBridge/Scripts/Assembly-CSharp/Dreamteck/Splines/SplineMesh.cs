...
namespace Dreamteck.Splines
{
	...
	public class SplineMesh : MeshGenerator
	{
		...
		private void Generate()
		{
			...
			for (int j = 0; j < channels.Count; j++)
			{
				...
				case Channel.Type.Extrude:
				{
					...
					{
						double num4 = DMath.Lerp(channels[j].clipFrom, channels[j].clipTo, (double)l * num2 + num3);
						...
						Stretch(channels[j], combineMeshes[num], num4, to);
						num++;
					}
					if (num3 == 0.0)
					{
		...
		private void Place(Channel channel, TS_Mesh target, double percent)
		{
			...
			vertexMatrix.SetTRS(splineResult.position + right * (base.offset.x + vector.x) * splineResult.size + normal * (base.offset.y + vector.y) * splineResult.size + direction * base.offset.z, splineResult.rotation * channel.NextPlaceRotation(percent) * Quaternion.AngleAxis(base.rotation, Vector3.forward), channel.NextPlaceScale() * splineResult.size);
			...
			{
				target.colors[j] = meshDefinition.colors[j] * splineResult.color;
			}
		}
		...
		private void Stretch(Channel channel, TS_Mesh target, double from, double to)
		{
			...
			for (int i = 0; i < meshDefinition.vertexGroups.Count; i++)
			{
				...
				Vector2 vector = channel.NextOffset(splineResult.percent);
				Vector3 vector2 = channel.NextExtrudeScale(splineResult.percent);
				float num = channel.NextExtrudeRotation(splineResult.percent);
				vertexMatrix.SetTRS(splineResult.position + right * (base.offset.x + vector.x) * splineResult.size + normal * (base.offset.y + vector.y) * splineResult.size + direction * base.offset.z, splineResult.rotation * Quaternion.AngleAxis(base.rotation + num, Vector3.forward), vector2 * splineResult.size);
				normalMatrix = vertexMatrix.inverse.transpose;
				if (i == 0)
				{
