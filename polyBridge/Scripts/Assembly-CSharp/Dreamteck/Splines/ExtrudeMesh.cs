...
namespace Dreamteck.Splines
{
	...
	public class ExtrudeMesh : MeshGenerator
	{
		...
		internal class ExtrudableMesh
		{
			...
			private void GroupVertices(Axis axis)
			{
				...
				for (int i = 0; i < vertices.Length; i++)
				{
					...
					{
						vertexGroups.Add(new VertexGroup(num2, new int[1] { i }));
					}
					...
					{
						vertexGroups.Insert(num3, new VertexGroup(num2, new int[1] { i }));
					}
					else if (num3 < vertexGroups.Count - 1)
					{
						vertexGroups.Insert(num3 + 1, new VertexGroup(num2, new int[1] { i }));
					}
					else
					{
						vertexGroups.Add(new VertexGroup(num2, new int[1] { i }));
		...
		private void Generate()
		{
			...
			{
				double num3 = (double)i * num + num2;
				double to = (double)i * num + num - num2;
				if (combineMeshes[i] == null)
				{
					combineMeshes[i] = new TS_Mesh();
				}
				Stretch(extrudableMeshes[GetMeshIndex(i)], combineMeshes[i], num3, to);
				if (_spacing == 0.0)
				{
					useLastResult = true;
				}
		...
		private void TRS(ExtrudableMesh source, TS_Mesh target, double percent)
		{
			...
			case Axis.X:
				...
				break;
			case Axis.Y:
				quaternion = Quaternion.LookRotation(Vector3.up, Vector3.back);
				break;
			}
			vertexMatrix.SetTRS(splineResult.position + splineResult.right * base.offset.x + splineResult.normal * base.offset.y + splineResult.direction * base.offset.z, splineResult.rotation * Quaternion.AngleAxis(base.rotation, Vector3.forward) * quaternion, new Vector3(_scale.x, _scale.y, 1f) * splineResult.size);
		...
		private void Stretch(ExtrudableMesh source, TS_Mesh target, double from, double to)
		{
			...
			case Axis.X:
				...
				break;
			case Axis.Y:
				quaternion = Quaternion.LookRotation(Vector3.up, Vector3.forward);
				break;
			}
			for (int i = 0; i < source.vertexGroups.Count; i++)
			{
				...
				matrix4x.SetTRS(splineResult.position + splineResult.right * base.offset.x + splineResult.normal * base.offset.y + splineResult.direction * base.offset.z, splineResult.rotation * Quaternion.AngleAxis(base.rotation, Vector3.forward), new Vector3(_scale.x, _scale.y, 1f) * splineResult.size);
				if (i == 0)
				{
				...
				for (int j = 0; j < source.vertexGroups[i].ids.Length; j++)
				{
					...
					zero2 = quaternion * source.vertices[num];
					zero2.z = 0f;
					target.vertices[num] = matrix4x.MultiplyPoint3x4(zero2);
					zero2 = quaternion * source.normals[num];
					target.normals[num] = matrix4x.MultiplyVector(zero2);
					target.colors[num] = target.colors[num] * splineResult.color;
					zero = target.uv[num];
					switch (_tileUVs)
					{
					case TileUVs.U:
						zero.x = (float)splineResult.percent;
						break;
					case TileUVs.V:
						zero.y = (float)splineResult.percent;
						break;
					case TileUVs.UniformU:
						zero.x = CalculateLength(0.0, splineResult.percent);
						break;
					case TileUVs.UniformV:
						zero.y = CalculateLength(0.0, splineResult.percent);
						break;
					}
					target.uv[num] = new Vector2(zero.x * base.uvScale.x, zero.y * base.uvScale.y);
