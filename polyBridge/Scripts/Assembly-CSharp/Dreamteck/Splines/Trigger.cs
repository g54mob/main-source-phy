...
namespace Dreamteck.Splines
{
	...
	public class Trigger
	{
		...
		public void AddListener(MonoBehaviour behavior, string method, object arg)
		{
			...
			MethodInfo methodInfo = ((!(type == null)) ? behavior.GetType().GetMethod(method, new System.Type[1] { type }) : behavior.GetType().GetMethod(method, new System.Type[0]));
			if (methodInfo == null)
			{
				Debug.LogError("There is no overload of the method " + method + " that uses a " + type?.ToString() + " parameter");
				return;
			}
			AddAction();
