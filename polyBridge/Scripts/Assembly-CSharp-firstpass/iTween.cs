...
public class iTween : MonoBehaviour
{
	...
	{
		bool flag = delay > 0f;
		if (loopType == LoopType.pingPong && reverse)
		{
			flag = false;
		}
		if (flag)
	...
	public static void SetDelay(GameObject target, float delayValue)
	{
		Component[] components = target.GetComponents<iTween>();
		components = components;
		for (int i = 0; i < components.Length; i++)
		{
			((iTween)components[i]).delay = delayValue;
		}
	}
	...
	public static void Pause(GameObject target)
	{
		...
		for (int i = 0; i < components.Length; i++)
		{
			...
			if (iTween2.delay > 0f)
			{
				iTween2.delay -= iTween2.GetTimeToUse() - iTween2.delayStarted;
				iTween2.StopCoroutine("TweenDelay");
				iTween2.StopCoroutine("TweenRestart");
			}
