	...
	public static class ArchitectureInfo
	{
		...
		internal unsafe static void SetRuntimePlatform(RuntimePlatform platform)
		{
			...
			case RuntimePlatform.WiiU:
				...
				{
					Debug.Log(string.Concat("Odin Serializer detected whitelisted runtime platform ", platform, " and memory read test succeeded; enabling all unaligned memory read/writes."));
					Architecture_Supports_All_Unaligned_ReadWrites = true;
				}
				else
				{
					Debug.Log(string.Concat("Odin Serializer detected whitelisted runtime platform ", platform, " and memory read test failed; disabling all unaligned memory read/writes."));
				}
				break;
			default:
				Architecture_Supports_Unaligned_Float32_Reads = false;
				Architecture_Supports_All_Unaligned_ReadWrites = false;
				Debug.Log(string.Concat("Odin Serializer detected non-white-listed runtime platform ", platform, "; disabling all unaligned memory read/writes."));
				break;
			}
		}
	}
