...
namespace Sirenix.Serialization
{
	public static class UnitySerializationUtility
	{
		...
		{
			if (!(member is FieldInfo { IsStatic: false, IsInitOnly: false } fieldInfo))
			{
				return false;
			}
		...
		private static bool GuessIfUnityWillSerializePrivate(Type type)
		{
			...
			if ((object)type.Assembly == String_Assembly || (object)type.Assembly == HashSet_Assembly || (object)type.Assembly == LinkedList_Assembly)
			{
				return false;
			}
