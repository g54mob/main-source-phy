...
public class Event_OnNotification : EventNode
{
	...
	protected override bool ShouldRun(EventData data)
	{
		...
		if (data != null)
		{
			...
			nint num2 = (nint)data;
			Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v40 @ rdx_v2 (Il2CppClass<SleepyNodes.EventData_Notification>)+130]");
			object obj = 0;
			...
			if (num3 >= 0)
			{
				...
				if (0 == (nint)typeof(EventData_Notification))
				{
					if (!string.IsNullOrWhiteSpace(NotifID))
					{
						...
						if (!notifID.Equals((string)0, StringComparison.OrdinalIgnoreCase))
						{
							goto IL_010f;
						}
					}
