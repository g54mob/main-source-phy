...
public class PunchcardRuntime : MonoBehaviour
{
	...
	public bool OnCardUsed(float destroyDelay = 0f)
	{
		...
		{
			PunchcardDefinitionV2 currentDefinition = CurrentDefinition;
			if ((object)CurrentDefinition == null)
			{
				goto IL_010f;
			}
			if (currentDefinition.MaxUses > 0)
			{
				int remainingUses = currentDefinition.RemainingUses - 1;
				currentDefinition.RemainingUses = remainingUses;
				PunchcardDefinitionV2 currentDefinition2 = CurrentDefinition;
				if ((object)CurrentDefinition == null)
				{
					goto IL_010f;
				}
				if (currentDefinition2.RemainingUses <= 0)
				{
