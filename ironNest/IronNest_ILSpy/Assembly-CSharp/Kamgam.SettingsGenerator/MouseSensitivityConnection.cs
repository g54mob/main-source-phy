...
public class MouseSensitivityConnection : Connection<float>
{
	...
	public override float Get()
	{
		...
		{
			MouseSensitivitySetter mouseSensitivitySetter = ResolveSetter(allowCache: true);
			if (!(mouseSensitivitySetter == null))
			{
				return mouseSensitivitySetter.CurrentSensitivity;
			}
		}
		return 1f;
	}
	...
	public override void Set(float uiValue)
	{
		if (Application.isPlaying)
		{
			...
			MouseSensitivitySetter mouseSensitivitySetter = ResolveSetter(allowCache);
			if (mouseSensitivitySetter != null)
			{
				mouseSensitivitySetter.ChangeSensitivity(uiValue);
		...
		if (!string.IsNullOrWhiteSpace(_targetTag))
		{
			...
			if (gameObject != null)
			{
				if ((object)gameObject != null)
				{
					...
					if (!(obj == null))
					{
						_cachedSetter = (MouseSensitivitySetter)obj;
						return (MouseSensitivitySetter)obj;
					}
					if (!_logWarnings)
					{
						goto IL_023b;
					...
					if (array != null)
					{
						array[0] = "[MouseSensitivityConnection] GameObject '";
						string name = gameObject.name;
						array[1] = name;
						array[2] = "' (tag '";
						array[3] = _targetTag;
						array[4] = "') has no MouseSensitivitySetter component.";
						text = string.Concat(array);
						goto IL_01bf;
					}
				}
				return (MouseSensitivitySetter)(object)new NullReferenceException();
			}
			if (_logWarnings)
			{
				text = "[MouseSensitivityConnection] No GameObject found with tag '" + _targetTag + "'.";
				goto IL_01bf;
			}
		}
		else if (_logWarnings)
		{
			message = "[MouseSensitivityConnection] TargetTag is empty. Cannot resolve target.";
			goto IL_0264;
		}
		goto IL_023b;
		IL_0264:
